...
int  test_page_zone(int flag) 
{	 
        struct page * page = NULL;
	page = alloc_pages(flag,0); 	 //分配一个物理页
	if(!page) 
	{ 
...
{
        pr_info("alloc GFP_KERNEL page \n");
	test_page_zone(GFP_KERNEL);
        pr_info("GFP_HIGHUSER_MOVABLE page\n");
	test_page_zone(GFP_HIGHUSER_MOVABLE);
        pr_info("GFP_DMA page\n");
	test_page_zone(GFP_DMA);
        pr_info("GFP_HIGHMEM page\n");
	test_page_zone(__GFP_HIGHMEM);
        return 0;
}
