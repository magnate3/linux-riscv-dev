...
int mmap_kmem(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (vma->vm_pgoff == 0) {
		...
		ret = remap_pfn_range(vma, vma->vm_start,virt_to_phys(dma_handle) + vma->vm_pgoff, length, vma->vm_page_prot);
	}
	/* #endif */
...
static int __init mmap_alloc_init(void){
    ...
    alloc_ptr = dma_alloc_coherent (&dev, (NPAGES + 2) * PAGE_SIZE, &dma_handle, GFP_KERNEL);
