    ...
    //};
     ...
     if (PageAnon(page)) {
          ...
          __anon_vma_interval_tree_foreach(avc, &anon_vma->rb_root,  pgoff, pgoff) {
                vma = avc->vma;
                printk("vma 0x%lx-0x%lx flag %lx , vma task comm: %s and pid %d \n", vma->vm_start, vma->vm_end, vma->vm_flags, vma->vm_mm->owner->comm, vma->vm_mm->owner->pid);
          }
#if 0
		...
		size_t count, loff_t * data)
{
	...
	if (memcmp(buf, "findtask", 8) == 0) {
		...
		test_cow();
	else if (memcmp(buf, "testtlb", 7) == 0) {
		if (sscanf(buf + 7, "%lx", &val) == 1) {
			printk_pagetable(val);
		}
	}
