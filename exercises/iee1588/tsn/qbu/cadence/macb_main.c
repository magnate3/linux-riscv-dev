...
#include "macb.h"
  ...
  static int mmsl_sys_init(struct net_device *dev);
  //static int mmsl_sys_init(struct macb *bp);
  //struct  net_device *g_dev = NULL;
  struct macb *g_bp =NULL;
...
//#if TEST_MMSL
#if 0
        u32 mmsl = macb_readl(bp, MMSL_CONTROL);
	pr_info("before write, mmsl : %x \n", mmsl);
        macb_writel(bp, MMSL_CONTROL, MACB_BIT(PRE_ENABLE));
        //mmsl = macb_readl(bp, MMSL_CONTROL);
	pr_info("after write, mmsl control  : %x , mmsl status : %x \n",  macb_readl(bp, MMSL_CONTROL),  macb_readl(bp, MMSL_STATUS));
#endif
	return 0;
...
#if TEST_MMSL
static int mmsl_value;
#if 0
static ssize_t mmsl_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    if (!g_bp)
    {
	    return 0;
    }
   pr_info("mmsl control  : %x , mmsl status : %x \n",  macb_readl(g_bp, MMSL_CONTROL),  macb_readl(g_bp, MMSL_STATUS));
   return 0;
}
// mmsl control  : 20 , mmsl status : 4
static ssize_t mmsl_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    int value;
    sscanf(buf, "%x", &value);
    if (!g_bp)
    {
            pr_info("failt to set mmsl control %x \n", value);
	    return 0;
    }
    pr_info("set mmsl control %x \n", value);
    macb_writel(g_bp, MMSL_CONTROL, value);
    //if (on)
    //{
    //     pr_info("set mml control %u \n", value);
    //     //macb_writel(g_bp, MMSL_CONTROL, MACB_BIT(PRE_ENABLE));
    //     macb_writel(g_bp, MMSL_CONTROL, MACB_BIT(PRE_ENABLE));
    //}
    return count;
...
static ssize_t mmsl_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
   ...
   int offset;
   sscanf(buf, "%x", &offset);
   //pr_info("mmsl offset: %x , status : %x \n",  offset, hw_readl_native(g_bp,offset));
   pr_info("mmsl control  : %x , mmsl status : %x \n",  macb_readl(g_bp, MMSL_CONTROL),  macb_readl(g_bp, MMSL_STATUS));
   return 0;
}
// mmsl control  : 20 , mmsl status : 4
static ssize_t mmsl_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    ...
    if (1 == rw)
    {
    pr_info("set mmsl addr %x, value %x \n", addr, value);
    hw_writel_native(g_bp, addr, value);
    }
    else 
    {
         pr_info("mmsl addr: %x , status : %x \n",  addr, hw_readl_native(g_bp,addr));
    }
    return count;

}
static struct kobj_attribute mmsl_attribute = __ATTR(mms_value, 0664, mmsl_show, mmsl_store);
static struct kobject *mmsl_kobj = NULL;
//static int mmsl_sys_init(struct macb *bp)
static int mmsl_sys_init(struct net_device *dev)
{
    int retval;
    struct macb *bp;
    if (0 != strncmp(ETH0_NAME, dev->name,strlen(ETH0_NAME)))
    {
             return 0;
    }
    mmsl_kobj = kobject_create_and_add(dev->name, kernel_kobj);
    if (!mmsl_kobj)
        return -ENOMEM;
    retval = sysfs_create_file(mmsl_kobj, &mmsl_attribute.attr);
    if (retval)
        kobject_put(mmsl_kobj);
   bp = netdev_priv(dev);
   g_bp = bp;
   pr_info(" mmsl_sys_init successfully \n");
   return retval;
}
static void mmsl_remove(struct net_device *dev)
{
	struct macb *bp;
	bp = netdev_priv(dev);
	if (0 != strncmp(ETH0_NAME, dev->name,strlen(ETH0_NAME)))
