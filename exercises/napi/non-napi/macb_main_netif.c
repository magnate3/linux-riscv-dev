...
#define TEST_POLL_DRIVER 1
#define TEST_KERNEL_5_14_12 1
#define TEST_POLL_DRIVER_CEC 1
#define TEST_POLL_DRIVER_POLL 2
#define TEST_NON_POLL_DRIVER_GMAC 0
#define TEST_LOT_POLL_DRIVER_GMAC 0
#define GAMC0_REG_START 0x43080000 
#define GAMC0_REG_END   (0x43080000 + 0x4000-1)
#define GAMC1_REG_START 0x43090000 
#define GAMC1_REG_END   (0x43090000 + 0x4000-1)
#ifdef  TEST_POLL_DRIVER
...
static void kthread_hrtimer_init(void);
#define HRTIMER_TEST_CYCLE   0, (1000000)
#define ETHERNET_HEADER_SIZE    14 
void print_skb(struct sk_buff* skb);
#endif
#define TEST_NAPI_THREADED 0
...
#if TEST_LOT_POLL_DRIVER_GMAC
static void lot_macb_get_hwaddr(struct net_device *dev, bool master)
{
    char mas[ETH_ALEN] = {0x70,0xb3,0xd5,0x92,0xfa,0x3c};
    char slav[ETH_ALEN] = {0x70,0xb3,0xd5,0x92,0xfa,0x3d};
    if (master)
    {
	memcpy(dev->dev_addr, mas,ETH_ALEN);
    }
    else
    {
	 memcpy(dev->dev_addr, slav,ETH_ALEN);
    }
}
#endif
...
#ifdef TEST_POLL_DRIVER
#if (1 == TEST_POLL_DRIVER_POLL)
		pr_err("************ open interrupt*************");
		for (q = 0, queue = bp->queues; q < bp->num_queues; ++q, ++queue)
...
#if  TEST_POLL_DRIVER && !TEST_POLL_DRIVER_CEC
	rtnl_lock();
...
#ifdef TEST_POLL_DRIVER
	//  to need free skb , free skb in igh clear device
	if (tx_skb->skb) {
		  ...
		  int budget)
{
	...
	while (count < budget) {
		...
		print_hex_dump(KERN_DEBUG, "data: ", DUMP_PREFIX_ADDRESS, 16, 1,
	        ...
	        pr_info(" htons((0x88A4) %x =?  eth->h_proto %x \n ", htons(0x88A4) , eth->h_proto);
                //netif_rx(skb);
...
enum hrtimer_restart hrtimer_cb_func(struct hrtimer *timer) { 
     ...
     struct macb *bp = netdev_priv(g_dev);
     struct macb_queue *queue;
     unsigned int q;
     for (q = 0, queue = bp->queues; q < bp->num_queues; ++q, ++queue)
     {
           //pr_err("********** do recv poll driver pkgs  ***********");
...
static void kthread_hrtimer_init() {
...
void print_skb(struct sk_buff* skb)
{
       	...
       	else 
	{
        ...
        pr_err("sk_buff: len:%d  skb->data_len:%d  truesize:%d head:%0X  data:%0X tail:%d end:%d, mac_header: %u"
				            ,skb->len,skb->data_len,skb->truesize,(skb->head),(skb->data),(skb->tail),(skb->end), skb->mac_header);
...
#if 0
	pr_err( "poll: status = %08lx, budget = %d\n",
...
static irqreturn_t macb_interrupt(int irq, void *dev_id)
{
	...
	while (status) {
		...
		if (unlikely(!netif_running(dev))) {
                        pr_err( "*********** macb_interrupt dev state down and break ****************\n");
		...
		if (status & bp->rx_intr_mask) {
			...
			if (bp->caps & MACB_CAPS_ISR_CLEAR_ON_WRITE)
				queue_writel(queue, ISR, MACB_BIT(RCOMP));
#ifdef TEST_POLL_DRIVER
			//pr_err( "*********** start scheduling RX softirq ****************\n");
			if (napi_schedule_prep(&queue->napi)) {
				//pr_err( "*********** execute scheduling RX softirq ****************\n");
		...
		{
			//pr_err( "*********** start macb_tx_interrupt ****************\n");
...
static netdev_tx_t macb_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
...
#if (TEST_POLL_DRIVER && !TEST_POLL_DRIVER_CEC) 
      // need to create skb
      //pr_err(" htons((0x88A4) %x =? skb->protocol %x \n ", htons(0x88A4) , skb->protocol);
      skb = skb_copy(skb, GFP_ATOMIC);
      //if(htons(0x88A4) == skb->protocol) 
      //{
      //      pr_err("********** send poll driver pkgs  ***********");
      //}
      //return 0;
#endif
	...
	if (CIRC_SPACE(queue->tx_head, queue->tx_tail, bp->tx_ring_size) < 1)
		netif_stop_subqueue(dev, queue_index);
#if (2==TEST_POLL_DRIVER_POLL)
       struct macb_queue *queue2;
       unsigned int q;
       for (q = 0, queue2 = bp->queues; q < bp->num_queues; ++q, ++queue)
       {
        	macb_tx_interrupt(queue);
       }
#endif
...
static int macb_open(struct net_device *dev)
{
	...
	{
	    pr_err(" call macb_mac_link_up \n");
            macb_mac_link_up(&bp->phylink_config, phydev, MLO_AN_PHY,  bp->phy_interface, SPEED_100,  DUPLEX_FULL, 0, 0);
	}
#endif
#ifdef TEST_POLL_DRIVER_CEC
#endif
	return 0;
...
#ifdef TEST_POLL_DRIVER
		//err = request_irq(queue->irq, macb_interrupt,
	        //			       IRQF_SHARED|IRQF_NO_THREAD, dev->name, queue);
#if (1 == TEST_POLL_DRIVER_POLL)
...
static int macb_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(mem))
		return PTR_ERR(mem);
#if TEST_NON_POLL_DRIVER_GMAC
        pr_err("reg statrt : %0x, reg end : %x \n", regs->start, regs->end);
        //if (regs->start >= GAMC0_REG_START &&  regs->end <= GAMC0_REG_END)
        if (regs->start >= GAMC0_REG_START &&  regs->end <= GAMC1_REG_END)
        {
	     return -1;
        }
#endif
...
#if TEST_LOT_POLL_DRIVER_GMAC
      {
	pr_info("tracing lot_macb_get_hwaddr \n");
        if (regs->start >= GAMC1_REG_START &&  regs->end <= GAMC1_REG_END)
	{
	      lot_macb_get_hwaddr(dev, true);
	}
	else
	{
	      lot_macb_get_hwaddr(dev, false);
	}
     }
#else
	macb_get_hwaddr(bp);
