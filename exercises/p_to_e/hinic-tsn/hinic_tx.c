...
//#if 0
        ...
        {
	    txq2 = netdev_get_tx_queue(netdev, skb->queue_mapping);
            if (txq2)
            {
                  pr_err(" txq1 %p  and  skb->queue_mapping %d \n", txq2,  skb->queue_mapping);
            }
...
#if 1
	    txq2 = netdev_get_tx_queue(skb->dev, skb->queue_mapping);
            //txq = netdev_pick_tx(skb->dev, skb, NULL);
            if (txq2)
            {
                  //q = rcu_dereference_bh(txq2->qdisc);
                  q = txq2->qdisc;
                  pr_err(" txq2 %p , %p, status %d \n", txq2, q->enqueue ? q->enqueue : NULL, test_bit(__QDISC_STATE_DEACTIVATED, &q->state) );
                  if (unlikely(test_bit(__QDISC_STATE_DEACTIVATED, &q->state))) {
                      pr_err("qdsic will drop skb \n");
                 }  
                //  else if ((q->flags & TCQ_F_CAN_BYPASS) && !qdisc_qlen(q) && qdisc_run_begin(q)) {
                //      pr_err("qdsic will sch_direct_xmit skb \n");
                //       //if (sch_direct_xmit(skb, q, dev, txq, root_lock, true)) {
                //       // __qdisc_run(q);
                //}
                else {
                      pr_err("qdsic enqueue and qdsic run \n");
                }
            }
#endif
            rc = dev_queue_xmit(skb);
            pr_err("rc %d, rc == NET_XMIT_SUCCESS: %d \n", rc,  rc == NET_XMIT_SUCCESS);
        ...
        else
        {
	     ...
	     //pr_err(" src mac %s \t",  buf);
             //sha = eth->h_dest;
             //sprintf(buf, "%x:%x:%x:%x:%x:%x \0",sha[0], sha[1], sha[2], sha[3], sha[4], sha[5]);
	     //pr_err(" dst mac %s \t",  buf);
             //pr_err(" eth name %s \n", netdev->name);
        }
#endif
