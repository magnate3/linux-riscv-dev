...
phys_addr_t rte_mem_virt2phy(const void *virtaddr)
{
	...
	if ((page & 0x7fffffffffffffULL) == 0)
         {
              printf("++++++++++++++++++ no physical page alloced \n");
		return RTE_BAD_IOVA;
         } 
...
int main()
{
	...
	printf("addr1 = %p, 0x%x\n", addr1, (void*)rte_iova);
        free(addr1); 
	addr2 = (char*)mmap(0, size, PROT_READ | PROT_WRITE,
			     MAP_PRIVATE | MAP_ANONYMOUS|MAP_FIXED, -1, 0);
	if( RTE_BAD_IOVA == (phy=rte_mem_virt2phy((void *)addr2)))
        {
            printf("rte_mem_virt2phy err \n");
        }
	printf("addr2 = %p, %p\n", addr2, (void*)phy);
        *(addr2) = 1;
	printf("+++++++++++++++ after alloc physical  page  \n");
	if( RTE_BAD_IOVA == (phy=rte_mem_virt2phy((void *)addr2)))
        {
            printf("rte_mem_virt2phy err \n");
        }
	printf("addr2 = %p, %p\n", addr2, (void*)phy);
        addr3 = addr2 + getpagesize()*2;
	if( RTE_BAD_IOVA == (phy=rte_mem_virt2phy((void *)addr3)))
        {
            printf("rte_mem_virt2phy err \n");
        }
	printf("addr3 = %p, 0x%p\n", addr3, (void*)phy);
