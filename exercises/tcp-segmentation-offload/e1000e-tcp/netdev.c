			...
			unsigned int nr_frags)
{
	...
	if(IPPROTO_TCP == iph->protocol) {
	    ...
	    pr_info("tcp proto and mms : %u, src port %d, dst pot %d, skb_is_gso: %d, gso_segs: %u , skb_shinfo(skb)->nr_frags:%u, skb_is_nonlinear:%d, dma count: %d \n", skb_shinfo(skb)->gso_size, htons(th->source), htons(th->dest), skb_is_gso(skb), skb_shinfo(skb)->gso_segs, skb_shinfo(skb)->nr_frags, skb_is_nonlinear(skb), count);
	}
#endif
	return count;
