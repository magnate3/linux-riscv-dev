...
//44:A1:91:A4:9C:0B
static struct rte_ether_addr gw_mac=
    {{0x0, 0x0, 0x0, 0x0, 0x0, 0x0}};
static struct rte_ether_addr net_ethaddr=
...
int ndisc_rcv(struct rte_mbuf *mbuf, struct netif_port *dev,uint16_t ip_offfset, uint16_t icmp6_offfset)
{
    ...
    case ND_NEIGHBOR_SOLICIT:
        ret = ndisc_recv_ns(mbuf,ipv6_hdr, msg, dev);
        break;

    case ND_NEIGHBOR_ADVERT:
        ret = ndisc_recv_na(mbuf,ipv6_hdr, dev);
        break;
