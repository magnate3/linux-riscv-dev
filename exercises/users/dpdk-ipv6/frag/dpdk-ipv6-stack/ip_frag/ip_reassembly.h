...
struct ip_frag_pkt_ext {
	RTE_TAILQ_ENTRY(ip_frag_pkt_ext) lru;      /* LRU list */
...
#define RTE_TAILQ_HEAD(name, type) TAILQ_HEAD(name, type)
 /* fragments tailq */
RTE_TAILQ_HEAD(ip_pkt_list, ip_frag_pkt_ext);
...
struct rte_ip_frag_tbl {
	...
	struct ip_frag_pkt_ext *last;     /* last used entry. */
	struct ip_pkt_list lru;       /* LRU list for table entries. */
	struct ip_frag_tbl_stat stat; /* statistics counters. */
	__extension__ struct ip_frag_pkt_ext pkt[]; /* hash table. */
};
