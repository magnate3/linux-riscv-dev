...
void open_pcap(struct netif_port *dev,const char *fname)
{
...
static int process_tcp46(struct rte_mbuf *mbuf,struct rte_ether_hdr * eth_h)
{
    uint16_t data_len = 0;
    uint16_t l4_len = 0;
    ...
    data_len = mbuf->pkt_len - ip4_hdrlen(mbuf);
    l4_len  = ntohs(ip4_hdr(mbuf)->total_length) - ip4_hdrlen(mbuf);
    //dpdk_dump_tcph(tcp_hdr(mbuf), l4_len);
    prepare_xmit46_out(mbuf);
    eth_send_packets(&mbuf, mbuf->port,0,1);
    return 0;
free_mbuf:
    rte_pktmbuf_free(mbuf);
...
static int process_ipv4(struct rte_mbuf *mbuf,struct rte_ether_hdr * eth_h)
{
    ...
    if (ip_h->next_proto_id == IPPROTO_TCP) {
        return process_tcp46(mbuf,eth_h);
    }
