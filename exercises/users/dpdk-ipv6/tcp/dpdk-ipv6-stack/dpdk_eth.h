...
static inline void rte_eth_hdr_set(struct rte_ether_hdr*eth, uint16_t type, const struct rte_ether_addr*d_addr,
    const struct rte_ether_addr *s_addr)
{
    eth->ether_type = htons(type);
    rte_ether_addr_copy(s_addr, &eth->s_addr);
    rte_ether_addr_copy(d_addr, &eth->d_addr);
}
static void eth_addr_to_str(const struct eth_addr *mac, char *str)
{
    const uint8_t *num = mac->bytes;
    sprintf(str, "%02x:%02x:%02x:%02x:%02x:%02x", num[0], num[1], num[2], num[3], num[4], num[5]);
}
static void dump_eth_addr(const struct eth_hdr*eth)
{
    
    char str[64]={0};
    eth_addr_to_str(&eth->s_addr,str);
    printf("src mac : %s \t",str);
    eth_addr_to_str(&eth->d_addr,str);
    printf("dst mac : %s\n",str);
}
...
static inline int rte_eth_addr_is_zero(const struct rte_ether_addr *ea)
{
    return ((ea)->addr_bytes[0] == 0) && ((ea)->addr_bytes[1] == 0) && ((ea)->addr_bytes[2] == 0) &&
            ((ea)->addr_bytes[3] == 0) && ((ea)->addr_bytes[4] == 0) && ((ea)->addr_bytes[5] == 0);
}
