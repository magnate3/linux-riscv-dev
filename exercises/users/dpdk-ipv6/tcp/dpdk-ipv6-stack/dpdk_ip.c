...
protocol_process_tcp(struct rte_mbuf * mbuf,const void *data, uint16_t len)
{
...
#endif
    tcp_server_process6(g_work_space, mbuf);
    return DPDK_IP_SUCC;
}
static enum DPDK_IP_Return
...
#endif
static int parse_ip6_frag(void *data)
{
    struct ip6_frag *fraghdr = (struct ip6_frag *) data;
    if(IPPROTO_ICMPV6 == fraghdr->ip6f_nxt)
    {
        printf("frag nxt proto is icmp6 \n");
    }
    return 0;
}
static int
...
static struct rte_mbuf * process_ipv6_frag(struct rte_mbuf *mbuf)
{
     return ipv6_reassemble(mbuf);
}
static enum DPDK_IP_Return
...
protocol_process_ip_raw(struct rte_mbuf *mbuf)
{
    ...
    if (ether_type == RTE_ETHER_TYPE_IPV4) {
        ...
        if(IPPROTO_FRAGMENT == proto)
        {
              struct rte_mbuf * temp = NULL;
              temp = process_ipv6_frag(mbuf);
              if(temp)
              {
                 //printf("temp addr %p \n", temp);
                 //mbuf_dump(temp); 
                 //rte_pktmbuf_mtod(temp, void*);
                 return protocol_process_ip_raw(temp);
              }
              else
              {
                 return DPDK_IP_REASSEMBLY;
              }
        }
    ...
    switch (proto) {
        case IPPROTO_TCP:
            ...
            return protocol_process_tcp(mbuf,next, next_len);
        case IPPROTO_UDP:
