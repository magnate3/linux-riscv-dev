...
const char *gwip6str= TCP_CLIENT82_IP6;
...
static int ip6_add_hdr(struct ip6_hdr *ip6, struct in6_addr *src, struct in6_addr *dest,
	    ...
	    struct in6_addr *target,struct nd_msg * msg)
{
	...
	ndisc_insert_option(msg, ND_OPT_TARGET_LL_ADDR, (uint8_t*)(&net_ethaddr.addr_bytes),
			    INETHADDRSZ);
        icmp6_send_csum(ip6,&msg->icmph);
        // this is a bad code
        struct rte_ether_addr eth_addr;
        rte_eth_macaddr_get(DEFAULT_PORTID, &eth_addr);
        ether_addr_dump("local nic mac addr =", &eth_addr);
...
{
    //printf("%s not implement \n",__func__);   
    struct rte_ether_hdr *eth = mbuf_eth_hdr(mbuf);
    if (ipaddr_eq(&gw_ip6, &ip6h->ip6_src)
            && rte_eth_addr_is_zero(&gw_mac)) {
        //ip6_dump_hdr(ip6h);
...
static void icmp6_multicast_ether_addr(const struct in6_addr *daddr, struct rte_ether_addr *mac)
{
    ...
    memcpy(mac->addr_bytes + 2, &mcast_addr.s6_addr32[3], sizeof(uint32_t));
}
static void icmp6_ns_eth_hdr_push(struct rte_mbuf *m)
{
    ...
    rte_eth_hdr_set(eth, ETHER_TYPE_IPv6, &dmac, &net_ethaddr);
#endif
}
...
#if 1
    eth = mbuf_push_eth_hdr(m);
    rte_eth_hdr_set(eth, ETHER_TYPE_IPv6, &gw_mac,&net_ethaddr);
#endif
}
