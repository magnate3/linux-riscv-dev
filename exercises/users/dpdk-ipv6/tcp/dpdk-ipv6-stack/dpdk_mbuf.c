...
const void *mbuf_userdata_const(const struct rte_mbuf *mbuf, mbuf_usedata_field_t field)
{
    return (const void *)mbuf + mbuf_dynfields_offset[field];
}
...
void mbuf_log(struct rte_mbuf *m, const char *tag)
{
    ...
    if (eh->ether_type == htons(RTE_ETHER_TYPE_IPV4)) {
        ...
        push = ((flags & TH_PUSH) != 0);
        rst = ((flags & TH_RST) != 0);
        fprintf(log, "syn %d fin %d push %d ack %d rst %d seq %u ack %u \n",syn, fin, push, ack, rst,ntohl(th->th_seq), ntohl(th->th_ack));
    } else if (eh->ether_type == htons(RTE_ETHER_TYPE_ARP)) {
        fprintf(log, "muf: %s -> %s arp\n", smac, dmac);
    } else {
        fprintf(log, "muf: %s -> %s unkown type %x\n", smac, dmac, ntohs(eh->ether_type));
    }
#endif
}
