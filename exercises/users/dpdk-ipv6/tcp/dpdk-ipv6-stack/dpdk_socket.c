...
static bool rss_check_socket(struct work_space *ws, struct socket *sk)
{
     return false;
}
     ...
     uint32_t server_ip, uint16_t server_port)
{
    ...
    printf("%s server ip 0x%x,client ip 0x%x,server port %u, client port %u \n",__func__,ntohl(server_ip), ntohl(client_ip), \
                                   ntohs(server_port), ntohs(client_port));
...
int socket_table_init(struct work_space *ws)
{
    ...
    printf("%s server port min %u, server port max  %u, client port min %u, client port max %u \n",__func__,  st->server_port_min,  st->server_port_max\
     , st->client_port_min, st->client_port_max);
    if (cfg->client_hop) {
        st->client_hop = 1;
    }
