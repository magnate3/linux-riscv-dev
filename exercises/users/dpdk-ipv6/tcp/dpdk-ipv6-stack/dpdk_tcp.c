...
#define mbuf_free2(m) rte_pktmbuf_free(m)
...
static inline struct rte_mbuf *tcp_new_packet(struct work_space *ws, struct socket *sk, uint8_t tcp_flags)
{
    ...
    if (ws->vxlan) {
        ...
        //http_get_response();
        memcpy((char*)(th +1),data,strlen(data));
        printf("tcp payload %s",(char*)(th +1));
#endif
    }
    if (!ws->ipv6) {
        ...
        //ip6_dump_hdr(ip6h);
        //dump_rte_eth_hdr(mbuf_eth_hdr(m));
    }
...
static inline void eth_addr_swap(struct rte_ether_hdr *eth)
{
#if 0
    ...
    rte_ether_addr_copy(&eth->s_addr, &addr);
...
static void tcp_reply_rst(struct work_space *ws, struct rte_mbuf *m)
{
    ...
    if (iph->version == 4) {
        ...
        nlen = sizeof(struct rte_ether_hdr) + sizeof(struct iphdr) + sizeof(struct tcphdr);
    } else {
        tcp_rst_set_ipv6((struct ip6_hdr *)iph);
        tcp_rst_set_tcp(th);
        th->th_sum = RTE_IPV6_UDPTCP_CKSUM(iph, th);
        nlen = sizeof(struct rte_ether_hdr) + sizeof(struct ip6_hdr) + sizeof(struct tcphdr);
    }
...
#if 0
static struct iphdr * get_ip_hdr(struct rte_mbuf *m)
{
    struct iphdr *iph ;
    struct ip6_hdr *ip6h = NULL;
    struct rte_ether_hdr *eh = NULL;
    eh = mbuf_eth_hdr(m);
...
void tcp_server_process6(struct work_space *ws, struct rte_mbuf *m)
{
