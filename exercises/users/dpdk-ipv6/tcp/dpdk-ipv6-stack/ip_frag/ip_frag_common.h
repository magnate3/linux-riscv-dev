...
struct rte_mbuf * ip_frag_process(struct ip_frag_pkt_ext *fp,
		struct rte_ip_frag_death_row *dr, struct rte_mbuf *mb,
		uint16_t ofs, uint16_t len, uint16_t more_frags);

struct ip_frag_pkt_ext * ip_frag_find(struct rte_ip_frag_tbl *tbl,
		struct rte_ip_frag_death_row *dr,
		const struct ip_frag_key *key, uint64_t tms);

struct ip_frag_pkt_ext * ip_frag_lookup(struct rte_ip_frag_tbl *tbl,
	const struct ip_frag_key *key, uint64_t tms,
	struct ip_frag_pkt_ext **free, struct ip_frag_pkt_ext **stale);

/* these functions need to be declared here as ip_frag_process relies on them */
struct rte_mbuf *ipv4_frag_reassemble(struct ip_frag_pkt_ext *fp);
struct rte_mbuf *ipv6_frag_reassemble(struct ip_frag_pkt_ext *fp);
...
static inline void
ip_frag_free(struct ip_frag_pkt_ext *fp, struct rte_ip_frag_death_row *dr)
{
	uint32_t i, k;
...
static inline void
ip_frag_free_immediate(struct ip_frag_pkt_ext *fp)
{
	uint32_t i;
...
static inline void
ip_frag_inuse(struct rte_ip_frag_tbl *tbl, const struct  ip_frag_pkt_ext *fp)
{
...
/* reset the fragment */
static inline void
ip_frag_reset(struct ip_frag_pkt_ext *fp, uint64_t tms)
{
