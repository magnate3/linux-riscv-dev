...
static inline void
ip_frag_tbl_add(struct rte_ip_frag_tbl *tbl,  struct ip_frag_pkt_ext *fp,
...
ip_frag_tbl_reuse(struct rte_ip_frag_tbl *tbl, struct rte_ip_frag_death_row *dr,
	struct ip_frag_pkt_ext *fp, uint64_t tms)
{
	ip_frag_free(fp, dr);
	ip_frag_reset(fp, tms);
...
struct rte_mbuf *
ip_frag_process(struct ip_frag_pkt_ext *fp, struct rte_ip_frag_death_row *dr,
...
{
	struct ip_frag_pkt_ext *p1, *p2;
	struct ip_frag_pkt_ext *empty, *old;
	uint64_t max_cycles;
