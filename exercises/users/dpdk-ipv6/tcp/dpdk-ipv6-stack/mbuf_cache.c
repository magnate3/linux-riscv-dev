...
static struct rte_mempool *mbuf_pool_create(const char *str, uint16_t port_id, uint16_t queue_id)
{
    int socket_id = 0;
...
static int mbuf_data_push_l2(struct mbuf_data *mdata, struct netif_port *port)
{
    ...
    rte_ether_addr_copy(&port->gateway_mac, &eth.d_addr);
    rte_ether_addr_copy(&port->local_mac, &eth.s_addr);
    mdata->l2_len = len;
