...
#endif
int request_to_primary(struct malloc_mp_req *user_req)
{
        printf("%s not support multiply processes\n",__func__);
    return 0;
}
int
request_sync(void)
...
int
test_malloc_socket_to_heap_id(unsigned int socket_id)
{
...
void
test_rollback_expand_heap(struct rte_memseg **ms, int n_segs,
...
/* this function is exposed in malloc_mp.h */
struct malloc_elem *
test_alloc_pages_on_heap(struct malloc_heap *heap, uint64_t pg_sz, size_t elt_size,
...
fail:
	test_rollback_expand_heap(ms, n_segs, elem, map_addr, alloc_sz);
	return NULL;
}
		...
		size_t bound, bool contig)
{
	...
	elem = test_alloc_pages_on_heap(heap, pg_sz, elt_size, socket, flags, align,
...
static void *
test_malloc_heap_alloc_on_heap_id(const char *type, size_t size,
...
void *
test_malloc_heap_alloc(const char *type, size_t size, int socket_arg,
		unsigned int flags, size_t align, size_t bound, bool contig)
{
	...
	heap_id = test_malloc_socket_to_heap_id(socket);
	/* if heap id is negative, socket ID was invalid */
	if (heap_id < 0)
		return NULL;

	ret = test_malloc_heap_alloc_on_heap_id(type, size, heap_id, flags, align,
			bound, contig);
	...
	for (i = 0; i < (int) rte_socket_count(); i++) {
		if (i == heap_id)
			continue;
		ret = test_malloc_heap_alloc_on_heap_id(type, size, i, flags, align,
				bound, contig);
		if (ret != NULL)
			return ret;
	}
...
void *
test_malloc_heap_alloc_biggest(const char *type, int socket_arg, unsigned int flags,
...
/* this function is exposed in malloc_mp.h */
int
test_malloc_heap_free_pages(void *aligned_start, size_t aligned_len)
{
...
int
test_malloc_heap_free(struct malloc_elem *elem)
{
	struct malloc_heap *heap;
	...
	if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
		...
		test_malloc_heap_free_pages(aligned_start, aligned_len);
...
int
test_malloc_heap_resize(struct malloc_elem *elem, size_t size)
{
	int ret;
 ...
 */
int
test_malloc_heap_get_stats(struct malloc_heap *heap,
 ...
 */
void
test_malloc_heap_dump(struct malloc_heap *heap, FILE *f)
{
	struct malloc_elem *elem;
...
test_malloc_heap_create_external_seg(void *va_addr, rte_iova_t iova_addrs[],
...
struct rte_memseg_list *
test_malloc_heap_find_external_seg(void *va_addr, size_t len)
{
...
int
test_malloc_heap_destroy_external_seg(struct rte_memseg_list *msl)
{
...
int
test_malloc_heap_add_external_memory(struct malloc_heap *heap,
		struct rte_memseg_list *msl)
{
...
int
test_malloc_heap_remove_external_memory(struct malloc_heap *heap, void *va_addr,
		size_t len)
{
...
int
test_malloc_heap_create(struct malloc_heap *heap, const char *heap_name)
{
	struct rte_mem_config *mcfg = test_rte_eal_get_configuration()->mem_config;
...
int
test_malloc_heap_destroy(struct malloc_heap *heap)
{
...
int
test_rte_memseg_contig_walk_thread_unsafe(rte_memseg_contig_walk_t func, void *arg)
{
        struct rte_mem_config *mcfg = test_rte_eal_get_configuration()->mem_config;
        int i, ms_idx, ret = 0;
...
int
test_rte_memseg_contig_walk(rte_memseg_contig_walk_t func, void *arg)
{
        int ret = 0;

        /* do not allow allocations/frees/init while we iterate */
        rte_mcfg_mem_read_lock();
        ret = test_rte_memseg_contig_walk_thread_unsafe(func, arg);
        rte_mcfg_mem_read_unlock();
        return ret;
}

int
test_rte_eal_malloc_heap_init(void)
{
	struct rte_mem_config *mcfg = test_rte_eal_get_configuration()->mem_config;
	unsigned int i;
...
//##################################will cause dead ########################
	...
	return test_rte_memseg_contig_walk(malloc_add_seg, NULL);
}
