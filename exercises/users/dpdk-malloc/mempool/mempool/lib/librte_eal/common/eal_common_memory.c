...
{
	return test_rte_eal_get_configuration()->mem_config->nchannel;
}

/* return the number of memory rank */
unsigned rte_memory_get_nrank(void)
{
	return test_rte_eal_get_configuration()->mem_config->nrank;
}
...
rte_eal_memdevice_init(void)
{
	...
	config = test_rte_eal_get_configuration();
...
sync_memory(void *va_addr, size_t len, bool attach)
{
	...
	/* find our segment */
	msl = test_malloc_heap_find_external_seg(va_addr, len);
	if (msl == NULL) {
		rte_errno = ENOENT;
		ret = -1;
		goto unlock;
