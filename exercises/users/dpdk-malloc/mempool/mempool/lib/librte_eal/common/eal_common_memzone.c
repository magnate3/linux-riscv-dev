		...
		unsigned int bound)
{
	...
	if (len == 0 && bound == 0) {
		...
		requested_len = 0;
		mz_addr = test_malloc_heap_alloc_biggest(NULL, socket_id, flags,
				align, contig);
	} else {
		if (len == 0)
			requested_len = bound;
		/* allocate memory on heap */
		mz_addr = test_malloc_heap_alloc(NULL, requested_len, socket_id,
	...
	if (mz == NULL) {
		...
		test_malloc_heap_free(elem);
		rte_errno = ENOSPC;
		return NULL;
	}

	strlcpy(mz->name, name, sizeof(mz->name));
	mz->iova = test_rte_malloc_virt2iova(mz_addr);
	mz->addr = mz_addr;
 ...
 */
int
test_rte_eal_memzone_init(void)
{
	struct rte_mem_config *mcfg;
	int ret = 0;
