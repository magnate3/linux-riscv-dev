	...
	void *opaque)
{
	...
	memhdr = test_rte_zmalloc("MEMPOOL_MEMHDR", sizeof(*memhdr), 0);
	if (memhdr == NULL)
		return -ENOMEM;
...
rte_mempool_get_page_size(struct rte_mempool *mp, size_t *pg_sz)
{
	...
	ret = test_rte_malloc_heap_socket_is_external(mp->socket_id);
	if (ret < 0)
		return -EINVAL;
...
rte_mempool_cache_create(uint32_t size, int socket_id)
{
	...
	cache = test_rte_zmalloc_socket("MEMPOOL_CACHE", sizeof(*cache),
	...
	int socket_id, unsigned flags)
{
	...
	te = test_rte_zmalloc("MEMPOOL_TAILQ_ENTRY", sizeof(*te), 0);
	if (te == NULL) {
	...
	int socket_id, unsigned flags)
{
	...
	if (ret)
		goto fail;
         RTE_LOG(ERR, MEMPOOL, "memsegs pool set ops\n");
