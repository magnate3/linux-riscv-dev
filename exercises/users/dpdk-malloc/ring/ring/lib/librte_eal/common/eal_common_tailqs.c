...
test_rte_eal_tailq_create(const char *name)
{
...
static void
test_rte_eal_tailq_update(struct rte_tailq_elem *t)
{
	if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
		/* primary process is the only one that creates */
		t->head = test_rte_eal_tailq_create(t->name);
	} else {
...
int
test_rte_eal_tailq_register(struct rte_tailq_elem *t)
{
