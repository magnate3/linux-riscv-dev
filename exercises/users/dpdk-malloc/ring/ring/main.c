...
int test_mempool()
{
      ...
      RTE_LOG(ERR, MEMPOOL, "****************** test memsegs pool begin \n");
      test_rte_mempool_init();
...
int test_ring()
{
    ...
    test_ring =  rte_ring_create("R1", RING_SIZE, SOCKET_ID_ANY, 0/*RING_F_SP_ENQ*/);
    enqueue_sample_data(test_ring); 
    print_ring(test_ring);
}
static int print_malloc_add_seg( struct rte_memseg_list *msl, struct rte_memseg *ms)
{
