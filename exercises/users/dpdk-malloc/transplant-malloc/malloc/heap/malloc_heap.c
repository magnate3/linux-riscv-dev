...
void *
test_test_malloc_heap_alloc_biggest(const char *type, int socket_arg, unsigned int flags,
...
/* this function is exposed in malloc_mp.h */
int
test_test_malloc_heap_free_pages(void *aligned_start, size_t aligned_len)
{
...
test_malloc_heap_free(struct malloc_elem *elem)
{
	...
	if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
		...
		test_test_malloc_heap_free_pages(aligned_start, aligned_len);
...
test_test_malloc_heap_create_external_seg(void *va_addr, rte_iova_t iova_addrs[],
...
int
test_test_malloc_heap_destroy_external_seg(struct rte_memseg_list *msl)
{
