...
static int mp_secondary_handle(const struct rte_mp_msg *mp_msg, const void *peer)
{
     char *param = mp_msg->param;
     printf("%s msg %s \n",__func__, param);
     reply_msg(param,peer);
     return 0;
}
static int mp_primary_handle(const struct rte_mp_msg *mp_msg, const void *peer)
{
     char *param = mp_msg->param;
     printf("%s msg %s \n",__func__, param);
     reply_msg("I'm ack you",peer);
     return 0;
}
