...
init_port(void)
{
	...
	struct rte_eth_dev_info dev_info;
        ...
        rss_conf.rss_key_len = I40E_RSS_HKEY_LEN,
	...
	if (ret != 0)
		rte_exit(EXIT_FAILURE,
			"Error during getting device (port %u) conf: %s\n",
			port_id, strerror(-ret));
#endif
...
main(int argc, char **argv)
{
	...
	if (!flow) {
        ...
        ip_format_addr(buf,IP_BUF_LEN,rte_be_to_cpu_32(ip)); 
        printf("flow dst ip<%u> : %s \n",rte_be_to_cpu_32(ip),buf);
        ...
        //generate_udp_fdir_rule(port_id, selected_queue,IPV4_ADDR(10,11,11,65),5000);
        fdir_get_infos(port_id);
	main_loop();
