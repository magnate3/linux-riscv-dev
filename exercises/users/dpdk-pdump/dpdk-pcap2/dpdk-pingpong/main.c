...
void write_pcap(struct rte_mbuf *mbuf);
...
static inline void close_pcap(pcap_t *pcap)
{
     if (NULL == pcap)
     {
         return;
...
void write_pcap(struct rte_mbuf *mbuf)
{
    ...
    */
     ret = pcap_sendpacket(pcap, rte_pktmbuf_read(mbuf, 0, len, temp_data), len);
     if (unlikely(ret != 0))
     {
...
static pcap_t * test_open_single_rx_pcap(const char *pcap_filename)
{
        pcap_t *pcap;
        pcap = pcap_open_offline(pcap_filename, errbuf);
        if (pcap == NULL) {
                fprintf(stderr, "Couldn't open %s: %s", pcap_filename,
                        errbuf);
                return NULL;
        }
...
#else
static inline void test_close_pcap_dumper(pcap_dumper_t * dumper)
{
     if (NULL == dumper)
     {
         return;
 ...
 *   */
static uint16_t
test_eth_pcap_tx_dumper( struct rte_mbuf *mbuf)
{
     unsigned char temp_data[RTE_ETH_PCAP_SNAPLEN];
     size_t len;
     //int ret = 0;
 ...
 *                                   * a pointer to temp_data after copying into it.
 *                                                    */
     pcap_dump((u_char *)dumper, &header, rte_pktmbuf_read(mbuf, 0, len, temp_data));
...
int main(int argc, char **argv)
{
    ...
    ret = 0;
     ...
     hz = rte_get_timer_hz();
     //g_pcap = test_open_single_rx_pcap("test.pcap"); 
     //g_pcap = open_iface_live("enp125s0f3"); 
     g_dumper = test_open_single_tx_pcap("test.pcap"); 
#endif
...
#if TEST_PACAP    
    //close_pcap(g_pcap);
    test_close_pcap_dumper(g_dumper);
#endif
