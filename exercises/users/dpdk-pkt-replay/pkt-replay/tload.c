...
int load_vxlan_trace_line(FILE *fp, struct packet_model *pm)
{
    ...
    //add udp hdr
    struct rte_udp_hdr *vx_udp_hdr = (struct rte_udp_hdr*)(&(pm->vxlan.udp));
    vx_udp_hdr->dst_port = htons(4789);
    vx_udp_hdr->src_port = htons(9999);
    vx_udp_hdr->dgram_len = htons(sizeof(struct rte_vxlan_hdr) + sizeof(struct rte_udp_hdr) + pkt_length - 4);
    //add ip hdr
    struct rte_ipv4_hdr *vx_ip_hdr = (struct rte_ipv4_hdr*)(&(pm->vxlan.ip));
    vx_ip_hdr->version_ihl = 0x45;
    vx_ip_hdr->total_length = htons(ntohs(vx_udp_hdr->dgram_len) + sizeof(struct rte_ipv4_hdr));
