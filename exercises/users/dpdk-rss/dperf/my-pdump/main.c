...
static void icmp_dump_hdr(const struct rte_mbuf *mbuf)
{
    ...
    if (unlikely( IPPROTO_ICMP  != iph->next_proto_id)) {
         return;
    }
...
#if 1
    ipv4_addr_dump("  IPV4: src=", iph->src_addr);
    ipv4_addr_dump(" dst=", iph->dst_addr);
    fprintf(stderr, "\tlcore %d port %d rss hash %u icmp type %u code %u id %u seq %u\n",
            lcore, mbuf->port, mbuf->hash.rss, ich->icmp_type, ich->icmp_code,
