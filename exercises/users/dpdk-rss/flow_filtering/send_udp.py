...
def test(network):
    ...
    print(ip)
    payload = 'a' * 100
    packet = IP(dst=ip,id=12345)/UDP(dport=5000)/payload
    frags=fragment(packet,fragsize=500) 
    for frag in frags:
        pkt = Ether(src= new_src_mac,dst= new_dst_mac,type=0x800)/frag
        sendp(pkt,verbose=0,iface='enahisic2i3') #sending packet at layer 2
def main():
    print("send udp pkt")
    payload = 'a' * 100
    packet = IP(dst="10.10.103.251",id=12345)/UDP(dport=5000)/payload
    frags=fragment(packet,fragsize=500) 
    for frag in frags:
        pkt = Ether(src= new_src_mac,dst= new_dst_mac,type=0x800)/frag
        sendp(pkt,verbose=0,iface='enahisic2i3') #sending packet at layer 2
