...
static void thread_loop(uint16_t port_id, uint16_t queue_id)
{
    ...
    while (!should_stop()) {
        ...
        if (rx_count > 0) {
            ...
            printf("Received %d packet/s on lcore %d, queue %u\n", rx_count, rte_lcore_id(),queue_id);
            int i;
                                 ...
                                 struct rte_eth_conf *port_conf)
{
    *port_conf = (struct rte_eth_conf){
            .rxmode = {
                    ...
                    .offloads = RTE_DEV_RX_OFFLOAD_CHECKSUM,  //rte_ethdev.h
#endif
                    .offloads = RTE_ETH_RX_OFFLOAD_CHECKSUM,  //rte_ethdev.h
                            ...
                            const struct rte_eth_conf *port_conf)
{
    ...
    for (queue_id = 0; queue_id < ctx->nb_queues; queue_id++) {
...
static void device_rss_configure(int portid, int nb_queues) {
    ...
    rss_action_conf.level = 0;
    rss_action_conf.types = RTE_ETH_RSS_NONFRAG_IPV4_OTHER;
    rss_action_conf.key_len = rss_conf.rss_key_len;
    rss_action_conf.key = rss_conf.rss_key;
    ...
    rss_action_conf.func = RTE_ETH_HASH_FUNCTION_SYMMETRIC_TOEPLITZ;
    rss_action_conf.level = 0;
    rss_action_conf.types = RTE_ETH_RSS_NONFRAG_IPV4_TCP;
    rss_action_conf.key_len = rss_conf.rss_key_len;
    rss_action_conf.key = rss_conf.rss_key;
    rss_action_conf.queue_num = 0;
    rss_action_conf.queue = NULL;
    ...
    rss_action_conf.func = RTE_ETH_HASH_FUNCTION_SYMMETRIC_TOEPLITZ;
    rss_action_conf.level = 0;
    rss_action_conf.types = RTE_ETH_RSS_NONFRAG_IPV4_UDP;
    rss_action_conf.key_len = rss_conf.rss_key_len;
    rss_action_conf.key = rss_conf.rss_key;
    rss_action_conf.queue_num = 0;
    rss_action_conf.queue = NULL;
...
int main(int argc, char *argv[])
{
    ...
    rte_eal_mp_remote_launch(thread_main, &ctx, CALL_MAIN);
