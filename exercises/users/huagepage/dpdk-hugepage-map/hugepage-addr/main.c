...
#define HUGEPAGE_FILE "/mnt/huge/test1"
//#define LENGTH (524288UL*1024)   // 1M
#define LENGTH (1024UL * 1024 * 1)   // 1M
#define PROTECTION (PROT_READ | PROT_WRITE)
//#define ADDR (void *)(0x10000UL)
#define ADDR (void *)(0x0UL)

#ifndef MAP_HUGETLB
#define MAP_HUGETLB 0x40
#endif
//#define FLAGS (MAP_SHARED | MAP_POPULATE|MAP_HUGETLB)
...
int main()
{
	...
	if (hugepage_fd < 0) {
		printf("Fail to open hugepage memory file: %s", HUGEPAGE_FILE);
		exit(1);
	}

	//addr = mmap(ADDR, LENGTH, PROTECTION, FLAGS, hugepage_fd, 0);
	addr = mmap(NULL, LENGTH, PROTECTION, FLAGS, hugepage_fd, 0);
	if (addr == MAP_FAILED) {
		perror("mmap");
