...
void*  producer(ngx_slab_pool_t  *shpool)
{
	...
	while (!quit_signal) {
#endif
		int_data = (int*)ngx_slab_alloc(shpool,sizeof(int));
		if(NULL == int_data)
		{
		    lfqueue_sleep(1000);
		...
		*int_data = i++;
		...
		/*Enqueue*/
		while (sh_lfqueue_enq(sh_q, int_data)) {
			printf("ENQ FULL?\n");
			lfqueue_sleep(10);
			--loop;
			if(loop <= 0)
...
void* ngx_lfqueue_alloc(void *pl, size_t sz) {
	...
	ngx_slab_pool_t  *sp;
        sp = (ngx_slab_pool_t *) pl; 
 	return ngx_slab_alloc( sp, sz);
 }
void ngx_lfqueue_free(void *pl, void *ptr) {
	        //printf("-----------%s---------------\n", __func__);
	ngx_slab_pool_t  *sp;
        sp = (ngx_slab_pool_t *) pl; 
 	ngx_slab_free( sp, ptr);
}
...
int init_sh_queue_func(share_lfqueue_t * sh_q)
{
     sh_q->sh_malloc = ngx_lfqueue_alloc;
     sh_q->sh_free = ngx_lfqueue_free;
     return 0;
}
#if !(TEST_MAP)
share_lfqueue_t * queue_create(const int key,share_lfqueue_t * sh_q, ngx_slab_pool_t *shpool)
{
        int shmid;
	lfqueue_t * queue;
        char *m;
        int alloc_size =  sizeof(lfqueue_t); 
        ...
        if ((m = (char *)shmat(shmid, NULL, 0)) == NULL)
	{
        ...
        queue = (lfqueue_t*)m;
        queue->shm_addr = (void*)shpool;
        sh_q->shm_addr = (void*)shpool;
	sh_q->lf_q  = queue;
	sh_q->sh_malloc = ngx_lfqueue_alloc;
	sh_q->sh_free = ngx_lfqueue_free;
#if 1
        //if (lfqueue_init_mf(queue, sh_q, ngx_lfqueue_alloc, ngx_lfqueue_free) == -1) {
        if (lfqueue_init_sh_mf(sh_q,shpool) == -1) {
#else
...
share_lfqueue_t *queue_open(int key, share_lfqueue_t *sh_q)
{
        int shmid;
        char *m;
	lfqueue_t * queue;
