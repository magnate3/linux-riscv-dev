...
*/
    ...
    pool->last = pool->pages + pages; /*pool->start  一样*/
...
{ //这儿假设page_size是4KB
    ...
    if (size > ngx_slab_max_size) { //如果是large obj, size >= 2048B
        ...
        //分配1个或多个内存页
        page = ngx_slab_alloc_pages(pool, (size >> ngx_pagesize_shift) + ((size % ngx_pagesize) ? 1 : 0)); //例如size刚好是4K,则page=1,如果是4K+1，则page=2
        if (page) {
                ...
                for (n = 0; n < map; n++) {
                    ...
                    if (bitmap[n] != NGX_SLAB_BUSY) {//如果page的obj块空闲,也就是bitmap指向的32个obj是否都已经被分配出去了
                        ...
                        for (m = 1, i = 0; m; m <<= 1, i++) {//如果obj块被占用,继续查找  从这32个obj中找出第一个未被分配出去的obj
                            ...
                            i = ((n * sizeof(uintptr_t) * 8) << shift)  + (i << shift); //该bit所处的整个bitmap中的第几位(例如需要3个bitmap表示所有的slab块，则现在是第三个bitmap的第一位，则i=32+32+1-1,bit从0开始)
...
ngx_slab_alloc_pages(ngx_slab_pool_t *pool, ngx_uint_t pages) //与ngx_slab_free_pages配合阅读理解
{
    ...
    */
		...
		if(NULL == page->next)
		{
			ngx_slab_error(pool, NGX_LOG_ALERT, "NULL == page->next");
		}
		//if(NULL == page->next->prev)
		//{
		//	ngx_slab_error(pool, NGX_LOG_ALERT, "NULL == page->next->pre");
		//}
...
static void
ngx_slab_error(ngx_slab_pool_t *pool, ngx_uint_t level, char *fmt,...)
{
    /*ngx_log_error(level, ngx_cycle->log, 0, "%s%s", text, pool->log_ctx);*/
#if 0
    char buffer[NGX_MAX_ERROR_STR];
    //last = errstr + NGX_MAX_ERROR_STR;
    va_list argptr;
    va_start(argptr, fmt);
    vsnprintf(buffer, NGX_MAX_ERROR_STR, format, ap);
    va_end(argptr);
    fprintf(stderr,text);
#endif
