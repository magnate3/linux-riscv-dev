...
void *handleConnectionThread(void *c)
{
	...
	while(1)
	{
		...
		if(FD_ISSET(net_fd, &rd_set))
		{
			...
			src.sin_addr.s_addr =  iphdr->source_ip;
			dst.sin_addr.s_addr =  iphdr->dest_ip;
			//dst.sin_addr.s_addr = ntohl(iph->daddr);
			//dst.sin_addr.s_addr = ntohl(iph->daddr);
			printf("################### source_ip %x , %s  and dest_ip %x, %s \n", ntohl(iphdr->source_ip), inet_ntoa(src.sin_addr) , ntohl(iphdr->dest_ip),  inet_ntoa(dst.sin_addr));
			//printf("################### NET2TAP src ip : %s, and dst ip : %s  \n", inet_ntoa(src.sin_addr),  inet_ntoa(dst.sin_addr));
#endif
