...
static void complete_tx(struct xsk_socket_info *xsk)
{
	...
	if(ret < 0 && (errno == ENXIO || errno == ENOBUFS || errno == EOPNOTSUPP))
	{
...
static bool filter_pass_icmp(__u32 saddr, __u32 daddr, int type, int code ) {
	fprintf(stdout,"icmp code %d type %d \n",code,type);
	return true ;
}
static inline void csum_replace2_ipv4(uint16_t* sum, uint16_t old, uint16_t new) {
...
static bool send_ipv6(struct xsk_socket_info *xsk,
			   uint64_t addr, uint32_t len, struct ethhdr *eth, struct ipv6hdr *ipv6)
{
     int ret;
     uint32_t tx_idx = 0;
...
#if 1
     printf("ipv6 Source MAC: %02x:%02x:%02x:%02x:%02x:%02x", eth->h_source[0], eth->h_source[1], eth->h_source[2], eth->h_source[3], eth->h_source[4], eth->h_source[5]);
...
static bool process_ipv6(struct xsk_socket_info *xsk,
			   uint64_t addr, uint32_t len)
{
    ...
    uint8_t currenthdr;
    currenthdr = ipv6->nexthdr;
    if(IPPROTO_FRAGMENT == currenthdr)
    {
	  ip6_frag = (struct ip6_frag *)(ipv6 +1);
	  if(IPPROTO_ICMP6 == ip6_frag->next_header) 
	      ...
	      offset = ntohs(ip6_frag->offset_flags) & IP6_F_MASK;
              printf("frag is icmp6 header, more %u, offset: %u,id: %u \n",more,offset, ip6_frag->ident);
              if(0 == offset && more)
              {
	            struct icmp6hdr *icmp = (struct icmp6hdr *) (ip6_frag + 1);
	      ...
	      return send_ipv6(xsk,addr,len,eth, ipv6);
          }
    }
		if (len > (sizeof(*eth) + sizeof(*ipv6) + sizeof(*icmp)) &&
		    IPPROTO_ICMPV6 ==  currenthdr && icmp->icmp6_type == ICMPV6_ECHO_REQUEST)
                { 
			   ...
			   uint64_t addr, uint32_t len)
{
	...
	if (ntohs(eth->h_proto) == ETH_P_IP &&
		...
		if ( protocol == IPPROTO_TCP ) {
			...
			filter_pass_tcp(saddr, daddr, sourceport, destport ) ;
		}
		else if ( protocol == IPPROTO_UDP ) {
			struct udphdr *tcp = (struct udphdr *) (ip + 1);
			__u32 sourceport=ntohs(tcp->source);
			__u32 destport=ntohs(tcp->dest) ;
			filter_pass_udp(saddr, daddr, sourceport, destport ) ;
		}
			...
			int code=icmp->code ;
		        ...
		        if(icmp->type != ICMP_ECHO)
	  	            return false;
			filter_pass_icmp(saddr, daddr, type,code);
		        memcpy(tmp_mac, eth->h_dest, ETH_ALEN);
		        memcpy(eth->h_dest, eth->h_source, ETH_ALEN);
		        memcpy(eth->h_source, tmp_mac, ETH_ALEN);
		        ...
		        icmp->type = ICMP_ECHOREPLY;
                        ...
                        csum_replace2_ipv4(&icmp->checksum, ICMP_ECHO, ICMP_ECHOREPLY);
		        ...
		        ret = xsk_ring_prod__reserve(&xsk->tx, 1, &tx_idx);
		        if (ret != 1) {
				printf("xsk_ring_prod__reserve fail \n");
		        	/* No more transmit slots, drop the packet */
		        	return false;
		        }
		        xsk_ring_prod__tx_desc(&xsk->tx, tx_idx)->addr = addr;
    ...
    {
	    return process_ipv6(xsk, addr, len);
    }
	return false;
}
