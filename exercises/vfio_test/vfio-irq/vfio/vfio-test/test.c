...
int main(int argc, const char **argv)
{
	...
	/* Open the group */
	dev_vfio_info.group = open("/dev/vfio/24", O_RDWR);
	...
	for (i = 0; i < dev.vfio_device_info.num_regions; i++) {
        ...
        qemu_vfio_pci_init_irq(dev.device_fd,VFIO_PCI_MSIX_IRQ_INDEX);
# if 0
         {
                ...
                for (i = 0; i < dev.vfio_device_info.num_irqs; i++) {
                        ...
                        //if ( (VFIO_PCI_MSI_IRQ_INDEX != irq->index) &&  (VFIO_PCI_MSIX_IRQ_INDEX != irq->index))
#if 1
                        if ( VFIO_PCI_MSI_IRQ_INDEX != irq->index)
                        {
                            printf(" index %u not msi(%d) or msix(%d)  interrupt \n",irq->index, VFIO_PCI_MSI_IRQ_INDEX  , VFIO_PCI_MSIX_IRQ_INDEX );
                            close(irqfd);
                            continue ;
                        }

                        if (vfio_irqfd_init(dev.device_fd, irq->index,irq->count, irqfd)) {
...
#else
                        if ( VFIO_PCI_MSIX_IRQ_INDEX != irq->index)
                        {
                            printf(" index %u not msi(%d) or msix(%d)  interrupt \n",irq->index, VFIO_PCI_MSI_IRQ_INDEX  , VFIO_PCI_MSIX_IRQ_INDEX );
                            close(irqfd);
                            continue ;
                        ...
                        if (-1 == vfio_enable_msix(dev.device_fd, irq->count, irqfd)) {
                                printf("  error while settion IRQ num.%d\n", irq->index);
                                close(irqfd);
                                goto error1;
                        }
