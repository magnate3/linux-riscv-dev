...
''' Get standard deviation result '''
def std_dev(input_tuple_list, index):
    input_list = [x[index] for x in input_tuple_list]
    if len(input_list) > 0:
        avg = average_result(input_tuple_list, index)
        return (float(sum([y**2 for y in [float(x)- avg for x in input_list]]))/len(input_list))**0.5
    else:
        return 0
...
def std_dev_fct_result(input_tuple_list):
    return std_dev(input_tuple_list, 1)
...
def print_result(results):
    ...
    print '%d' % (average_fct_result(results))
    print '%d' % (std_dev_fct_result(results))
    print '%d' % (cdf_fct_result(results, 0.99))
    print '%d' % (cdf_fct_result(results, 0.75))
    print '%d' % (cdf_fct_result(results, 0.50))
    print '%d' % (average_goodput_result(results))
