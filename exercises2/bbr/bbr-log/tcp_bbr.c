...
static const int bbr_pacing_gain[] = {
	...
	BBR_UNIT * 1 / 2,	/* drain queue and/or yield bw to other flows */
	BBR_UNIT, BBR_UNIT, BBR_UNIT,	/* cruise at 1.0*bw to utilize pipe, */
	BBR_UNIT, BBR_UNIT, BBR_UNIT	/* without creating excess queue... */
};
#endif
			 ...
			 u32 acked, u32 bw, int gain)
{
	...
	if (bbr->mode == BBR_PROBE_RTT)  /* drain queue, refresh min_rtt */
		tp->snd_cwnd = min(tp->snd_cwnd >> 1, bbr_cwnd_min_target);
}
...
static void bbr_reset_startup_mode(struct sock *sk)
{
	...
	bbr->cwnd_gain	 = bbr_high_gain;
        //if(((struct inet_sock*)sk)->inet_dport == 5202)
        {
...
static u32 bbr_undo_cwnd(struct sock *sk)
{
	...
	bbr->full_bw_cnt = 0;
	bbr_reset_lt_bw_sampling(sk);
        //if(ntohs(((struct inet_sock*)sk)->inet_dport) == 5202)
        //if(((struct inet_sock*)sk)->inet_dport == 5202)
        {
        pr_err("mode 2 mode=%d,min_rtt_us=%d,full_bw=%d,cycle_idx=%d,pacing_gain=%d,cwnd_gain=%d,rtt_cnt=%d\r\n",bbr->mode,bbr->min_rtt_us,bbr->full_bw,bbr->cycle_idx,bbr->pacing_gain,bbr->cwnd_gain,bbr->rtt_cnt);
        }
