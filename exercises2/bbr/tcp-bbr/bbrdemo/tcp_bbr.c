...
static const uint32_t bbr_extra_acked_max_us = 100 * 1000;
...
static uint32_t bbr_max_bw(const struct sock *sk)
{
...
static uint32_t bbr_bw(const struct sock *sk)
{
	struct bbr *bbr = inet_csk_ca(sk);
 ...
 */
static uint16_t bbr_extra_acked(const struct sock *sk)
{
	struct bbr *bbr = inet_csk_ca(sk);
...
static unsigned long bbr_bw_to_pacing_rate(struct sock *sk, uint32_t bw, int gain)
{
	uint64_t rate = bw;

	rate = bbr_rate_bytes_per_sec(sk, rate, gain);
	rate = min_t(uint64_t, rate, sk->sk_max_pacing_rate);
	return rate;
}
...
static void bbr_init_pacing_rate_from_rtt(struct sock *sk)
{
	...
	bw = (uint64_t)tp->snd_cwnd * BW_UNIT;
	do_div(bw, rtt_us);
	sk->sk_pacing_rate = bbr_bw_to_pacing_rate(sk, bw, bbr_high_gain);
}

/* Pace using current bw estimate and a gain factor. */
static void bbr_set_pacing_rate(struct sock *sk, uint32_t bw, int gain)
{
	struct tcp_sock *tp = tcp_sk(sk);
	struct bbr *bbr = inet_csk_ca(sk);
...
static uint32_t bbr_min_tso_segs(struct sock *sk)
{
 ...
 */
static uint32_t bbr_bdp(struct sock *sk, uint32_t bw, int gain)
{
	struct bbr *bbr = inet_csk_ca(sk);
	uint32_t bdp;
	uint64_t w;
 ...
 */
static uint32_t bbr_quantization_budget(struct sock *sk, uint32_t cwnd)
{
	struct bbr *bbr = inet_csk_ca(sk);
...
static uint32_t bbr_inflight(struct sock *sk, uint32_t bw, int gain)
{
	uint32_t inflight;
 ...
 */
static uint32_t bbr_packets_in_net_at_edt(struct sock *sk, uint32_t inflight_now)
{
	struct tcp_sock *tp = tcp_sk(sk);
	struct bbr *bbr = inet_csk_ca(sk);
	uint64_t now_ns, edt_ns, interval_us;
	uint32_t interval_delivered, inflight_at_edt;

	now_ns = tp->tcp_clock_cache;
	edt_ns = max(tp->tcp_wstamp_ns, now_ns);
	interval_us = div_u64(edt_ns - now_ns, NSEC_PER_USEC);
	interval_delivered = (uint64_t)bbr_bw(sk) * interval_us >> BW_SCALE;
...
static uint32_t bbr_ack_aggregation_cwnd(struct sock *sk)
{
	uint32_t max_aggr_cwnd, aggr_cwnd = 0;

	if (bbr_extra_acked_gain && bbr_full_bw_reached(sk)) {
		max_aggr_cwnd = ((uint64_t)bbr_bw(sk) * bbr_extra_acked_max_us)
				/ BW_UNIT;
	...
	struct sock *sk, const struct rate_sample *rs, uint32_t acked, uint32_t *new_cwnd)
{
	struct tcp_sock *tp = tcp_sk(sk);
	struct bbr *bbr = inet_csk_ca(sk);
	uint8_t prev_state = bbr->prev_ca_state, state = 0; //inet_csk(sk)->icsk_ca_state;
	 ...
	 */
	if (rs->losses > 0)
		cwnd = max_t(int32_t, cwnd - rs->losses, 1);
...
static void bbr_update_bw(struct sock *sk, const struct rate_sample *rs)
{
	 ...
	 */
	bw = div64_long((uint64_t)rs->delivered * BW_UNIT, rs->interval_us);
				       ...
				       const struct rate_sample *rs)
{
	...
	expected_acked = ((uint64_t)bbr_bw(sk) * epoch_us) / BW_UNIT;
	...
	bbr->ack_epoch_acked = min_t(uint32_t, 0xFFFFF,
				     bbr->ack_epoch_acked + rs->acked_sacked);
