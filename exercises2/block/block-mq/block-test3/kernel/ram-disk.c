...
static bool test_blk_mq_get_driver_tag(struct request *rq)
{
  ...
  {
         pr_err("get  tag fail   %u \n", rq->tag);
         return false;
  }
  //rq->tag = tag + tag_offset;
  if(tag + tag_offset == rq->tag) {
           pr_err("get tag   %u \n", rq->tag);
  }
  return true;
}
...
static inline struct blk_mq_hw_ctx *blk_mq_map_queue(struct request_queue *q, blk_opf_t opf, struct blk_mq_ctx *ctx)
{
	        return ctx->hctxs[blk_mq_get_hctx_type(opf)];
}
static inline void show_hctx_type(enum hctx_type type)
{
	   switch(type){
   	       case HCTX_TYPE_DEFAULT:
	           pr_err("***HCTX_TYPE_DEFAULT***\n");
		   break;
   	       case HCTX_TYPE_READ:
	           pr_err("*** HCTX_TYPE_READ***\n");
		   break;
   	       case HCTX_TYPE_POLL:
	           pr_err("***  HCTX_TYPE_POLL ***\n");
		   break;
	       default :
	           pr_err("unkown hctx type\n");
           }
}
...
static void blk_mq_test_req(struct request*rq)
{
    ...
    pr_err("q->nr_hw_queues %d, has elevator\n", q->nr_hw_queues);
    ...
    show_hctx_type(hctx->type);
    pr_info("show hctx type over\n");
    for_each_possible_cpu(i) {
	ctx2 = per_cpu_ptr(q->queue_ctx, i);
	//hctx2 = blk_mq_map_queue(q, data.cmd_flags, data.ctx);
	if(NULL == ctx2){
	   all_cpu = false;
           pr_err("soft ctx is null on cpu  %u, this cpu does not have soft ctx \n",i);
           continue;																
	}
	for (j = 0; j < set->nr_maps; j++) {
	    ...
	    pr_err("hctx index is %d \n", hctx_idx);
	    if (set->tags && !set->tags[hctx_idx]){
		struct blk_mq_tags *tags2 = set->tags[hctx_idx];
	       if( hctx->sched_tags == set->tags[hctx_idx]){
                 pr_err("hctx sched_tags == set->tags[hctx_idx %u]  \n",hctx_idx);
	       }
	       if( hctx->tags == set->tags[hctx_idx]){
                 pr_err("hctx tags == set->tags[hctx_idx %u]  \n",hctx_idx);
	       }
               if(rq == tags2->static_rqs[tag]){
                 pr_err("set ->tags[hctx_idx %u] -> staic_rqs[index  %u] euqal rq  \n",hctx_idx,tag);
               }
	    }
            //data.hctx = q->queue_hw_ctx[hctx_idx];
	    if(hctx == q->queue_hw_ctx[hctx_idx]){
#if 0
	            pr_err("queue_hw_ctx [index %u] equal hctx \n",hctx_idx);
#endif
	    }
            ...
            if(hctx2 == hctx){
#if 0
            pr_err("cpu%d 's ctx->hctxs [index %u] equal hctx \n",i,j);
#endif
	}
      }
    }
