	...
	if (tsk->bio_list) {
		...
		if (bio)
		{
			...
			if(bio_data_dir(bio) == READ)
			{
     	                                      ...
     	                                      printk(KERN_ALERT "read addr : %p \n", ptr);
			}
			else if(bio_data_dir(bio) == WRITE)
			{
    	                                      printk(KERN_ALERT "write addr : %p \n", ptr);
			}
...
// refer to ext4_walk_page_buffers
static int ext4_bio_write_page_test(struct page *page)
{
    ...
    do {
      ...
      disk = b_bdev->bd_disk;
      printk(KERN_ALERT "block_start : %u , blocknr %llu, disk name  %s \n", block_start, b_blocknr, disk->disk_name);
