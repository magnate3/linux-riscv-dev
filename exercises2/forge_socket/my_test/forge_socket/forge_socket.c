...
#endif
                ret.flight = tcp_packets_in_flight(tcp_sk(sk));
                ret.snd_nxt = tcp_sk(sk)->snd_nxt;
                ret.snd_cwnd = tcp_sk(sk)->snd_cwnd;
                ret.mss_cache = tcp_sk(sk)->mss_cache;
                ret.current_mss = test_tcp_current_mss(sk);
                ret.snd_ssthresh= tcp_sk(sk)->snd_ssthresh;
                ret.snd_cwnd_cnt= tcp_sk(sk)->snd_cwnd_cnt;
                 
		ret.snd_una = tcp_sk(sk)->snd_una;
		ret.snd_wnd = tcp_sk(sk)->snd_wnd;
		ret.rcv_wnd = tcp_sk(sk)->rcv_wnd;
		ret.rcv_nxt = tcp_sk(sk)->rcv_nxt;
		ret.rcv_wup = tcp_sk(sk)->rcv_wup;
