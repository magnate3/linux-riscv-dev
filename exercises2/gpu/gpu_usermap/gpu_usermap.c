...
vm_fault_t gpu_usermap_fault( struct vm_fault *vmf);
#else
int gpu_usermap_fault(struct vm_area_struct *vma, struct vm_fault *vmf);
#endif
...
int gpu_usermap_fault(struct vm_area_struct *vma, struct vm_fault *vmf) {
	...
	resource_size_t offset = ((unsigned long)vmf->address - vma->vm_start) >> PAGE_SHIFT;
#else
