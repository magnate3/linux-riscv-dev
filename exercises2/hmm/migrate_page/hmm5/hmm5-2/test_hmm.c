...
static unsigned long vm_page_find = 0;
module_param(vm_page_find, ulong, 0644);
static unsigned long sym = 0;
module_param(sym, ulong, 0644);
static const struct dev_pagemap_ops dmirror_devmem_ops;
static const struct mmu_interval_notifier_ops dmirror_min_ops;
static dev_t dmirror_dev;
//g_page_offset will be set in dmirror_migrat
static unsigned long  g_page_offset = 0 & ~PAGE_MASK;
static unsigned long  g_addr = 0;
static int count = 0;
struct dmirror_device;

void test_migrate_vma_finalize(struct migrate_vma *migrate);
...
static void cmp_page(struct mm_struct *mm , const struct page * src, const struct page *dst)
{
	...
	//pr_info(" enter cmp page<%dth> src page %p and dst page %p \n", ++count, src, dst);
	...
	else
	{
	     ...
	     page_offset = g_addr & ~PAGE_MASK;
             //addr1 += page_offset;
	     //memcpy(buf,addr1,32);
	     //pr_info("src buf is %s \n", buf);
             //memset(buf, 0, sizeof(buf));
             addr2 += page_offset;
	     //memcpy(buf,addr2,32);
	     //pr_info("dst buf is %s \n", buf);
	     get_random_bytes(&seq,sizeof(seq));
	     snprintf(addr2, sizeof(seq) + 1 , "%d", seq);
	     memcpy(addr2 + sizeof(seq),str1,strlen(str1)+1);
	     //pr_info("src page @ %p, phy addr in src page %lu \n",src, (page_to_pfn(src)<< PAGE_SHIFT) + page_offset);
	     //pr_info("dst page @ %p, phy addr in dst page %lu\n", dst, (page_to_pfn(dst)<< PAGE_SHIFT) + page_offset);
#endif
}
					   ...
					   struct dmirror *dmirror)
{
	...
	for (addr = args->start; addr < args->end; addr += PAGE_SIZE,
		...
		if (spage)
		{
		        pr_info("%s call copy highpage dpage @%p <not use> ,rpage @ %p, spage @ %p \n", __func__,dpage,rpage, spage);
			copy_highpage(rpage, spage);
			if(addr == args->start)
			{
	                    struct mm_struct *mm = dmirror->notifier.mm;
			    cmp_page(mm, spage, rpage);
			}
		}
		else
...
void test_migrate_vma_finalize(struct migrate_vma *migrate)
{
	...
	unsigned long i;
        //void (*test_remove_migration_ptes)(struct page *old, struct page *new, bool locked) = 0xffffffff9a0f02a0 ;
        //void (*test_remove_migration_ptes)(struct page *old, struct page *new, bool locked) = kallsyms_lookup_name("remove_migration_ptes");
        void (*test_remove_migration_ptes)(struct page *old, struct page *new, bool locked) = sym;
	for (i = 0; i < npages; i++) {
		...
		test_remove_migration_ptes(page, newpage, false);
		unlock_page(page);
		...
		if (newpage != page) {
			...
			{
			        pr_info("put  newpage %p \n",newpage);
				put_page(newpage);
			}
			else
			{
			        pr_info("put lru newpage %p \n",newpage);
				putback_lru_page(newpage);
			}
		}
	}
}
static int print_swp_pte(struct page *page,  unsigned long mpfn)
{
     swp_entry_t entry;
     pte_t swp_pte;
     ...
     if (is_device_private_entry(entry))
          printk(KERN_INFO "************* device private entry");
     return 0;
}
static int test_migrate_vma_collect_pmd(struct mm_struct *mm,pte_t *ptep, unsigned long addr)
{
    pte_t pte = *ptep;
	...
	if (!pte_present(pte)) {
			...
			if (is_write_device_private_entry(entry))
				mpfn |= MIGRATE_PFN_WRITE;
                        printk(KERN_INFO "************* pte not present, and it's device page @ %p", page);
		} else {
			...
			if (is_zero_pfn(pfn)) {
				mpfn = MIGRATE_PFN_MIGRATE;
                                printk(KERN_INFO "************* zero page frame struct is @ %p", page);
				return 0;
				//migrate->cpages++;
				//goto next;
			}
	                if(0 != vm_page_find)
	                {
	                     struct page *(*vm_normal_page)(struct vm_area_struct *vma, unsigned long addr, pte_t pte) = vm_page_find;
			     page = vm_normal_page(vma, addr, pte);
		        }
			mpfn = migrate_pfn(pfn) | MIGRATE_PFN_MIGRATE;
			mpfn |= pte_write(pte) ? MIGRATE_PFN_WRITE : 0;
                        printk(KERN_INFO "*************  pte present,sys page frame struct is @ %p", page);
	                if(0 != vm_page_find)
	                {
				//coredump
			     //print_swp_pte(page,mpfn);
		        }
		}
     //printk(KERN_INFO "swap pte,sys page frame struct is @ %p, and user paddr %lu, virt addr %lu", page, paddr, addr);
     return 0;
}
...
static struct page * test_walk_page_table(struct mm_struct *mm ,unsigned long addr)
{
    ...
    //test_migrate_vma_collect_pmd(mm,ptep,  addr);
    if(is_swap_pte(pte)){
	//u64 frame = 0, flags = 0;
	 ...
	 if (is_device_private_entry(entry))
         {
	      ...
	      if(0 != vm_page_find)
	      {
	           struct page *(*vm_normal_page)(struct vm_area_struct *vma, unsigned long addr, pte_t pte) = vm_page_find;
	           page = vm_normal_page(vma, addr, pte);
                   paddr = (page_to_pfn(page)<< PAGE_SHIFT) | page_offset;
                   printk(KERN_INFO "swap pte,sys page frame struct is @ %p, and user paddr %lu, virt addr %lu", page, paddr, addr);
	      }
#if 0
	      pfn = pte_pfn(pte);
	  ...
	  {
              printk(KERN_INFO "swap pte,but not device private entry,pte %s",pte_present(pte) ?"present": "not present" );
	  }
    }
    else
    {
	...
	page_addr = pte_pfn(pte) << PAGE_SHIFT;
        ...
        //paddr = page_addr + page_offset;
        paddr = page_addr | page_offset;
        printk(KERN_INFO "page frame struct is @ %p, and kernel paddr %lu", page, paddr);
    }
#endif
 out1:
    pte_unmap(ptep);
...
static int print_page( struct page * page, unsigned long addr)
{
	char buf[64] = {0};
	unsigned long page_offset=0, addr1;
	addr1 =  (unsigned long)page_address(page); 
	page_offset = addr & ~PAGE_MASK;
	addr1 += page_offset;
        memcpy(buf,(void*)addr1,32);
	pr_info("sys buf is %s \n", buf);
	return 0;
}
			   ...
			   struct hmm_dmirror_cmd *cmd)
{
	...
	pr_info("********* begin do dmirror migrate\n");
	page = test_walk_page_table(mm,start);
	//print_page(page,start);
	//g_page_offset = start  & ~PAGE_MASK;
	g_addr = start;
	if (end < start)
		return -EINVAL;
	...
	mmap_read_unlock(mm);
	mmput(mm);
	//pr_info("after test_migrate_vma_finalize : \n");
	//test_walk_page_table(mm,start);
					...
					unsigned long arg)
{
	...
	case HMM_DMIRROR_MIGRATE:
		...
		pr_info("dmirror migrate return val: %d \n", ret);
		break;
						      ...
						      struct dmirror *dmirror)
{
	...
	for (addr = start; addr < end; addr += PAGE_SIZE,
		...
		dpage2 = alloc_page_vma(GFP_HIGHUSER_MOVABLE, args->vma, addr);
