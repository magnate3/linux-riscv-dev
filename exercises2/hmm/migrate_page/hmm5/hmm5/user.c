...
int main()
{
	...
	self = (struct hmm2 *)malloc(sizeof(struct hmm2));
        //printf("self physical addr is present %lu\n",mem_virt2phy(self));
	init_hmm2(self);
	npages = 1;
	...
	ASSERT_EQ(ret, 0);
        paddr = mem_virt2phy(buffer->ptr);
        printf("**** after first migrate: \n Physical address is %llu\n", (unsigned long long)paddr);
	//virt_to_phys_user(&paddr, getpid(), (unsigned long)buffer->ptr);
        //printf("**** after first migrate: \n call virt_to_phys_user Physical address is %llu\n", (unsigned long long)paddr);
	memset(buf, 0 ,sizeof(buf));
	memcpy(buf, buffer->ptr, sizeof(buf) -1);
	printf("content is %s \n", buf);
	//virt_to_phys_user(&paddr, getpid(), (unsigned long)buffer->ptr);
        paddr = mem_virt2phy(buffer->ptr);
        printf("**** after first migrate and swap page fault :  Physical address is %llu\n", (unsigned long long)paddr);

#if 0
	ret = hmm_dmirror_cmd(self->fd0, HMM_DMIRROR_MIGRATE, buffer, 1);
	ASSERT_EQ(ret, 0);
        paddr = mem_virt2phy(buffer->ptr);
        printf("**** after send migrate: \n Physical address is %llu\n", (unsigned long long)paddr);
	memset(buf, 0 ,sizeof(buf));
	memcpy(buf, buffer->ptr, sizeof(buf) -1);
	printf("content is %s \n", buf);
#endif
