...
static char func_name2[NAME_LEN] = "ipv6_defrag";
...
static int entry_handler(struct kretprobe_instance *ri, struct pt_regs *regs)
{
	...
	if (IS_ERR_OR_NULL(data->skb)) {
        ...
        if(NULL == data->skb || SKB_MARK != (SKB_MARK & data->skb->mark))
        {
		return 1;
        }
	...
	data->e = (struct nf_hook_entries *) regs_get_kernel_argument(regs, 2);
	if (!data->e) {
...
static int ret_handler(struct kretprobe_instance *ri, struct pt_regs *regs)
{
	...
	if (NF_DROP_GETERR(NF_DROP) != verdict ||  -EPERM != verdict)
        {
	    return 1;
	...
	e = data->e;
        ...
        pr_info("%s - devin=%s/%d, devout=%s/%d,  proto=%d, "
		"verdict=0x%x\n", func_name,  devin,
...
static struct kretprobe my_kretprobe2 = {
	.handler		= ret_handler2,
	.entry_handler		= entry_handler2,
...
static int __init kretprobe_init(void)
{
	...
	my_kretprobe2.kp.symbol_name = func_name2;
