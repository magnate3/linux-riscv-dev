...
static void __iomem *clk_remap;

static int force_spi0_psclk_enable(int div)
{
        volatile int clk_status;
	int div_data = ((div & 0xff) << 16) | 0x5a0000a1;

        clk_remap = ioremap(0x43050000, 1024);
        if (!clk_remap)
                      return -ENOMEM;

        writel(0xa1, clk_remap + 0x1d4);
        writel(0xeb, clk_remap + 0x1d8);

        do{

                clk_status = readl(clk_remap + 0x1dc);

        }
        while(clk_status == 0x0);


     	printk("set spi0 sclk,div=%d\n",div);
        writel(div_data, clk_remap + 0x1e0);
        writel(0x5c3b00eb, clk_remap + 0x1e4);

        do{

                clk_status = readl(clk_remap + 0x1e8) & 0x01000000;
...
static int dw_spi_mmio_probe(struct platform_device *pdev)
{
	...
	//liang add
	ret = force_spi0_psclk_enable(1);
	printk("liang>> force spi clk enable!!!%d\n",ret);
