...
#4) change qcow2
#4.1 guestmount -a centos7.qcow2 -m /dev/sda3 /mnt ; 4.2 guestunmount /mnt
...
# root change pass virt-sysprep --root-password password:123456 -a bionic-server-cloudimg-arm64.img
 ...
 qemu-system-x86_64 -enable-kvm -smp 4 -m 8G  -M pc  -hda ubuntu-18.04-server-cloudimg-amd64.img     -netdev tap,id=tap0,ifname=tap0,script=no,downscript=no,vhost=on  -device virtio-net-pci,netdev=tap0,mac=52:55:00:d1:55:01  -nographic
 qemu-system-x86_64 -enable-kvm -smp 4 -m 8G  -M pc -kernel  /work/ovs_p4/vm_test/linux-4.18.9/k_install/vmlinuz-4.18.9  -append "console=ttyS0 root=/dev/sda1"  -hda ubuntu-18.04-server-cloudimg-amd64.img -netdev tap,id=tap0,ifname=tap0,script=no,downscript=no,vhost=on -device virtio-net-pci,netdev=tap0,mac=52:55:00:d1:55:01  -nographic
