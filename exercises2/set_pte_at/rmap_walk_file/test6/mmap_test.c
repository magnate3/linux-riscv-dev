...
{   
    ...
    char *addr;
    int map_size = 4096;
    fd = open("hybridmem.txt", O_RDWR);
    if(fd < 0) {
	   printf("Error : can't open file\n");
	   return -1;
	}
    addr = (char*)mmap(NULL, map_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    assert(addr != MAP_FAILED);
    strcpy(addr , "helloworld");
    printf("virt addr %lx, phy addr %lx\n",(long unsigned)addr,  mem_virt2phy(addr));
    read(devfd,addr,64);
    close(fd);
