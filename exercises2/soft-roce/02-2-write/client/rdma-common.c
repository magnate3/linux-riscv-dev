...
void on_completion(struct ibv_wc *wc)
{
  ...
  if (conn->send_state == SS_MR_SENT && conn->recv_state == RS_MR_RECV) {
    ...
    printf("peer mr rkey is %u and lkey is %u \n",  conn->peer_mr.rkey,  conn->peer_mr.lkey);
    printf("send packet to server : %s \n", conn->rdma_local_region);
...
void send_mr(void *context)
{
  ...
  printf("send mr rkey is %u and lkey is %u \n",  conn->rdma_remote_mr->rkey,  conn->rdma_remote_mr->lkey);
