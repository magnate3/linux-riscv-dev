...
struct IDX_BKT *ht_index=NULL;
char *ht_log=NULL;
struct PL_IT pipeline[2]={0};  // The pipeline
volatile struct KV *server_req_area=NULL;
volatile struct KV *server_resp_area=NULL;
volatile struct KV *client_req_area=NULL;
volatile struct UD_KV *client_resp_area=NULL;
struct ibv_mr *server_req_area_mr=NULL, *server_resp_area_mr=NULL;
struct ibv_mr *client_resp_area_mr=NULL, *client_req_area_mr=NULL;

struct stag server_req_area_stag[NUM_SERVERS]={0}, client_resp_area_stag[NUM_CLIENTS]={0};
...
uint16_t get_local_lid(struct ibv_context *context) {
  ...
  if (ibv_query_port(context, IB_PHYS_PORT, &attr)) return 0;
...
void create_qp(struct ctrl_blk *ctx) {
  ...
  for (i = 0; i < ctx->num_conn_qps; i++) {
    ctx->conn_cq[i] = ibv_create_cq(ctx->context, Q_DEPTH + 1, NULL, NULL, 0);
  ...
  for (i = 0; i < ctx->num_local_dgram_qps; i++) {
    ctx->dgram_cq[i] = ibv_create_cq(ctx->context, Q_DEPTH + 1, NULL, NULL, 0);
...
void modify_qp_to_init(struct ctrl_blk *ctx) {
  int i;
  struct ibv_qp_attr dgram_attr = {.qp_state = IBV_QPS_INIT,
                                   .pkey_index = 0,
  ...
  struct ibv_qp_attr conn_attr = {.qp_state = IBV_QPS_INIT,
                                  .pkey_index = 0,
                                  .port_num = IB_PHYS_PORT,
...
int setup_buffers(struct ctrl_blk *cb) {
  ...
  if (cb->is_client) {
    ...
    client_resp_area_mr =
        ibv_reg_mr(cb->pd, (char *)client_resp_area, RESP_AC * S_UD_KV, FLAGS);
    ...
    client_req_area_mr =
        ibv_reg_mr(cb->pd, (char *)client_req_area, WINDOW_SIZE * S_KV, FLAGS);
    ...
    if (cb->id == 0) {
      ...
      server_req_area_mr =
          ibv_reg_mr(cb->pd, (char *)server_req_area, M_2, FLAGS);
      CPE(!server_req_area_mr, "Failed to register server's request area",
          errno);
      ...
      server_resp_area_mr =
          ibv_reg_mr(cb->pd, (char *)server_resp_area, M_2, FLAGS);
      CPE(!server_resp_area_mr, "Failed to register server's response area",
          errno);
    } else {
...
void print_ht_index() {
  ...
  for (bkt_i = 0; bkt_i < NUM_IDX_BKTS; bkt_i++) {
    for (slot_i = 0; slot_i < 8; slot_i++) {
      ...
      fprintf(stdout, "%d|%lld\t", SLOT_TO_TAG(ht_index[bkt_i].slot[slot_i]),
              offset);
    }
    fprintf(stdout, "\n");
  }
...
void init_ht(struct ctrl_blk *cb) {
  ...
  int ht_log_sid = shmget(BASE_HT_LOG_SHM_KEY + cb->id, LOG_SIZE, shm_flags);
  if (ht_log_sid == -1) {
