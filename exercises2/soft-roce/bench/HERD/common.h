...
#define SLOTS_PER_BKT \
  8  // Number of <tag, pointer> pairs in a MICA-style index bkt
...
#define CL_SEMI_BTCH_SZ \
  64  // Client posts new RECVs if the number drops below this
...
// SHM keys for servers. The request and response area is shared among server
// processes
// and needs a single key. For lossy index and circular log, each server uses a
// separate
...
struct ctrl_blk {
  ...
  struct qp_attr
      *local_dgram_qp_attrs;  // Local and remote queue pair attributes
...
extern struct stag server_req_area_stag[NUM_SERVERS],
    client_resp_area_stag[NUM_CLIENTS];
...
#define LL long long
#define KEY_TO_BUCKET(k) \
  ((int)(k >> 16) & NUM_IDX_BKTS_)         // 3 bytes (up to 16 Mi buckets)
#define KEY_TO_TAG(k) ((int)(k & 0xffff))  // 2 bytes
#define KEY_TO_SERVER(k) \
  ((int)((k >> 40) % (NUM_SERVERS - 1) + 1))  // DO NOT shift by 48
