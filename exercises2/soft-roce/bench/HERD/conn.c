...
int connect_ctx(struct ctrl_blk *ctx, int my_psn, struct qp_attr dest,
  ...
  struct ibv_qp_attr conn_attr = {
      ...
      .rq_psn = dest.psn,
      .ah_attr = {.is_global = (is_roce() == 1) ? 1 : 0,
...
void server_exch_dest(struct ctrl_blk *cb) {
  ...
  int on = 1, status = -1;
  status = setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (const char *)&on,
                      sizeof(on));
  if (-1 == status) {
  ...
  for (i = 0; i < NUM_CLIENTS; i++) {
    ...
    server_req_area_stag[0].buf = (uint64_t)(unsigned long)server_req_area;
