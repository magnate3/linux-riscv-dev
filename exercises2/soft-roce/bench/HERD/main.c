  ...
  for (k = 1; k <= 2; k++) {
    ...
    if (k == 1) {
      if (req_type == PUT_TYPE) { /*PUT*/
        // If there is ANY chance that the written KV will overflow the log,
        // reset
    ...
    for (i = 0; i < NUM_CLIENTS; i++) {
      ...
      if ((num_resp & M_1_) == M_1_ && num_resp > 0 && num_resp != last_resp) {
        ...
        fprintf(stderr, "Server %d, IOPS: %f, used fraction: %f\n", cb->id,
                M_1 / seconds, (double)log_head / LOG_SIZE);
        clock_gettime(CLOCK_REALTIME, &start);
        last_resp = num_resp;
      }
      ...
      // Is the output legit?
      if (pipeline_out->req_type != DUMMY_TYPE &&
          pipeline_out->req_type != EMPTY_TYPE) {
        ...
        cb->wr.send_flags = (num_resp & WS_SERVER_) == 0
                                ? MY_SEND_INLINE | IBV_SEND_SIGNALED
                                : MY_SEND_INLINE;
  ...
  for (iter = 0; iter < NUM_ITER; iter++) {
    ...
    if ((iter & M_1_) == M_1_ && iter != 0) {
      ...
      fprintf(stderr, "Avg wait = %f, avg fail = %f\n",
              (double)wait_cycles / M_1, (double)num_fails / M_1_);
      wait_cycles = 0;
      num_fails = 0;
    ...
    int req_offset = (sn * WINDOW_SIZE * NUM_CLIENTS) + (WINDOW_SIZE * cb->id) +
                     (num_req_arr[sn] & WINDOW_SIZE_);
    ...
    if (req_kv->len == 0) {  // GET
      ...
      cb->wr.wr.rdma.remote_addr =
          server_req_area_stag[0].buf + (req_offset * S_KV) + KV_KEY_OFFSET;
    } else {
      cb->sgl.addr = (uint64_t)(unsigned long)req_kv;
      cb->wr.sg_list->length = S_KV;
      cb->wr.wr.rdma.remote_addr =
          server_req_area_stag[0].buf + (req_offset * S_KV);
    }
    ...
    if (num_req - num_resp == WINDOW_SIZE) {
      ...
      if (pndng_keys[rws] != 0) {
        if (client_resp_area[ras].kv.len < GET_FAIL_LEN_1) {
          if (!valcheck(client_resp_area[ras].kv.value, pndng_keys[rws])) {
            fprintf(stderr, "Client %d get() failed in iter %d. ", cb->id,
                    num_resp);
      ...
      if (CLIENT_PRINT_LAT ==
          1) {  // Print latency so that we can compute percentiles
