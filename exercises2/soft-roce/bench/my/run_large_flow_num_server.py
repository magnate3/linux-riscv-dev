...
def start_test(small_qp_num, large_qp_num, small_msg_size, large_msg_size):
    ...
    #cmd_list = [
    #     SVR_CMD1 + " > test_result_s1_bw &",
    #     SVR_CMD2 + " > test_result_s2_bw &",
    #     SVR_CMD3 + " > test_result_s3_bw &",
    #     SVR_CMD4 + " > test_result_s4_bw &",
    #     SVR_CMD5 + " > test_result_s5_bw &",
    #]
    for i in range(0,small_qp_parellel):
        ...
        cmd = "{} > logs/test_result_s{}_bw &".format(cmd,i+1) 
        cmd_list.append(cmd)
    for i in range(0,large_qp_parellel):
        ...
        cmd = "{} > logs/test_result_s{}_bw &".format(cmd,i+1+small_qp_parellel) 
        cmd_list.append(cmd)
    for cmd in cmd_list:
        try:
            ...
            for large_qp_num in large_qp_num_list:
                ...
                for i in range(0,3600):
                    ...
                    if not RUNNING:
                        break
                stop()
                msg_rate = parse_throughput_file("logs/test_result_s1_bw", small_msg_size, large_msg_size) + \
                           parse_throughput_file("logs/test_result_s2_bw", small_msg_size, large_msg_size) + \
                           parse_throughput_file("logs/test_result_s3_bw", small_msg_size, large_msg_size) + \
                           parse_throughput_file("logs/test_result_s4_bw", small_msg_size, large_msg_size)
                if large_qp_num == 0:
                    bandwidth = 0
                else:
