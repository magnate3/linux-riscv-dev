...
#include <rdma/rw.h>
#define DEBUG 1
struct dentry *debugfs_root2 = NULL;
struct dentry *send_file2 = NULL;
...
static int read_data_by_rdma_read(struct rdma_connection *rdma_d);
...
static ssize_t send_file_write(struct file *file, const char __user *ubuf, size_t cnt, loff_t *ppos)
{
	...
	if (rdma_cc)
	{
            read_data_by_rdma_read(rdma_cc);
	 }
	 else
         {
	     printk(KERN_ERR "rdma cc is NULL.\n");
	 }
#else
	if (rdma_cc)
		recv_data(rdma_cc);
...
static const char * str2 = "hello cli from srv \n";
static int prepare_buffer(struct rdma_connection *rdma_c)
{
	...
	rdma_c->rdma_buf = (char *)__get_free_page(GFP_KERNEL | GFP_DMA);
	        if (IS_ERR(rdma_c->rdma_buf)) {
		             printk(KERN_ERR "alloc send_buf failed.\n");
		                goto free_rdma_buf;
		      }
	...
	rdma_c->send_dma_addr = ib_dma_map_single(rdma_c->pd->device, rdma_c->send_buf, PAGE_SIZE, DMA_BIDIRECTIONAL);
        rdma_c->rdma_dma_addr = ib_dma_map_single(rdma_c->pd->device, rdma_c->rdma_buf, PAGE_SIZE, DMA_BIDIRECTIONAL); 
	if (!rdma_c->rdma_buf || !rdma_c->send_dma_addr || !rdma_c->recv_dma_addr) {
		printk(KERN_ERR "map dma addr failed\n");
		goto free_dma_addr;
	}
	memcpy(rdma_c->rdma_buf, str2, len);
	rdma_c->rdma_buf[len] = 0;
#if DEBUG
        dma_unmap_addr_set(rdma_c, recv_mapping, rdma_c->send_dma_addr);
        dma_unmap_addr_set(rdma_c, send_mapping, rdma_c->send_dma_addr);
        dma_unmap_addr_set(rdma_c, rdma_mapping, rdma_c->rdma_dma_addr);
#endif
...
free_dma_addr:
	...
	if (rdma_c->rdma_buf)
		ib_dma_unmap_single(rdma_c->pd->device, (unsigned long)rdma_c->rdma_buf, PAGE_SIZE, DMA_BIDIRECTIONAL);
	free_page((unsigned long)rdma_c->rdma_buf);
free_rdma_buf:
...
static int do_rdma_accept(struct rdma_cm_id *cm_id, struct rdma_cm_event *event)
{
...
static int send_mr(struct rdma_connection *rdma_c)
{
	...
	sg_dma_address(&sg) = rdma_c->rdma_dma_addr;
	sg_dma_len(&sg) = PAGE_SIZE;
	printk(KERN_ERR "server reg_mr key=%d, dma_addr=0x%llx.\n", rdma_c->reg_mr_wr.key, rdma_c->rdma_dma_addr);
...
#if DEBUG
         rdma_c->reg_mr_wr.wr.num_sge = 0;
         rdma_c->reg_mr_wr.wr.send_flags = IB_SEND_SIGNALED;
#endif
	...
	if (ret) {
        ...
        rdma_c->local_key = rdma_c->mr->rkey;
	return 0;
}
...
static int send_rdma_addr(struct rdma_connection *rdma_c)
{
	...
	msg->remote_key = cpu_to_le32(rdma_c->mr->rkey);
	msg->remote_addr = cpu_to_le64((unsigned long)rdma_c->rdma_buf);
...
static void rdma_rdma_read_done(struct ib_cq *cq, struct ib_wc *wc)
{
        ...
        struct rdma_connection *rdma_d = cm_id->context;
...
static int read_data_by_rdma_read(struct rdma_connection *rdma_c)
{
#if 1
        ...
        int ret;
       	...
       	rdma_c->rdma_read_sgl.addr = rdma_c->recv_dma_addr;
       	//rdma_c->rdma_read_sgl.addr = rdma_c->rdma_dma_addr;
        rdma_c->rdma_read_sgl.lkey = rdma_c->pd->local_dma_lkey;
       	rdma_c->rdma_read_wr.wr.opcode = IB_WR_RDMA_READ;
        rdma_c->rdma_read_wr.wr.send_flags = IB_SEND_SIGNALED;
       	rdma_c->rdma_read_wr.wr.sg_list = &rdma_c->rdma_read_sgl;
       	rdma_c->rdma_read_wr.wr.num_sge = 1;
        rdma_c->rdma_read_wr.wr.wr_cqe = &rdma_c->rdma_read_cqe;
       	rdma_c->rdma_read_cqe.done = rdma_rdma_read_done;
        printk(KERN_ERR "%s()\n", __func__);
        //rdma_c->rdma_read_wr.rkey = (rdma_c->remote_key);
        rdma_c->rdma_read_wr.rkey = cpu_to_le32(rdma_c->remote_key);
        rdma_c->rdma_read_wr.remote_addr = (rdma_c->remote_addr);
        //rdma_c->rdma_read_wr.remote_addr = cpu_to_le64(rdma_c->remote_addr);
        rdma_c->rdma_read_wr.wr.sg_list->length = PAGE_SIZE;
        rdma_c->rdma_read_wr.wr.next = NULL;
        printk("RDMA read data from rkey=%lld, raddr=0x%llx.\n", rdma_c->remote_key, rdma_c->remote_addr);
        ret = ib_post_send(rdma_c->cm_id->qp, &rdma_c->rdma_read_wr.wr, &bad_wr);
        if (ret) {
...
static void rdma_recv_mr_done(struct ib_cq *cq, struct ib_wc *wc)
{
	...
	if (likely(wc->status == IB_WC_SUCCESS)) {
		...
		if (rdma_c->recv_mr_finished == 0) {
			...
			rdma_c->remote_key = le32_to_cpu(msg->remote_key);
			rdma_c->remote_addr = le64_to_cpu(msg->remote_addr);
...
static int rdma_cm_handler(struct rdma_cm_id *cm_id, struct rdma_cm_event *event) {
	...
	switch (event->event) {
		case RDMA_CM_EVENT_CONNECT_REQUEST:
			...
			err = do_rdma_accept(cm_id, event);
			if (err) {
...
static void __init debugfs_init(void)
{
        ...
        debugfs_root2 = dentry;

        send_file2 = debugfs_create_file("server", 0600, debugfs_root2, NULL, &send_file_fops);
}
...
//        char *ip = "10.11.11.251";
        char *ip = "192.168.11.33";
	char _addr[16] = {0};
	int port = 1;

        debugfs_init();
        printk(KERN_INFO"************** rdma server test begin \n");
...
{
        debugfs_remove(send_file2);
        send_file2 = NULL;
        debugfs_remove(debugfs_root2);
        debugfs_root2 = NULL;
}
