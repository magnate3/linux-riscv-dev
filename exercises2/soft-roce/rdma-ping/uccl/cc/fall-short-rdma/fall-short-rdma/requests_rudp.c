...
/*void * srv_poll_cq(void *ev_ctx){
	struct ibv_exp_wc wc[srv_state->max_cwnd_size];
	int i=0;
	struct ibv_cq *cq;
	...
	while(1){
		...
		ibv_ack_cq_events(cq, srv_state->max_cwnd_size);
		TEST_NZ(ibv_req_notify_cq(cq, 0));
		k = ibv_exp_poll_cq(cq, srv_state->max_cwnd_size, wc, sizeof(wc[0]));
		while(k){
...
add_packet_to_cwnd (struct hdr *old_packet_hdr, uint32_t len, struct ibv_send_wr new_wr, rudp_srv_state_t *srv_state)
{
	...
	srv_state->cwnd_free--;
	srv_state->cwnd_end = (srv_state->cwnd_end + 1) % srv_state->max_cwnd_size;
	return 0;
}
/*
 * print_cwnd
...
print_cwnd (rudp_srv_state_t *srv_state)
{
	...
	idx = (srv_state->cwnd_size+start-1)% srv_state->max_cwnd_size;
...
update_cwnd_after_valid_ack (rudp_srv_state_t *srv_state, uint32_t start, int num_rcvd_acks)
{
	...
	src_idx = (src_idx + 1)% srv_state->max_cwnd_size;
	dest_idx = (dest_idx + 1)% srv_state->max_cwnd_size;
	}*/
...
/*	for (i = 0; i < srv_state->max_cwnd_size; i++) {
	...
	for (i = srv_state->cwnd_start; i < srv_state->cwnd_start+srv_state->cwnd_size; i++) {
		src_idx = i%srv_state->max_cwnd_size;	
		//printf("i %d, src_idx %d\n", i, src_idx);
		if (srv_state->cwnd[src_idx].valid == 0) {
			//	srv_state->free_bytes += srv_state->cwnd[src_idx].data_size;		
			srv_state->cwnd_free++;	
		}
	}
...
//		srv_state->cwnd_free++;
		...
		srv_state->cwnd_start =  
			(srv_state->cwnd_start + 1) % srv_state->max_cwnd_size;
		*pending_acks = *pending_acks - 1;
...
//	srv_state->message_size = GET_MIN( srv_state->message_size, SEG_MESSAGE_SIZE*64);
	srv_state->message_size = bytes_remaining;
	while (bytes_remaining ) { //|| pending_acks

		/* Figure out how many packets to send */
		num_slots = find_packet_count(srv_state);
		//printf("srv_state->free_bytes %d\n", srv_state->free_bytes);
...
//			printf("offset %d, seq %d\n",offset, packet_hdr->seq);
		...
		/*	if(1 || (packet_hdr->seq % SAMPLE_RTT_INTERVAL) == 2 || num_slots==1  ) //1 || ||
 				packet_hdr->nic_ts = query_hardware_elapsed_time(srv_state->ib_ctx);
			else{
				packet_hdr->nic_ts = 0x7fff;
				//	printf("beg_snd_nxt %d, current_seq %d\n", vegas->beg_snd_nxt, srv_state->current_seq);
			}*/
			if(packet_hdr->seq % INIT_CWND == 0  ){	
...
/*	if (sigsetjmp(jmpbuf, 1) != 0) {
	...
	struct ibv_exp_wc wc[srv_state->cwnd_size];
	int i=0;
	/*for(i=0;i<srv_state->max_cwnd_size;i++)
	  memset(&wc[i], 0, sizeof(wc[0]));
	 */
	...
	do{
		...
		//	printf("***************************************\n");
			ibv_ack_cq_events(cq, srv_state->cwnd_size);
			TEST_NZ(ibv_req_notify_cq(cq, 0));
			k = ibv_exp_poll_cq(cq, srv_state->cwnd_size, wc, sizeof(wc[0]));
		}else{
			k = ibv_exp_poll_cq(send_cq, srv_state->cwnd_size, wc, sizeof(wc[0]));
		}
		//srv_state->cwnd_free += k;	
	}while(k<=0);

//	fprintf(stderr,"k= %d, cwnd_size %d, current_seq %d, thread index %d\n", k, srv_state->cwnd_size, srv_state->current_seq,srv_state->index);
			...
			if(  acked_count >= srv_state->expected_ack){
				...
				if(recvheader->nic_ts != 0x7fff ){
					if(srv_state->send_transport == RC_TRANSPORT){
						...
						printf(" %d %d, %"PRIu32",%"PRIu32", %"PRIu32"\n", k, acked_count, byte_len,  sample_rtt, sample_rtt-byte_len*8/LINE_RATE);
