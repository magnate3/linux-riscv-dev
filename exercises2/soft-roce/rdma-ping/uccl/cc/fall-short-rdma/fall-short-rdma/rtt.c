...
rtt_init(struct rtt_info *ptr)
{
	...
	ptr->rtt_base = tv.tv_sec*MILLION+tv.tv_usec;		/* #sec since 1/1/1970 at start */
 ...
 * Our timestamps are 32-bit integers that count microseconds since
 * rtt_init() was called.
 */
...
rtt_ts(struct rtt_info *ptr)
{
	...
	gettimeofday(&tv, NULL);
	ts = (tv.tv_sec*MILLION + tv.tv_usec )- ptr->rtt_base;
	return(ts);
}
...
{
	uint32_t start = ptr->rtt_rto;
	//int start=((int) (ptr->rtt_rto + 0.5));
	//printf("timer start: %d\n", start);
	return start;		/* round float to int */
...
rtt_stop(struct rtt_info *ptr, uint32_t ms)
{
	...
	ptr->rtt_rto = rtt_minmax(RTT_RTOCALC(ptr));
	/*printf("ms: %llu, rto: %llu, rttvarl: %llu, srtt: %llu, rtt: %llu\n", 
		(long long unsigned)ms,	(long long unsigned) ptr->rtt_rto,
		 (long long unsigned)ptr->rtt_rttvar, (long long unsigned) ptr->rtt_srtt, (long long unsigned) ptr->rtt_rtt);*/
}
/* end rtt_stop */
