		...
		struct ibv_qp *qp, int seq, struct ibv_cq *send_cq, rudp_srv_state_t *srv_state)
{
	...
	if(wait_for_acks_ret  == -1){	
		...
		for (i = 0; i < num_packets; i++) {
			...
			srv_state->send_bytes += (uint64_t)len;
			/* 
			...
			if( srv_state->send_bytes % (16*1024) == 0 || num_slots == 1 
				||	i == num_packets-1  )//1 || || 				
...
//	do{
	...
	for(ack_i=0; ack_i< k; ack_i++){
		...
		if(wc[ack_i].exp_opcode == IBV_EXP_WC_RDMA_WRITE && srv_state->opcode == READ_OPERATOR ){
			if(srv_state->expected_ack == 1){
				uint32_t base_rtt;
		...
		//	printf("opcode = %s\n", ibv_wc_opcode_str(wc[ack_i].exp_opcode));
			continue;
		}
		...
		srv_state->cp_bytes += (uint64_t)byte_len;
		if(recvheader->nic_ts != 0x7fff || ack_i == k-1 || timeout || retransmit){
			...
			if(  acked_count >= srv_state->expected_ack){
				...
				/*		if(srv_state->cwnd_size < 3 && srv_state->current_seq > 10000000){
						...
						srv_state->rtt_samples[srv_state->cwnd_update_cnt]=sample_rtt;	
					...
					/*	if(srv_state->rate_limiter > srv_state->max_rate_limiter)
							srv_state->max_rate_limiter = srv_state->rate_limiter;*/
					
					/*	if((srv_state->cp_bytes%(1024*1024) == 0 || srv_state->rate_limiter < (srv_state->conf_rate_limiter-1))
								&& fabs(srv_state->rate_limiter - srv_state->conf_rate_limiter)>=2 ){*/
					
						srv_state->cwnd_update_cnt ++;
//#if ENABLE_RL
						if(srv_state->rate_limit_mode == 1){//|| srv_state->rate_limiter < srv_state->conf_rate_limiter
							if((srv_state->cwnd_update_cnt%8 == 0 || srv_state->rate_limiter < srv_state->conf_rate_limiter )){										//printf("index %d, rate %f, prev_rate %f\n", srv_state->index,  srv_state->max_rate_limiter*1.0, srv_state->conf_rate_limiter*1.0);
								...
								if(srv_state->conf_rate_limiter == target_rate || 
									...
									//printf("index %d, rate %f, prev_rate %f\n", srv_state->index,
									//		srv_state->max_rate_limiter, srv_state->conf_rate_limiter);
...
//#endif
			...
			//	printf("cwnd_size %d,free_bytes %d, free_slots %d\n", srv_state->cwnd_size, srv_state->free_bytes, srv_state->free_slots);
				if( retransmit ){
					/*	
