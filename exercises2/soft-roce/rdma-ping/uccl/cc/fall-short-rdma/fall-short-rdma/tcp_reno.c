...
/* Slow start is used when congestion window is no greater than the slow start
 ...
 {
         uint32_t cwnd = GET_MIN(tp->cwnd_size + acked, tp->ss_thresh);
 
         acked -= cwnd - tp->cwnd_size;
         tp->cwnd_size = GET_MIN(cwnd, tp->max_cwnd_size);
	 //printf("@tcp_slow_start index %d, current_seq %d\n", tp->index, tp->current_seq);
         return acked;
 }
...
*/ 
         ...
         tcp_cong_avoid_ai(tp, tp->cwnd_size, acked);
 }
 
 /* Slow start threshold is half the congestion window (min 2) */
 uint32_t tcp_reno_ssthresh(rudp_srv_state_t *tp )
 {
         return GET_MAX(tp->cwnd_size >> 1U, 2U);
 }
