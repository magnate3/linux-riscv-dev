...
static void * cong_response_tput(void * arg){
	...
	struct context *s_ctx=multi_ctx[index];
	struct connection *conn = s_ctx->conn;
	uint64_t sleeptime = 60000; //10 mili
	uint64_t cp_bytes;	
	uint64_t prev_cp_bytes = conn->num_completions*conn->send_message_size;
	struct timespec start, end;
	clock_gettime(CLOCK_MONOTONIC, &start);
	uint64_t starttime = start.tv_sec*BILLION + start.tv_nsec;
	while(1){
		cp_bytes = conn->num_completions*conn->send_message_size;
...
//	signal(SIGALRM, sigalarm_handler);
	...
	//	for(i=num_threads; i< num_threads*2; i++){	
			if(pthread_create(&latency_thread[i%num_threads], NULL, latency_measure, (void *) i) != 0)
				die("main(): Failed to create latency_measure thread .");	
	//	}
		pthread_t cong_res;
			if(pthread_create(&cong_res, NULL, cong_response_tput, (void *) 0) != 0)
