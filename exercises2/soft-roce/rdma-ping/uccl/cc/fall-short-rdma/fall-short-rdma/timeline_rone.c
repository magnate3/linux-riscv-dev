...
uint64_t lats_time[MAX_NUM_IB_THREADS/2][MAX_TIMESTAMP_SIZE];

double tputs[MAX_NUM_IB_THREADS/2][MAX_TIMESTAMP_SIZE];
uint64_t tputs_time[MAX_NUM_IB_THREADS/2][MAX_TIMESTAMP_SIZE];
...
void parseOpt(int argc, char **argv){
	int c; 
	int i=0;	
	while ((c = getopt(argc, argv, "s:m:n:c:b:h:p:e:M:S:R:")) != -1) {
		switch (c)
		{
			case 'm':
...
static void * latency_measure(void * arg){
	...
	while(1){
		...
		for(ack_i=0; ack_i< k; ack_i++){
			...
			lats_time[index%num_threads][i]=starttime/1000;
			i++;
		}
		usleep(10*1000);
		//usleep(50000);
...
static void * cong_response_tput(void * arg){
	int index = (int) arg;
	bindingCPU(index);
	struct context *s_ctx=multi_ctx[index%num_threads];
	struct connection *conn = s_ctx->conn;
	uint64_t sleeptime = 10000; //100 mili
	uint64_t cp_bytes;
	uint64_t prev_cp_bytes = s_ctx->srv_state->cp_bytes;
	struct timespec start, end;
	clock_gettime(CLOCK_MONOTONIC, &start);
	uint64_t starttime = (uint64_t)(start.tv_sec*BILLION + start.tv_nsec);
	int i=0;
	while(1){
		cp_bytes = s_ctx->srv_state->cp_bytes;
		double tput =(double)(cp_bytes -prev_cp_bytes)*8.0/(sleeptime*1000);
		if(cp_bytes > 0){
			tputs[index%num_threads][i]=tput;
			tputs_time[index%num_threads][i] = starttime/1000;
			//printf("- [%d, %"PRIu64", %.9g]\n", index%num_threads, tputs_time[index%num_threads][i], tputs[index%num_threads][i]);
			i++;
			//printf("- [%"PRIu64",%.9g]\n", tputs_time[i], tputs[i]);*/
			//printf("- [%"PRIu64",%.9g]\n", starttime/1000, tput);
			//fflush(stdout);
		}
			
		prev_cp_bytes = cp_bytes;
		usleep(sleeptime);
		starttime +=sleeptime*1000;
		if(i>MAX_TIMESTAMP_SIZE)
			break;
...
static void * incast(void * arg){
	...
	clock_gettime(CLOCK_MONOTONIC, &start);
	/*if(index >0)
		conn->num_requests = conn->num_requests/((int)pow(2,index));
*/
	...
	//sleep(30);
	//usleep(index*1000*50*1000);
			//for (i=0;i<conn->num_requests;i++){
	//printf("start sending %d, request %d\n", index, conn->num_requests);
	clock_gettime(CLOCK_MONOTONIC, &start);
	uint64_t t = start.tv_sec*BILLION + start.tv_nsec;
	printf("- [%"PRIu64", %d]\n", t/1000,index);
	...
	while(s_ctx->srv_state->cp_bytes < tot_bytes){
...
end_sending:
	...
	//printf("- [%.9g, %lu, %.9g, %d]\n", tot_time, data, tput,index);

	t = end.tv_sec*BILLION + end.tv_nsec;
	printf("- [%"PRIu64", %d]\n", t/1000,index);
...
int main(int argc, char **argv)
{
	...
	memset(tputs,0, sizeof(tputs));
	memset(tputs_time,0, sizeof(tputs_time));
	memset(lats_time,0, sizeof(lats_time));
	int index;
...
//	signal(SIGALRM, sigalarm_handler);
	...
	if(is_client==1){
		...
		pthread_t cong_res[num_threads];
		for(i=num_threads; i< num_threads*2;i++){
			if(pthread_create(&cong_res[i%num_threads], NULL, cong_response_tput, (void *) i) != 0)
				die("main(): Failed to create cong_response_tput thread .");	
		}
...
/*		clock_gettime(CLOCK_MONOTONIC, &incast_end);
		...
		if(is_sync == 1)
			diff = diff -BILLION;
		s_ctx = multi_ctx[0];
		long unsigned data = (long unsigned)s_ctx->conn->num_sendcount*s_ctx->conn->send_message_size*num_threads;
