...
static double TIMELY_BETA = 0.7;
static double TIMELY_ALPHA = 0.125; 
...
//	printf( "index %d, miniRTT %"PRIu32"\n", tp->index, timely->minRTT);
         ...
         if (after(ack, timely->beg_snd_nxt)) {
				...
				//FIX ME TODO
                 ...
                 if (timely->cntRTT <=0) {
                          ...
                          */
                        // tcp_reno_cong_avoid(tp, ack, acked);
			if (tcp_in_slow_start(tp) && timely->slow_start_done ==0) {
                                 /* Slow start.  */
			...
			//printf("in tcp_timely\n");
			 ...
			 double normalized_gradient;		
                          ...
                          */
                         rtt = timely->minRTT;
			 new_rtt_diff = (long )rtt-timely->prev_minRTT;
			if(new_rtt_diff < 0)
				timely->add_count ++;
			else
				timely->add_count = 0;
			 // printf("prev %"PRIu32"\n", timely->prev_minRTT); 
               		 timely->prev_minRTT = rtt;
			 ...
			 timely->rtt_diff =(long) ((double)(1-TIMELY_ALPHA) * timely->rtt_diff + (double)TIMELY_ALPHA* new_rtt_diff);
			 //printf("rtt_diff %ld, baseRTT %"PRIu32"\n", timely->rtt_diff, timely->baseRTT);
			 normalized_gradient = (double)timely->rtt_diff/timely->baseRTT; 
                         //printf("normalized_gradient %f\n", normalized_gradient); 
			/*target_cwnd = (uint64_t)tp->cwnd_size * timely->baseRTT;
			...
			/* Calculate the difference between the window we had,
                        ...
                        // diff = tp->cwnd_bytes/PACKET_SIZE * (rtt-timely->baseRTT) / timely->baseRTT;
                         if (rtt> thigh && tcp_in_slow_start(tp) && timely->slow_start_done == 0) {
                                  ...
                                  * utilization.
                                  */
                                 //tp->cwnd_bytes = GET_MIN(tp->cwnd_bytes, (uint32_t)target_cwnd+1);
				 
                                 tp->ss_thresh = tcp_timely_ssthresh(tp);
 				
                         } else if (tcp_in_slow_start(tp) && timely->slow_start_done ==0) {
                                 /* Slow start.  */
                                 tcp_slow_start(tp, acked);
				 //fprintf(stdout, "in tcp_slow_start %"PRIu64"\n", tp->cwnd_bytes);
                         } else {
				 ...
				 timely->slow_start_done = 1;
			         if (rtt > thigh) {
                                         /* The old window was too fast, so
                                          * we slow down.
                                          */
					long a = (long) ((double)tp->cwnd_bytes*(1-(double)TIMELY_BETA*(1-(double)thigh/rtt)));
                                         if(a< PACKET_SIZE)	a = PACKET_SIZE;
					 tp->cwnd_bytes =((uint64_t)a/PACKET_SIZE)*PACKET_SIZE ;
                                         tp->ss_thresh  = tcp_timely_ssthresh(tp);
					
					//fprintf(stdout,"timely > beta%"PRIu64",a %ld\n", tp->cwnd_bytes,a);
                                 } else if (rtt < tlow) {
					 ...
					 if(normalized_gradient <= 0){
						 if (timely->add_count >= 5)
							 tp->cwnd_bytes += 5*PACKET_SIZE;
						 else
							 tp->cwnd_bytes += PACKET_SIZE;	
						 //fprintf(stdout, "in normalized_gradient <=0 %"PRIu64"\n", tp->cwnd_bytes);
					 }else{
						 //fprintf(stderr,"%"PRIu64", normalized_gradient %f\n", tp->cwnd_bytes, normalized_gradient); 
						 long  b = (long)((double)tp->cwnd_bytes*(1-(double)TIMELY_BETA*normalized_gradient));
						 if(b < PACKET_SIZE)
							 b = PACKET_SIZE;
						 tp->cwnd_bytes = (uint64_t)(b/PACKET_SIZE )*PACKET_SIZE;	
						 //fprintf(stdout,"timely >0  %"PRIu64",b %ld\n", tp->cwnd_bytes,b);
					 }
				}
        		}
		         if (tp->cwnd_bytes < PACKET_SIZE)
                                 tp->cwnd_bytes = PACKET_SIZE;
                         tp->ss_thresh = tcp_current_ssthresh(tp);
			 //fprintf(stderr, "after ss_thresh %"PRIu64"\n", tp->cwnd_bytes);
		 	//printf( "index %d, miniRTT %"PRIu32", normalized_gradient %f\n", tp->index, timely->minRTT, normalized_gradient);
		 }
	...
	double new_rate = (double)(tp->cwnd_bytes*8.0/timely->baseRTT)/1000;
	if(new_rate > 10.0)
		new_rate = 10.0;
	tp->rate_limiter = new_rate;
//	fprintf(stdout, "index %d, cwnd %"PRIu64", rate %f, baseRTT %"PRIu32"\n", 
//		tp->index, tp->cwnd_bytes, tp->rate_limiter, timely->baseRTT);
