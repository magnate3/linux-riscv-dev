...
#define RUDP_DEFAULT_SSTHRESH           65536

#define MINIMUM_BASIC_RTT  (6)
...
typedef struct rudp_srv_state_s {
	...
	uint64_t 	prev_rtt;
	int sl;
	uint64_t 	t_enqueue[16];
	uint32_t 		current_seq;
//	uint32_t        cwnd_size;
	uint32_t 		cwnd_size_cnt; /* Linear increase counter              */
	uint64_t        cwnd_bytes;
	double 		conf_rate_limiter;
	double 		max_rate_limiter;
	double          rate_limiter;
//	uint32_t        max_cwnd_size;
	uint32_t        advw_size;
//	uint64_t        cwnd_free;
	uint64_t        rtt_samples[10000];
	uint64_t        free_bytes;
	uint64_t 	free_slots;
	uint32_t        ss_thresh;
