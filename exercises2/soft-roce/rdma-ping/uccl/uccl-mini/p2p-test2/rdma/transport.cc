...
int RDMAEndpoint::submit_fifo_metadata(UcclFlow* flow,
  ...
  engine_idx = ureq->engine_idx;
  FifoItem* slots =
      flow->post_fifo(engine_idx, data_arr, size_arr, 1, mhandles, &ureq->recv.wr, &ureq->recv.sge);
  ...
  ureq->context = flow;
  ureq->n = n;
  ureq->recv.data_len[0] = size;
  ureq->poll_ctx = ctx_pool_->pop();
  ureq->recv.elems = slots;
  ...
  auto rxq = channel_vec_[engine_idx]->rx_cmdq_;
  while (jring_mp_enqueue_bulk(rxq, &msg, 1, nullptr) != 1) {
  }

  UCCL_LOG_EP << "submit_fifo_metadata: posted " << n << " requests"
              << " on engine " << engine_idx << " size: " << size;
...
int RDMAEndpoint::uccl_read_one(UcclFlow* flow, Mhandle* local_mh, void* dst,
                                  size_t size, ucclRequest* ureq) {
  int slot, nmsg;
  ureq->type = ReqRead;
  ureq->send.data_len = size;
  printf("check fifo ready for read one \n");
  if (!flow->check_fifo_ready(&slot, &nmsg)) return -1;
  UCCL_LOG_EP << "fifo ready for read one " << size;
  printf("fifo ready for read one \n");
  DCHECK(slot < kMaxReq && nmsg <= kMaxRecv) << slot << ", nmsg" << nmsg;
  auto send_comm = &flow->send_comm_;
  auto ureqs = send_comm->fifo_ureqs[slot];
  auto rem_fifo = send_comm->base.fifo;
  struct FifoItem* slots = rem_fifo->elems[slot];
  FifoItem const& slot_item = slots[0];
  int rc;
  do {
    rc = uccl_read_async(flow, local_mh, dst, size, slot_item, ureq);
    if (rc == -1) {
      std::this_thread::yield();
    }
  } while (rc == -1);
