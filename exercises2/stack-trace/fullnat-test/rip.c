...
static int rip_needed_tcp(struct sk_buff *skb, struct rip_order *order)
{
	...
	printk(KERN_INFO "recv tcp packet and not writeable \n");
        return -1;
    }
#endif
#endif
    iph = rip_ip_hdr(skb);
    tcp = rip_tcp_hdr(skb);
    //if(7777 == ntohs(tcp->dest)){
    if( tcp->syn && (7777== ntohs(tcp->dest) || 22 == ntohs(tcp->dest))){
        printk(KERN_INFO "[tcp tuple] IP4 %pI4.%d > %pI4.%d: TCP\n", &iph->saddr, ntohs(tcp->source), &iph->daddr, ntohs(tcp->dest));
    }
...
NF_HOOK_FN(rip_inet_hook_input)
{
	...
	//printk(KERN_INFO "tcp packet recv \n");
        ret = rip_hook_input_tcp(skb);
    } else if (iph->protocol == IPPROTO_UDP) {
        ret = rip_hook_input_udp(skb);
    } else if (iph->protocol == IPPROTO_ICMP) {
        printk(KERN_INFO "[icmp tuple] IP4 %pI4 > %pI4 \n", &iph->saddr,  &iph->daddr);
    }
    else { 
	//printk(KERN_INFO "recv  packet proto %u and not tcp or udp ,",iph->protocol);
        //printk(KERN_INFO "[tcp tuple] IP4 %pI4 > %pI4 \n", &iph->saddr,  &iph->daddr);
    }

    return ret;
}
NF_HOOK_FN(rip_inet_hook_preroute)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 24)
    struct sk_buff *skb = *pskb;
#endif
