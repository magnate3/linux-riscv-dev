...
static void process_pkt(void *pkt, uint32_t len)
{
    	struct ethhdr *eth = (struct ethhdr *) pkt;
       	struct iphdr *ip = (struct iphdr *) (eth + 1);	
        uint32_t addr = 0;
	if (ntohs(eth->h_proto) == ETH_P_IP && len > (sizeof(*eth) + sizeof(*ip))) {
		...
		if(icmp->type != ICMP_ECHO)
		    return;
                ...
                //printf("[icmp tuple] IP4 %pI4 > %pI4 \n", &ip->saddr,  &ip->daddr);
		addr = ip->saddr;
		printf("src %u.%u.%u.%u \t ", addr & 0xFF, (addr>>8) & 0xFF, (addr>>16) & 0xFF, (addr>>24) & 0xFF);
		addr  = ip->daddr;
		printf("dst %u.%u.%u.%u", addr & 0xFF, (addr>>8) & 0xFF, (addr>>16) & 0xFF, (addr>>24) & 0xFF);
		printf("\n");
...
static void l3_fwd(struct xdpsock *xsk)
{
	while(g_running){
		struct xdp_desc descs[BATCH_SIZE];
		unsigned int rcvd, i;
