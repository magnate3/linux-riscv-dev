...
def parse_ss_output(dport, output_file=None):
    # 获取命令输出
    cmd = ["ss", "-tinm", "state", "established", f"dport = :{dport}"]
...
def main():
    ...
    parser.add_argument('dport', type=str, help='dst port to monitor')
    parser.add_argument('-i', '--interval', type=float, default=1.0,
                        help='Collection interval in seconds (default: 1.0)')
    parser.add_argument('-o', '--output', type=str, help='Output file path')
    args = parser.parse_args()

    print(f"Monitoring TCP connections on port {args.dport}")
    ...
    try:
        while running:
            parse_ss_output(args.dport, args.output)
