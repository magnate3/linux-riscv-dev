...
for i, f in enumerate(sorted(args.files)):
    ...
    ax.plot(xaxis, deliver_gain, label="cwnd_gain", lw=2, **get_style(i+1))
    ...
    ani = animation.FuncAnimation(fig, update, frames=len(xaxis), interval=100, 
                                  fargs=[xaxis, pace_gain, line1], blit=True)
...
if args.out:
    ...
    ani.save(filename=args.out, writer="pillow",fps=60)
    #ani.save(args.out, writer='imagemagick', fps=60)
else:
    plt.show()
