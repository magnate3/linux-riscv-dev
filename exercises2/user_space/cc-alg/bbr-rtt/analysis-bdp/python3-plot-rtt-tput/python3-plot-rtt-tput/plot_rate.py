...
for i, f in enumerate(sorted(args.files)):
    ...
    pace_rate = list(map(float, col(1, data)))
    pace_rate = [t for j, t in enumerate(pace_rate)
                  if xaxis[j] <= args.xlimit]
    deliver_rate = list(map(float, col(2, data)))
    deliver_rate = [t for j, t in enumerate(deliver_rate)
                  if xaxis[j] <= args.xlimit]
    xaxis = [x for x in xaxis if x <= args.xlimit]

    ax.plot(xaxis, pace_rate, label="pace_rate", lw=2, **get_style(i))
    ax.plot(xaxis, deliver_rate, label="deliver_rate", lw=2, **get_style(i+1))
