...
def main():
    ...
    #classvector = bytes(classvectorbyteUpper,'UTF-8') + classvectorbyteLower
    classvector = classvectorbyteUpper + str(classvectorbyteLower)
    ...
    #classtimebyteUpper = bytes(classtimebyteUpper,'UTF-8')
    ...
    if options.quanta0:
        if options.quanta0 <= 65535:
            ...
            if options.quanta0 > 255:
                ...
                #pfc0=bytes(classtimebyteUpper,'UTF-8')+binascii.unhexlify(format(options.quanta0, '#04x').replace('0x',''))
                #pfc0=classtimebyteUpper+binascii.unhexlify(format(options.quanta0, '#04x').replace('0x',''))
                pfc0=classtimebyteUpper+str(binascii.unhexlify(format(options.quanta0, '#04x').replace('0x','')))
        else:
    ...
    while options.iteration > 0:
        s.send(bytes(fullrawpacket,'UTF-8'))
        options.iteration -= 1
