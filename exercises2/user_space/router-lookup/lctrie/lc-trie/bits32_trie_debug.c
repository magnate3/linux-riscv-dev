...
static void b32t_print_leaf(struct b32t_key_vector *l, int cindex){
    ...
    if(hlist_empty(&l->leaf)){
        printf("%sleaf[0x%x]:pos[0x%x]bits[0x%x]cindex[0x%x]", offset_char, l->key, l->pos, l->bits, cindex);
        return;
    }
    hlist_for_each_entry(fa, &l->leaf, fa_list) {
        printf("%sleaf[0x%x]:pos[0x%x]bits[0x%x]data[0x%x]cindex[0x%x]", 
...
static void b32t_print_each_tnode(struct b32t_key_vector *n, int cindex){
    ...
    printf("%stnode[0x%x]:pos[0x%x]bits[0x%x]cindex[0x%x]", offset_char, n->key, n->pos, n->bits, cindex);
}
