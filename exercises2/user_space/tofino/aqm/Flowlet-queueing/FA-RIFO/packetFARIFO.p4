...
const bit<32>  CounterLimit = 96;
const bit<32> round_add = 1;
const bit<32> mulT = 96;
   ...
   inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    //ingress counter register
    Register<bit<32>,bit<5>>(32,0) Ingress_Counter_Reg;
    RegisterAction<bit<32>,bit<5>,bit<32>> (Ingress_Counter_Reg) set_ig_counter_reg = {
        void apply(inout bit<32> value){
            value = hdr.worker.counter;
        }
    };

    RegisterAction<bit<32>,bit<5>,bit<32>> (Ingress_Round_Reg) get_ig_counter_reg = {
        void apply(inout bit<32> value,out bit<32> result){
            result = value;
        }
    };
   ...
   apply {
        if (hdr.ipv4.isValid()) {
            ...
            if(hdr.worker.isValid()){
                ...
                //get counter
                meta.counter = get_ig_counter_reg.execute(0);

                //get rank
                update_and_get_f_finish_time.apply();

                //set min max
                if(meta.counter == CounterLimit){
   ...
   inout egress_intrinsic_metadata_for_output_port_t  eg_oport_md)
{
    ...
    RegisterAction<bit<32>,bit<5>,bit<32>> (Egress_Round_Reg) get_eg_round_reg = {
     ...
     //Engress counter register
    Register<bit<32>,bit<5>> (32,0) Egress_Counter_Reg;
    RegisterAction<bit<32>,bit<5>,bit<32>> (Egress_Counter_Reg) set_eg_counter_reg = {
        void apply(inout bit<32> value,out bit<32> result){
    ...
    RegisterAction<bit<32>,bit<5>,bit<32>> (Egress_Counter_Reg) get_eg_counter_reg = {
        void apply(inout bit<32> value,out bit<32> result){
            result = value;
        }
    };
   ...
   apply {
	   if(hdr.worker.isValid()){
            ...
            hdr.worker.counter = get_eg_counter_reg.execute(0);
  	    }
