...
uint64_t pkts_send_limit_ms = 1000;
...
int main(int argc, char **argv) {
    ...
    uint32_t lcore_id;
    uint32_t pktsize;
    pktsize = sizeof(MessageHeader) + sizeof(struct rte_udp_hdr) + sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_ether_hdr);
    ...
    rte_eal_mp_remote_launch(client_loop, NULL, CALL_MAIN);
