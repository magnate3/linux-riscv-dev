...
#define NC_MBUF_CACHE_SIZE      64 // 32
#define NC_MAX_BURST_SIZE       64 // 32
...
#define NODE_NUM                128
#define IP_SRC                  "10.1.0.2"
#define IP_DST                  "192.168.40.20"
...
static void send_pkt_burst(uint32_t lcore_id) {
    ...
    uint32_t ret = rte_eth_tx_burst(
        ...
        m_table,
        lconf->tx_mbufs.len);
    tput_stat[lconf->vid].tx += ret * 8 * (sizeof(MessageHeader) + sizeof(struct rte_udp_hdr) + sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_ether_hdr));
    if (unlikely(ret < n)) {
...
static void init_header_template(void) {
    ...
    struct rte_ether_addr src_addr = {
        .addr_bytes = {0xb8, 0x3f, 0xd2, 0x9f, 0x2e, 0x9b}};
    struct rte_ether_addr dst_addr = {
        .addr_bytes = {0xb8, 0x3f, 0xd2, 0xb0, 0xd7, 0x79}};
    /*struct rte_ether_addr src_addr = {
        .addr_bytes = {0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff}};
    struct rte_ether_addr dst_addr = {
        .addr_bytes = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55}};
    *///rte_ether_addr_copy(&port_eth_addrs[0], &eth->src_addr);
...
static void print_packet(struct rte_mbuf *mbuf) {
    ...
    char mac_buf[RTE_ETHER_ADDR_FMT_SIZE];
    rte_ether_format_addr(mac_buf, RTE_ETHER_ADDR_FMT_SIZE, &eth->src_addr);
    printf("\teth: src_mac:%s", mac_buf);
    rte_ether_format_addr(mac_buf, RTE_ETHER_ADDR_FMT_SIZE, &eth->dst_addr);
    printf(" dst_mac:%s\n", mac_buf);
