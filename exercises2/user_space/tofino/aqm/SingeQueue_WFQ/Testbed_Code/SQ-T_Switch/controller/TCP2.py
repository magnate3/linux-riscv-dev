...
flowindex = 1
tcp_flowindex = 11
dstport_current = 9000      #udp from 9000
dstport_tcp_current = 5001                            #tcp from 5001
...
tcp_addr=["10.0.0.2","10.0.0.2","10.0.0.2","10.0.0.3","10.0.0.3","10.0.0.3"]
tcp_weights = [3,3,3,3,3,3]
udp_weights = [3,3]
# udp_addr=["10.0.0.2","10.0.0.3","10.0.0.2","10.0.0.3"]
udp_addr=["10.0.0.4","10.0.0.4"]
dst_addr="10.0.0.1"
...
for i in range(0,tcpflownum):
    tcp_flows[tcp_flowindex] = flow(tcp_flowindex,tcp_addr[i%tcpflownum],srcport_current,dst_addr,dstport_tcp_current,"TCP",tcp_weights[i])
    tcp_flowindex+=1
    dstport_tcp_current+=1
    srcport_current+=1
...
try:
    ...
    match_table_forward.entry_add(
        target,
        [match_table_forward.make_key([client.KeyTuple("hdr.ipv4.dst_addr",dst_addr)])],
        [match_table_forward.make_data([client.DataTuple("port",output_port)],action_name = "forward")]
    )
    match_table_forward.entry_add(
        target,
    ...
    match_table_forward.entry_add(
        target,
        [match_table_forward.make_key([client.KeyTuple("hdr.ipv4.dst_addr",tcp_addr[3])])],
        [match_table_forward.make_data([client.DataTuple("port",tcp_port2)],action_name = "forward")]
    )
...
try:
    keyname = "meta.weight"
    for i in range(0,Shift_limit):
        # limit = Q/R>>i
        limit = PACKET_LIMIT>>i
        match_table_limit.entry_add(
            target,
...
ends = []
values = []
for i in range(0,PACKET_LIMIT):
    starts.append(i*1500)
    ends.append(starts[i]+1499) 
    values.append(int(Decimal((1500/R)*(PACKET_LIMIT*1.0/(i+1))).quantize(Decimal("1."), rounding = "ROUND_HALF_UP"))) 
...
#round_index
# match_table_egress_round_index = bfrt_info.table_get("SwitchEgress.get_round_index_tbl")
# try:
#     keyname1 = "eg_intr_md.egress_port"
#     keyname2 = "eg_intr_md.egress_qid"
#     match_table_egress_round_index.entry_add(
#         target,
#         [match_table_egress_round_index.make_key([client.KeyTuple(keyname1,output_port),client.KeyTuple(keyname2,0)])],
#         [match_table_egress_round_index.make_data([client.DataTuple("round_index",0)],action_name = "SwitchEgress.get_round_index_action")]
#     )
# finally:
#     pass

# match_table_worker_egress_round_index = bfrt_info.table_get("SwitchEgress.worker_get_round_index_tbl")
# try:
#     keyname1 = "hdr.worker_t.egress_port"
#     keyname2 = "hdr.worker_t.qid"
#     match_table_worker_egress_round_index.entry_add(
#         target,
#         [match_table_worker_egress_round_index.make_key([client.KeyTuple(keyname1,output_port),client.KeyTuple(keyname2,0)])],
#         [match_table_worker_egress_round_index.make_data([client.DataTuple("worker_round_index",0)],action_name = "SwitchEgress.worker_get_round_index_action")]
#     )
# finally:
#     pass
...
#SQS table:
wf_origin_shift = 3      #for now,TCP flow weight origin
wf_origin = 8
ro = 0.5
time_unit = 12000
shift = 0
match_table_newlimit = bfrt_info.table_get("SwitchIngress.tbl_get_new_wf")
#细粒度
#0~~time_unit-1:1
#2*time_unit~~3*time_unit-1:1/2
#3*time_unit~~4*time_unit-1:1/3


#       1/64
try:
    keyname_high = "meta.timediff_high"
    keyname_low = "meta.timediff_low"
    keyname_weight = "meta.weight"
    for i in range(0,16):
        if i+1 > wf_origin:
            break 
        low = i*time_unit
        high = low+time_unit-1
        match_table_newlimit.entry_add(
            target,
            # [match_table_newlimit.make_key([client.KeyTuple(keyname_high,0),client.KeyTuple(keyname_low,low = low, high = high),client.KeyTuple(keyname_weight,wf_origin_shift)])],
            [match_table_newlimit.make_key([client.KeyTuple(keyname_low,low = low, high = high),client.KeyTuple(keyname_weight,wf_origin_shift)])],
            [match_table_newlimit.make_data([client.DataTuple("new_limit", int(PACKET_LIMIT/(i+1)) )],action_name = "SwitchIngress.get_limit_new")]
        )
finally:
    pass
...
#     match_table_newlimit.entry_add(
#             target,
#             [match_table_newlimit.make_key([client.KeyTuple(keyname_high,0),client.KeyTuple(keyname_low,low = 0, high = time_unit-1),client.KeyTuple(keyname_weight,wf_origin_shift)])],
#             [match_table_newlimit.make_data([client.DataTuple("newlimit",Q/R)])]
#         )
#     #others
#     for i in range(0,wf_origin_shift):
#         low = time_unit*(1<<i)
#         high = low * 2 -1
#         match_table_newlimit.entry_add(
#             target,
#             [match_table_newlimit.make_key([client.KeyTuple(keyname_high,0),client.KeyTuple(keyname_low,low = low, high = high),client.KeyTuple(keyname_weight,wf_origin_shift)])],
#             [match_table_newlimit.make_data([client.DataTuple("newlimit",Q/(R*(i+2)))])]
#         )
# finally:
#     pass