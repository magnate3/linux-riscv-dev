...
class flow:
    def __init__(self,index,srcIP,srcPort,dstIP,dstPort,type,weight):
        self.index = index
        self.srcIP = srcIP
...
tcp_addr=["10.0.0.2","10.0.0.3","10.0.0.4","10.0.0.4"]
tcp_weights = [1,2,3,3]
udp_weights = [2,3,2,4]
# udp_addr=["10.0.0.2","10.0.0.3","10.0.0.2","10.0.0.3"]
udp_addr=["10.0.0.2","10.0.0.3","10.0.0.4"]
dst_addr="10.0.0.1"
...
try:
    ...
    match_table_forward.entry_add(
        target,
        [match_table_forward.make_key([client.KeyTuple("hdr.ipv4.dst_addr",tcp_addr[1])])],
        [match_table_forward.make_data([client.DataTuple("port",148)],action_name = "forward")]
    )
    match_table_forward.entry_add(
        target,
        [match_table_forward.make_key([client.KeyTuple("hdr.ipv4.dst_addr",tcp_addr[2])])],
...
for i in range(0,PACKET_LIMIT):
    starts.append(i*500)
    ends.append(starts[i]+499) 
    values.append(int(Decimal((500/R)*(PACKET_LIMIT*1.0/(i+1))).quantize(Decimal("1."), rounding = "ROUND_HALF_UP"))) 
