...
/*
 * Implementation of Horus Spine scheduler
 *
 ...
 * 
 * The main procedures for scheduling is similar to Horus Leaf (avoided duplicate explanation comments),
...
control SpineIngress(
    ...
    Register<leaf_id_t, _>(MAX_LEAFS) idle_list; // Maintains the list of idle leafs for each vcluster (array divided based on cluster_id)
        ...
        RegisterAction<leaf_id_t, _, leaf_id_t>(idle_list) write_idle_list = {
            void apply(inout bit<16> value, out bit<16> rv) {
                value = horus_md.task_resub_hdr.ds_index_2;
                rv = value;
            }
        };
    ...
    Register<queue_len_t, _>(MAX_TOTAL_LEAFS) queue_len_list_1; // List of queue lens for all vclusters
        RegisterAction<queue_len_t, _, queue_len_t>(queue_len_list_1) update_queue_len_list_1 = {
            void apply(inout queue_len_t value, out queue_len_t rv) {
                value = value + horus_md.selected_ds_qlen_unit;
                rv = value;
            }
        };
    ...
    Register<queue_len_t, _>(MAX_TOTAL_LEAFS) deferred_queue_len_list_2; // List of queue lens for all vclusters
        ...
        RegisterAction<queue_len_t, _, queue_len_t>(deferred_queue_len_list_2) read_deferred_queue_len_list_2 = {
            void apply(inout queue_len_t value, out queue_len_t rv) {
                rv = value + horus_md.not_selected_ds_qlen; // to avoid using another stage for this calculation
            }
        };
    ...
    Register<bit<16>, _>(MAX_TOTAL_LEAFS) idle_list_idx_mapping; // Maintains the position of leaf in the idle list so we can later remove it in O(1)
        RegisterAction<bit<16>, _, bit<16>>(idle_list_idx_mapping) write_idle_list_idx_mapping = {
            void apply(inout bit<16> value, out bit<16> rv) {
                value = horus_md.idle_ds_index;
                rv = value;
            }
        };
        RegisterAction<bit<16>, _, bit<16>>(idle_list_idx_mapping) update_idle_list_idx_mapping = {
            void apply(inout bit<16> value, out bit<16> rv) {
                value = horus_md.task_resub_hdr.ds_index_1;
                rv = value;
            }
        };
        RegisterAction<bit<16>, _, bit<16>>(idle_list_idx_mapping) read_idle_list_idx_mapping = { // Read last value update current to horus_md.cluster_idle_count -1 (we swap with top element when removing)
            void apply(inout bit<16> value, out bit<16> rv) {
                rv = value;
            }
        };
    ...
    Register<bit<16>, _>(MAX_VCLUSTERS) rr_counter; // 
        RegisterAction<bit<16>, _, bit<16>>(rr_counter) inc_rr_counter  = {
            void apply(inout bit<16> value, out bit<16> rv) {
                rv = value;
                if (value >= (bit<16>) horus_md.cluster_num_valid_queue_signals - 1) {
                    value = 0;
                } else {
                    value = value + 1;
                }
    ...
    action get_array_indices () {
        ...
        horus_md.cluster_absolute_leaf_index = horus_md.cluster_ds_start_idx + hdr.horus.src_id;
    }
    ...
    action select_idle_index() {
        // horus_md.idle_ds_index = horus_md.cluster_ds_start_idx + horus_md.idle_rr_index;
        horus_md.idle_ds_index = horus_md.idle_ds_index - 1;
    }
    //////// 
    action gen_random_leaf_index_16() {
        horus_md.t1_random_ds_index_1 = (bit<16>) random_ds_id.get();
        horus_md.t1_random_ds_index_2 = (bit<16>) random_ds_id.get();
    }

    action retry_gen_random_leaf_index_16(){
        horus_md.t2_random_ds_index_1 = (bit<16>) random_ds_id_retry.get();
        horus_md.t2_random_ds_index_2 = (bit<16>) random_ds_id_retry.get();
    }

    action adjust_random_leaf_index_5() {
        horus_md.t1_random_ds_index_1 = horus_md.t1_random_ds_index_1 >> 11;
        horus_md.t1_random_ds_index_2 = horus_md.t1_random_ds_index_2 >> 11;
        horus_md.t2_random_ds_index_1 = horus_md.t2_random_ds_index_1 >> 11;
        horus_md.t2_random_ds_index_2 = horus_md.t2_random_ds_index_2 >> 11;
    }
    action adjust_random_leaf_index_4() {
        horus_md.t1_random_ds_index_1 = horus_md.t1_random_ds_index_1 >> 12;
        horus_md.t1_random_ds_index_2 = horus_md.t1_random_ds_index_2 >> 12;
        horus_md.t2_random_ds_index_1 = horus_md.t2_random_ds_index_1 >> 12;
        horus_md.t2_random_ds_index_2 = horus_md.t2_random_ds_index_2 >> 12;
    }

    action adjust_random_leaf_index_3() {
        horus_md.t1_random_ds_index_1 = horus_md.t1_random_ds_index_1 >> 13;
        horus_md.t1_random_ds_index_2 = horus_md.t1_random_ds_index_2 >> 13;
        horus_md.t2_random_ds_index_1 = horus_md.t2_random_ds_index_1 >> 13;
        horus_md.t2_random_ds_index_2 = horus_md.t2_random_ds_index_2 >> 13;
    }

    action adjust_random_leaf_index_2() {
        horus_md.t1_random_ds_index_1 = horus_md.t1_random_ds_index_1 >> 14;
        horus_md.t1_random_ds_index_2 = horus_md.t1_random_ds_index_2 >> 14;
        horus_md.t2_random_ds_index_1 = horus_md.t2_random_ds_index_1 >> 14;
        horus_md.t2_random_ds_index_2 = horus_md.t2_random_ds_index_2 >> 14;
    }

    action adjust_random_leaf_index_1() {
        horus_md.t1_random_ds_index_1 = horus_md.t1_random_ds_index_1 >> 15;
        horus_md.t1_random_ds_index_2 = horus_md.t1_random_ds_index_2 >> 15;
        horus_md.t2_random_ds_index_1 = horus_md.t2_random_ds_index_1 >> 15;
        horus_md.t2_random_ds_index_2 = horus_md.t2_random_ds_index_2 >> 15;
    }
    ...
    action inc_repeated_rand() {
        horus_md.random_ds_index_2 = horus_md.random_ds_index_2+1;
    }
    ...
    action compare_random_idx() {
        horus_md.t1_random_ds_index_1 = min(horus_md.cluster_num_valid_queue_signals, horus_md.t1_random_ds_index_1);
        horus_md.t1_random_ds_index_2 = min(horus_md.cluster_num_valid_queue_signals, horus_md.t1_random_ds_index_2);
        horus_md.t2_random_ds_index_1 = min(horus_md.cluster_num_valid_queue_signals, horus_md.t2_random_ds_index_1);
        horus_md.t2_random_ds_index_2 = min(horus_md.cluster_num_valid_queue_signals, horus_md.t2_random_ds_index_2);
    }
    ...
    action compare_idle_index() {
        horus_md.idle_remove_min_id = min(horus_md.task_resub_hdr.ds_index_1, horus_md.idle_ds_index);
    }
    ...
    action act_set_queue_len_unit_1(len_fixed_point_t cluster_unit) {
        horus_md.qlen_unit_1 = cluster_unit;
    }
    ...
    action act_set_queue_len_unit_2(len_fixed_point_t cluster_unit) {
        horus_md.qlen_unit_2 = cluster_unit;
    }
    ...
    action act_set_queue_len_unit_resub(len_fixed_point_t cluster_unit) {
        horus_md.selected_ds_qlen_unit = cluster_unit;
    }
    ...
    action act_get_switch_index(switch_id_t switch_index) {
        horus_md.child_switch_index = switch_index;
    }
    ...
    apply {
        ...
        if (hdr.horus.dst_id == SWITCH_ID) { // If this packet is destined for this spine do horus processing ot. its just an intransit packet we need to forward on correct port
            ...
            */
                ...
                if (ig_intr_md.resubmit_flag!=0) {
                    if (hdr.horus.pkt_type == PKT_TYPE_NEW_TASK){
                        ...
                        if (horus_md.min_correct_qlen == horus_md.task_resub_hdr.qlen_1) {
                            horus_md.selected_ds_index = horus_md.task_resub_hdr.ds_index_1;
                            horus_md.selected_ds_qlen = horus_md.task_resub_hdr.qlen_1;
                        } else {
                            horus_md.selected_ds_index = horus_md.task_resub_hdr.ds_index_2;
                            horus_md.selected_ds_qlen = horus_md.task_resub_hdr.qlen_2;
                        }
                    }
                } else {
            ...
            @stage(3) {
                if (ig_intr_md.resubmit_flag != 0){
                    if (hdr.horus.pkt_type == PKT_TYPE_IDLE_REMOVE && horus_md.task_resub_hdr.ds_index_1 != INVALID_VALUE_16bit) { // Second pass in remove process, update the position for the leaf that was top of idle list in previous pass (we moved it to the position for the leaf that is removed)
                        update_idle_list_idx_mapping.execute(horus_md.task_resub_hdr.ds_index_2);
                    }
                } else {
                    compare_random_idx();
                    ...
                    if (hdr.horus.pkt_type == PKT_TYPE_NEW_TASK) {
                        select_idle_index(); // 
                    }  else if (hdr.horus.pkt_type == PKT_TYPE_IDLE_SIGNAL) {
                        write_idle_list_idx_mapping.execute(horus_md.cluster_absolute_leaf_index);
                    } else if (hdr.horus.pkt_type == PKT_TYPE_IDLE_REMOVE) { // First pass in remove process, find position of the to-be-removed leaf in idle list from mapping reg 
                        horus_md.task_resub_hdr.ds_index_1 = read_idle_list_idx_mapping.execute(horus_md.cluster_absolute_leaf_index);
            ...
            @stage(4) {
                if (ig_intr_md.resubmit_flag != 0) {
                    if(hdr.horus.pkt_type == PKT_TYPE_IDLE_REMOVE) { // Second pass for removing idle
                        if (horus_md.task_resub_hdr.ds_index_1 != INVALID_VALUE_16bit){
                            write_idle_list.execute(horus_md.task_resub_hdr.ds_index_1);
                            decrement_indices(); // decrement the idle index so we read the correct idle leaf ID
                        }
                        _drop();
                    } else if(hdr.horus.pkt_type == PKT_TYPE_NEW_TASK) {
                        set_queue_len_unit_resub.apply();  // Only needed in resub path since we need the qlen unit at earlier stage (before the qlen lists stage)
                        horus_md.random_ds_index_1 = horus_md.selected_ds_index; // we already selected minimum set this so we can get the corresponding id of the selected index in random_id_1 meta field
                    }
                } else {
                    if (hdr.horus.pkt_type == PKT_TYPE_NEW_TASK) {
                        ...
                        if(horus_md.t2_random_ds_index_1 == horus_md.cluster_num_valid_queue_signals) {
                             ...
                             * This will tell the compiler to put the horus_md.random_ds_index_1 in seperate MAU group 
                             * See: //https://community.intel.com/t5/Intel-Connectivity-Research/PHV-allocation-problem/m-p/1392964
                            */
                            @in_hash { 
                                horus_md.random_ds_index_1 = horus_md.t2_random_ds_index_1;    // Use second try (another random generated number)
                            }   
                        }
                    } else {
                        @in_hash {
                            horus_md.random_ds_index_1 = horus_md.t1_random_ds_index_1; // First try was fine
                        }
                    }
                    
                    }  else if (hdr.horus.pkt_type == PKT_TYPE_IDLE_REMOVE){
                        compare_idle_index();
                        horus_md.idle_ds_id = read_invalidate_idle_list.execute(horus_md.idle_ds_index);
                    }
                    else if(hdr.horus.pkt_type == PKT_TYPE_IDLE_SIGNAL) {
                        add_to_idle_list.execute(horus_md.idle_ds_index);
                        _drop();
                    } 
                }
            }
            ...
            @stage(5) {
                if (hdr.horus.pkt_type == PKT_TYPE_NEW_TASK) {
                    if(horus_md.cluster_idle_count == 0) {
                        if (horus_md.t1_random_ds_index_2 == horus_md.cluster_num_valid_queue_signals) {  // generated index was out-of-range
                            if(horus_md.t2_random_ds_index_2 == horus_md.cluster_num_valid_queue_signals) {
                                horus_md.random_ds_index_2 = horus_md.random_ds_index_1 >> 1;  // Both tries out-of-range, Divide other by two (arbitrary resolve function)
                            } else {
                                @in_hash {
                                    horus_md.random_ds_index_2 = horus_md.t2_random_ds_index_2; // Use second try (another random generated number)
                                }
                            }
                        } else {
                            @in_hash {
                                horus_md.random_ds_index_2 = horus_md.t1_random_ds_index_2;  // First try was fine  
                            }
                        }
                    } 
                } else if (hdr.horus.pkt_type == PKT_TYPE_IDLE_REMOVE) { 
                    horus_md.task_resub_hdr.ds_index_2 = horus_md.idle_ds_id; // put last idle node in resub header so we write it on the pos of deleted idle node in resub path
                } 
            }
            ...
            @stage(6) {
                ...
                if (ig_intr_md.resubmit_flag != 0) {
                     if(hdr.horus.pkt_type == PKT_TYPE_NEW_TASK) {
                        update_queue_len_list_1.execute(horus_md.selected_ds_index);
                        update_queue_len_list_2.execute(horus_md.selected_ds_index);
                     }
                } else {
                    if(hdr.horus.pkt_type == PKT_TYPE_NEW_TASK) {
                        if (horus_md.cluster_idle_count == 0) {
                            horus_md.random_ds_qlen_1 = read_queue_len_list_1.execute(horus_md.random_ds_index_1); // Read qlen for leafID1
                            horus_md.random_ds_qlen_2 = read_queue_len_list_2.execute(horus_md.random_ds_index_2); // Read qlen for leafID2
                        }
                    } else {
                        write_queue_len_list_1.execute(horus_md.child_switch_index); // Write the qlen at corresponding index for the leaf in this cluster
                        write_queue_len_list_2.execute(horus_md.child_switch_index); // Write the qlen at corresponding index for the leaf in this cluster
                        if (hdr.horus.pkt_type == PKT_TYPE_IDLE_REMOVE) {
                            if (horus_md.task_resub_hdr.ds_index_2 == INVALID_VALUE_16bit) {
                                horus_md.task_resub_hdr.ds_index_1 = INVALID_VALUE_16bit;
                            }
            ...
            @stage(8) {
                if (ig_intr_md.resubmit_flag == 0) {
                    if (hdr.horus.pkt_type == PKT_TYPE_NEW_TASK) { 
                        if (horus_md.random_ds_index_1 == horus_md.random_ds_index_2 || horus_md.cluster_idle_count > 0) {
                                horus_md.queue_len_diff = INVALID_VALUE_16bit; // No need to compare qlen diff with drift (in next stages) since rack was selected based on idle list
                        }
                        if (horus_md.cluster_idle_count == 0 && horus_md.cluster_num_valid_queue_signals > 0) {
                            if (horus_md.random_ds_index_1 != horus_md.random_ds_index_2){
                                calculate_queue_len_diff();    
                            }
                            if (horus_md.selected_ds_qlen == horus_md.random_ds_qlen_1) {
                                hdr.horus.dst_id = horus_md.random_id_1;
                                horus_md.selected_ds_index = horus_md.random_ds_index_1;
                                horus_md.task_resub_hdr.ds_index_2 = horus_md.random_ds_index_2;
                                horus_md.selected_ds_qlen_unit = horus_md.qlen_unit_1;
                                horus_md.not_selected_ds_qlen_unit = horus_md.qlen_unit_2;
                            } else {
                                hdr.horus.dst_id = horus_md.random_id_2;
                                horus_md.selected_ds_index = horus_md.random_ds_index_2;
                                horus_md.task_resub_hdr.ds_index_2 = horus_md.random_ds_index_1;
                                horus_md.selected_ds_qlen_unit = horus_md.qlen_unit_2;
                                horus_md.not_selected_ds_qlen_unit = horus_md.qlen_unit_1;
                            }
                        } else { // Idle selection
                            
                            horus_md.selected_ds_qlen_unit = horus_md.qlen_unit_1; // Put it selected_ds_qlen_unit so in next stages we increment the average load of this rack 
                            hdr.horus.dst_id = horus_md.idle_ds_id;
                            hdr.horus.qlen = 1; // This is to indicate (to leaf) that spine's view is that leaf is idle and linked with this spine (so that if leaf is not idle anymore it will break the linkage)
                        }   
                    }
                }
            }
            ...
            @stage(9) {
                if (ig_intr_md.resubmit_flag == 0) {
                    if (hdr.horus.pkt_type == PKT_TYPE_NEW_TASK && horus_md.cluster_idle_count == 0) {
                        ...
                        horus_md.deferred_qlen_1 = check_deferred_queue_len_list_1.execute(horus_md.selected_ds_index); // Returns Deferred[selected_index]
                        horus_md.task_resub_hdr.ds_index_1 = horus_md.selected_ds_index; // We keep the index of the node that initially selected in task_resub_hdr.ds_index_1
                        ...
                        horus_md.selected_ds_qlen = horus_md.selected_ds_qlen + horus_md.selected_ds_qlen_unit;
                        // Same as above but for the *other* node that initially (based on qlen list) had higher load
                        horus_md.not_selected_ds_qlen = horus_md.not_selected_ds_qlen + horus_md.not_selected_ds_qlen_unit;
                    } else if (hdr.horus.pkt_type != PKT_TYPE_NEW_TASK) {
                        reset_deferred_queue_len_list_1.execute(horus_md.child_switch_index); // Just updated the queue_len_list so write 0 on deferred reg
                    }   
                }
            }
            ...
            @stage(10) {
                if (ig_intr_md.resubmit_flag == 0) {
                    if (hdr.horus.pkt_type == PKT_TYPE_NEW_TASK && horus_md.cluster_idle_count == 0) {
                        horus_md.task_resub_hdr.qlen_1 = horus_md.deferred_qlen_1 + horus_md.selected_ds_qlen;
                        if(horus_md.deferred_qlen_1 == 0) { // This return value means that we do not need to check deffered qlens, difference between samples were large enough that our decision is still valid
                            inc_deferred_queue_len_list_2.execute(horus_md.selected_ds_index); // increment the second copy to be consistent with first one
                        ...
                        reset_deferred_queue_len_list_2.execute(horus_md.child_switch_index); // Just updated the queue_len_list so write 0 on deferred reg
                        if (hdr.horus.pkt_type == PKT_TYPE_QUEUE_SIGNAL){
                            _drop();    
                        }
                    }
