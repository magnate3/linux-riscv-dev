...
def main(args):
    ...
    pkt =  Ether(src=get_if_hwaddr(iface), dst='e4:a4:71:cd:52:99')
    #ff:ff:ff:ff:ff:ff
    #00:00:0a:00:03:02
    if(args.l4 == 'tcp'):
        pkt = pkt /IP(dst=addr) / TCP(dport=args.port, sport=random.randint(49152,65535)) / args.m
    if(args.l4 == 'udp'):
        pkt = pkt /IP(dst=addr, src='10.0.2.18') / UDP(dport=int(args.port), sport=random.randint(49152,65535)) / args.m
    pkt.show2()
