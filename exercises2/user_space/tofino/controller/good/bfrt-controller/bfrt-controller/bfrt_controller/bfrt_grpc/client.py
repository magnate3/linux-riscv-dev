...
import time
import grpc
from bfrt_controller.bfrt_grpc import bfruntime_pb2_grpc as bfruntime_pb2_grpc
from bfrt_controller.bfrt_grpc import bfruntime_pb2 as bfruntime_pb2
...
class BfruntimeErrorInResponseException(BfruntimeRpcException):
    '''
    @brief parses errors from the server.
    Errors from server has both success and fail case. Filters out success cases.
    '''
    def __init__(self, errors):
        ...
        for error in errors:
            if error.canonical_code != code_pb2.OK:
                self.errors.append((x, error))
            x = x+1
...
class _Key:
    ...
    def _get_val(self, field_in):
        ...
        if f_type == "uint64" or f_type == "bytes" or f_type == "uint32" or f_type == "uint16" or f_type == "uint8":
            ...
            key_dict["value"] = six.ensure_str(
                bytes(field_in.value) if isinstance(field_in.value, bytearray) else field_in.value)
        return key_dict
...
class _Data:
    ...
    def __init__(self, obj, data_field_list_in=[], action_name=None, get=False):
        ...
        # For each passed in field, verify size and convert to
        # appropriate format
        if not get:
...
class _GetParser:
        ...
        try:
            for rep in response:
                for entity in rep.entities:
                    ...
                    elif attr.HasField("dyn_key_mask"):
                        ret_dict["fields"] = self._parse_dkm(attr.dyn_key_mask.fields)
                    ...
                    elif attr.HasField("pre_device_config"):
                        ret_dict["pre_global_rid"] = attr.pre_global_rid.global_rid
                        ret_dict["pre_port_protection"] = attr.pre_port_protection.enable
                        ret_dict["pre_fast_failover"] = attr.pre_fast_failover.enable
                        ret_dict["pre_max_nodes_before_yield"] = attr.pre_max_nodes_before_yield.count
                        ret_dict["pre_max_node_threshold"] = {}
                        ret_dict["pre_max_node_threshold"]["node_count"] = attr.pre_max_node_threshold.node_count
                        ret_dict["pre_max_node_threshold"]["port_lag_count"] = attr.pre_max_node_threshold.port_lag_count
                    yield ret_dict
        except grpc.RpcError as e:
            raise BfruntimeReadWriteRpcException(e)
    ...
    def _parse_entry_write_response(self, resp, metadata=None):
        if metadata is not None and ("error_in_resp", "1") in metadata:
            if self._status_has_error(resp.status):
                raise BfruntimeErrorInResponseException(resp.status)

    def _parse_entry_get_response(self, response, get_default_entry=False, metadata=None):
        try:
            for rep in response:
                for entity in rep.entities:
                    data = key = tgt = None
                ...
                if metadata is not None and ("error_in_resp", "1") in metadata:
                    if self._status_has_error(rep.status):
                        raise BfruntimeErrorInResponseException(rep.status)

        except grpc.RpcError as e:
            raise BfruntimeReadWriteRpcException(e)

    def _status_has_error(self, status):
        for error in status:
            if error.canonical_code != code_pb2.OK:
                return True
        return False
...
class _Table:
    ...
    def entry_add(self, target, key_list=None, data_list=None,
            ...
            metadata=None, entry_tgt_list=None):
        """@brief Insert table entries.
            len of key_list, data_list, action_name_list and entry_tgt_list(when specified)
            should be equal
            When entry_tgt_list is provided, it takes precedence over the target.
            ...
            @param metadata : optional metadata to send with write request
            @param entry_tgt_list List of entry_tgts.
            Used for specifying target at entry level for asymmetric tables.
        """
        ...
        resp = self.reader_writer_interface._write(self._entry_write_req_make(req, key_list,
            data_list, bfruntime_pb2.Update.INSERT, entry_tgt_list=entry_tgt_list), metadata)
        self.get_parser._parse_entry_write_response(resp, metadata=metadata)

    def entry_mod(self, target, key_list=None, data_list=None,
                  flags={"reset_ttl":True}, p4_name=None, metadata=None,
                  entry_tgt_list=None):
        ...
        _cpy_target(req, target)
        resp = self.reader_writer_interface._write(self._entry_write_req_make(req, key_list,
            data_list, bfruntime_pb2.Update.MODIFY, flags=flags, entry_tgt_list=entry_tgt_list), metadata)
        self.get_parser._parse_entry_write_response(resp, metadata=metadata)

    def entry_mod_inc(self, target,
                           key_list=None, data_list=None,
                           modify_inc_type=bfruntime_pb2.TableModIncFlag.MOD_INC_ADD,
                           p4_name=None, metadata=None, entry_tgt_list=None):
        """@brief Modify table entries.
            len of key_list, data_list, action_name_list and entry_tgt_list(when specified)
            should be equal
            When entry_tgt_list is provided, it takes precedence over the target.
        ...
        _cpy_target(req, target)
        resp = self.reader_writer_interface._write(self._entry_write_req_make(req, key_list,
            data_list, bfruntime_pb2.Update.MODIFY_INC, modify_inc_type, entry_tgt_list=entry_tgt_list), metadata)
        self.get_parser._parse_entry_write_response(resp, metadata=metadata)

    def entry_del(self, target, key_list=None, p4_name=None, metadata=None, entry_tgt_list=None):
        """@brief Delete table entries.
            len of key_list and entry_tgt_list(when specified)
            should be equal
            When entry_tgt_list is provided, it takes precedence over the target.
            @param target target device
            @param key_list List of Keys.
        ...
        if key_list == None or key_list == []:
            key_list = [None]
        resp = self.reader_writer_interface._write(self._entry_write_req_make(req, key_list,
            None, bfruntime_pb2.Update.DELETE, entry_tgt_list=entry_tgt_list), metadata)
        self.get_parser._parse_entry_write_response(resp, metadata=metadata)
    ...
    def default_entry_set(self, target, data, p4_name=None, metadata=None):
        ...
        resp = self.reader_writer_interface._write(req, metadata)
        self.get_parser._parse_entry_write_response(resp, metadata=metadata)
    ...
    def default_entry_get(self, target, flags={"from_hw":True}, required_data=None, p4_name=None, metadata=None):
        ...
        return self.get_parser._parse_entry_get_response(resp, get_default_entry=True, metadata=metadata)
    ...
    def entry_get(self, target,
        ...
        resp = self.reader_writer_interface._read(self._entry_read_req_make(req, key_list, flags,
            required_data, False, handle, entry_tgt_list=entry_tgt_list), metadata)
        return self.get_parser._parse_entry_get_response(resp, metadata=metadata)
    ...
    def attribute_get(self, target, attribute_name, p4_name=None, metadata=None):
        """@brief Get attribute
            ...
            c. DynamicKeyMask = {"fields": <obj of _Key class>}
            ...
            h. pre_device_config = {"pre_global_rid": <int>,
                                    "pre_port_protection": <int>,
                                    "pre_fast_failover": <int>,
                                    "pre_max_nodes_before_yield": <int>,
                                    "pre_max_node_threshold": {"node_count":<int>,
                                                               "port_lag_count":<int>}}
    ...
    def attribute_dyn_key_mask_set(self, target, key, p4_name=None, metadata=None):
        """ Set dynamic key mask for the exact match table
            @param target : target device
            @param key List of (name, mask) tuples.
            @param p4_name string containing name of P4 with which to communicate
            optional for subscribed clients, it is mainly used for independent clients
            who want to communicate with a p4 without having to subscribe
            @param metadata : optional metadata to send with write request
        ...
        for field in list(key.field_dict.values()):
            ...
            key_field.mask =  bytes(field.value) if isinstance(field.value, bytearray) else six.ensure_binary(field.value)

        resp = self.reader_writer_interface._write(req, metadata)
        self.get_parser._parse_entry_write_response(resp, metadata=metadata)

    def attribute_dyn_hashing_set(self, target, alg_hdl = 0, seed = 0, p4_name=None, metadata=None):
        """@brief Set dynamic hashing attribute (algorithm_handler and seed) for the dynamic hashing table
            @param target target device
    ...
    def attribute_pre_device_config_set(self, target, global_rid=None, port_protection_enable=None, fast_failover_enable=None, max_nodes_before_yield=None, max_node_threshold_node_count=None, max_node_threshold_port_lag_count=None, p4_name=None, metadata=None):
    ...
    def _attribute_read_req_make(self, req, attribute_name):
        ...
        elif attribute_name == "DynamicKeyMask":
            table_attribute.dyn_key_mask.SetInParent()
        ...
        elif attribute_name == "pre_device_config":
            table_attribute.pre_device_config.SetInParent()
        return req
    ...
    def _entry_read_req_make(self, req, key_list,
            flags, required_data, default_entry, handle=None, entry_tgt_list=None):
        action_name = None
        req_field_list = []
        ...
        if handle is not None:
            ...
            # In case of read by handle, entry_tgt_list has only one target
            if entry_tgt_list is not None and entry_tgt_list[0] is not None:
                self._set_entry_tgt(table_entry, entry_tgt_list[0])

        elif key_list is not None and len(key_list) > 0:
            #if entry_tgt_list present, it should be equal in length to key_list/data_list
            if entry_tgt_list is not None:
                assert(len(entry_tgt_list) == len(key_list))
            ...
            for idx in range(len(key_list)):
                ...
                if entry_tgt_list is not None and entry_tgt_list[idx] is not None:
                    self._set_entry_tgt(table_entry, entry_tgt_list[idx])
        else:
    ...
    def _entry_write_req_make(self, req, key_list, data_list, update_type, modify_inc_type=None,
                        flags=None, entry_tgt_list=None):
        ...
        if key_list:
            ...
            ''' If both key_list and entry_tgt_list are provided,
                they should be of equal len.
            '''
            if entry_tgt_list is not None and key_list[0] is not None:
                assert(len(entry_tgt_list) == len(key_list))
        else:
    ...
    def _set_entry_tgt(self, table_entry, target):
        table_entry.entry_tgt.device_id = target.device_id_
        table_entry.entry_tgt.pipe_id = target.pipe_id_
        table_entry.entry_tgt.direction = target.direction_
        table_entry.entry_tgt.prsr_id = target.prsr_id_
    ...
    def _set_table_key(self, table, key_fields):
        ...
        for field in key_fields:
            ...
            if field.mask is not None:
                key_field.ternary.value = bytes(field.value) if isinstance(field.value, bytearray) else six.ensure_binary(field.value)
                key_field.ternary.mask = bytes(field.mask) if isinstance(field.mask, bytearray) else six.ensure_binary(field.mask)
            elif field.prefix_len is not None:
                key_field.lpm.value = bytes(field.value) if isinstance(field.value, bytearray) else six.ensure_binary(field.value)
                key_field.lpm.prefix_len = field.prefix_len
            elif field.low is not None or field.high is not None:
                key_field.range.low = bytes(field.low) if isinstance(field.low, bytearray) else six.ensure_binary(field.low)
                key_field.range.high = bytes(field.high) if isinstance(field.high, bytearray) else six.ensure_binary(field.high)
            elif field.is_valid is not None:
                key_field.optional.value = bytes(field.value) if isinstance(field.value, bytearray) else six.ensure_binary(field.value)
                key_field.optional.is_valid = field.is_valid
            else:
                if isinstance(field.value, str):
                    key_field.exact.value = field.value.encode()
                else:
                    key_field.exact.value = bytes(field.value) if isinstance(field.value, bytearray) else six.ensure_binary(field.value)
    ...
    def _set_table_data_single_field(self, action, field, proto_data_field):
        ...
        if field.val is not None:
            proto_data_field.stream = bytes(field.val) if isinstance(field.val, bytearray) else six.ensure_binary(field.val)
...
class _BfRtInfo:
    ...
    def __init__(self, p4_name, p4_json_data, non_p4_json_data, reader_writer_interface):
        ...
        # Create a sorted table list which is primarily topologically sorted according
        # to depends_on of the table and secondarily sorted lexicographically.
        # So MAT would come first, then related ActSel and then ActProf.
        # If MAT1 and MAT2 are sharing same ActProf, then
        # MAT1, MAT2, ActProf will be the order where MAT1< MAT2 lexicographically
        self.table_list_sorted = _BfRtInfo._create_sorted_table_list(
                sorted(list(self.parsed_info.table_info_dict_get().keys()), reverse=True),
                self.table_dict,
                self.table_id_dict)

    @staticmethod
    def _create_sorted_table_list(table_names, table_dict, table_id_dict):
        """@brief Util func for recursively topological sorting
        """
        def topo_sort_util(table_name, visited, stack, table_dict, table_id_dict):
            visited[table_name] = True
            # Recur for all tables that are dependent on this
            for dep_id in table_dict[table_name].info.depends_on_get():
                dep_table = table_id_dict[dep_id]
                if visited[dep_table.info.name_get()] == False:
                    topo_sort_util(dep_table.info.name_get(), visited, stack, table_dict, table_id_dict)
            stack.append(table_name)

        visited = {table_name:False for table_name in table_names}
        stack = []
        for table_name in table_names:
            if visited[table_name] == False:
                topo_sort_util(table_name, visited, stack, table_dict, table_id_dict)
        return stack[::-1]
    ...
    def table_name_list_get(self):
        """@brief Get list of fully-qualified names of all tables
            @return List
        """
        return self.table_list_sorted

    def learn_name_list_get(self):
        """@brief Get list of fully-qualified names of all learns
            @return List
        """
        return list(self.parsed_info.learn_info_dict_get().keys())
