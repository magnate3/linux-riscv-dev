...
class _TableInfo:
    ...
    """
    def __init__(self, name, table_id, table_type, size, attributes, operations, annotations,
            has_const_default_action, depends_on):
    ...
    def depends_on_get(self):
        """@brief Get list of IDs of tables
                this table depends upon. For
                example, for a Match table, its
                ActionProfile and ActionSelector tables'
                IDs would be returned
...
class BfRtInfoParser:
    ...
    def _parse_table(table_json):
        ...
        if "depends_on" in table_json:
            depends_on = table_json["depends_on"]
