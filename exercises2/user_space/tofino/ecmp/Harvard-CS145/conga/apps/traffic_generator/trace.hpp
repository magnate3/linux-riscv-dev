...
public:
    ...
    int get_next_trace_item(std::string src_host, TraceItem* trace_item)
    {
        ...
        while (cur_item_index_ < trace_item_list_.size()) {
            if (trace_item_list_[cur_item_index_].src_host_ == src_host && trace_item_list_[cur_item_index_].trace_type_ == 2) {
    ...
    int print_traces()
    {
        for (auto item : trace_item_list_) {
            std::cout << item.src_host_ << " " << item.start_time_ << " " << item.trace_type_ << " " << item.dst_host_or_mc_key_ << " " << item.flow_duration_ << std::endl;
        }
    }
    ...
    int parse_trace_str(std::string trace_str, TraceItem& item)
    {
        ...
        if (host_list_.find(src_host) == host_list_.end()) {
            std::cerr << "Invalid hostname " << src_host << ", line: " << trace_str << std::endl;
            return -1;
        }
        ...
        if (trace_type == 2) {
            ...
            if (flow_duration < 0) {
                std::cerr << "Invalid flow duration time " << flow_duration << std::endl;
                return -1;
            }
        ...
        if (trace_type == 2) {
            start_idx = idx + delimiter.length();
            idx = trace_str.find(delimiter, start_idx);
            std::string flowlet_duration_str = trace_str.substr(start_idx, idx - start_idx);
            double flowlet_duration = atof(flowlet_duration_str.c_str());
            if (flowlet_duration < 0) {
                std::cerr << "Invalid flowlet duration time " << flowlet_duration << std::endl;
                return -1;
            }
            ...
            start_idx = idx + delimiter.length();
            idx = trace_str.find(delimiter, start_idx);
            std::string flowlet_gap_str = trace_str.substr(start_idx, idx - start_idx);
