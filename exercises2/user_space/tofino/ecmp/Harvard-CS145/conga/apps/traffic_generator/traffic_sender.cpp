...
DEFINE_string(tracefile, "apps/trace/trace.txt", "The trace filename to replay");
...
void threadTcpConnection(size_t tid, std::string host, std::string dst_ip_addr, double flow_start_time, double flow_duration, double flowlet_duration, double flowlet_gap)
{
    ...
    cur_time = GetTimeUs();
    std::cout << "Total bytes sent: " << total_bytes_sent / 1000 << "Kbytes" << std::endl;
    std::cout << "Total time: " << (cur_time - flow_start_time) / 1000000 << " seconds" << std::endl;
    std::cout << "Average throughput: " << total_bytes_sent * 8 / 1000 / ((cur_time - flow_start_time) / 1000000) << "Kbps" << std::endl;
    all_client_thread_throughputs[tid] = total_bytes_sent * 8 / 1000 / ((cur_time - flow_start_time) / 1000000);
}
...
void LaunchTcpClient(std::string host, Trace& trace)
{
    ...
    while (trace.get_next_trace_item(host, &item) == 0) {
        ...
        std::thread *t = new std::thread(threadTcpConnection, cur_tid, host, item.dst_host_or_mc_key_, item.start_time_ + FLAGS_start_time * 1000000, item.flow_duration_, item.flowlet_duration_, item.flowlet_gap_);
...
void threadUdpConnection(size_t tid, std::string host, std::string dst_ip_addr, double flow_start_time, double flow_duration, double flowlet_duration, double flowlet_gap)
{
    std::cout << "threadUdpConnection: " << host << " " << dst_ip_addr << " " << flow_start_time << " " << flow_duration << std::endl;
...
void LaunchUdpClient(std::string host, Trace& trace)
{
    ...
    while (trace.get_next_trace_item(host, &item) == 0) {
        WaitUntil(item.start_time_ + FLAGS_start_time * 1000000 - 1000000);
        std::cout << "Sending traffic to " << item.dst_host_or_mc_key_ << std::endl;
        std::thread *t = new std::thread(threadUdpConnection, cur_tid, host, item.dst_host_or_mc_key_, item.start_time_ + FLAGS_start_time * 1000000, item.flow_duration_, item.flowlet_duration_, item.flowlet_gap_);
        all_client_threads.push_back(t);
        all_client_thread_throughputs.push_back(0);
        cur_tid++;
    }
