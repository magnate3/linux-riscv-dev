...
control MyIngress(
    ...
    action update_bloom_filter() {
        meta.ctr_exceeded = read_and_update_bloom_filter.execute(meta.index);
    }
    ...
    apply {
        ...
        if (hdr.ipv4.isValid()) {
            if (hdr.transport.isValid()){
                ...
                if (meta.ctr_exceeded == 1){
                    tbl_drop_table.apply();
                }
            }
        }
    }
