...
# P4 Target
if [ -z $P4C_DEFAULT_TARGET ]; then
   with_tofino="--with-tofino"     # Compile for Tofino/Tofino-model
else
    with_tofino=--with-$P4C_DEFAULT_TARGET
fi
...
#
print_version() {
    echo "Designed for SDE-9.7.1 (cmake-based p4-build)"
}
...
check_program() {
    if p4_includes $P4_REALPATH tofino/intrinsic_metadata.p4 inc_tofino; then
        ...
        p4_includes $P4_REALPATH tofino1_arch.p4     inc_tofino1arch
        p4_includes $P4_REALPATH t2na.p4            inc_t2na
        p4_includes $P4_REALPATH tofino2_arch.p4     inc_tofino2arch
...
EOF
        ...
        AUTO_P4_VERSION=p4_16
        if [ $inc_tofino1arch == "yes" -o $inc_tofino2arch == "yes" ]; then
            if [ $with_tofino == "--with-tofino" ]; then
                AUTO_P4_ARCHITECTURE=tna
                p4_autodetected=1
            elif [ $with_tofino == "--with-tofino2" ]; then
...
EOF
        ...
        # Target is tofino or tofino2
        P4_TARGET=`echo $with_tofino | cut -d- -f4`
    fi
    ...
    P4_BUILD=$SDE_BUILD/p4-build/${P4_TARGET}/${P4_NAME}${P4_SUFFIX}
    P4_LOGS=$SDE_LOGS/p4-build/${P4_TARGET}/${P4_NAME}${P4_SUFFIX}
	...
	echo "Ok: p4-build package  found in $SDE"
...
build_p4_prog_cmake() {
    ...
    if [ $P4_VERSION == "p4_14" ]; then
        ...
        if [ $with_thrift == '--with-thrift' ]; then
            thrift='-DTHRIFT-DRIVER=ON'
        fi
    else
        P4_ARCH=$P4_ARCHITECTURE
        if [ $P4_ARCH == "t2na" ]; then
...
EOF
    ...
    P4_ARCH=`echo $P4_ARCH | tr a-z A-Z`

    
    case $with_tofino in
        --with-tofino) targets='-DTOFINO=True -DTOFINO2=False';;
        --with-tofino2) targets='-DTOFINO=False -DTOFINO2=True';;
    esac
    ...
    for user_var in $p4_build_vars; do
        case $user_var in
            ...
            ;;
            *) eval \
               p4_build_args="\"-D$user_var=\\\"\${$user_var}\\\" $p4_build_args\""
               ;;
        esac
    done
    #echo "p4_build_args=$p4_build_args"
    ...
    start_log $P4_LOGS/cmake.log $cmd "$p4_build_args" "$@"
    if eval $cmd "$p4_build_args" "$@" &>> $P4_LOGS/cmake.log; then 
        echo -ne "\b\b\b\b"
    else
        echo FAILED
        show_log $P4_LOGS/cmake.log
        cd $WORKDIR
        return 1
    fi
...
#
#if [ -f $SDE/cmake/P4Build.cmake ]; then
build_p4_prog_cmake "$@"
#else
    #build_p4_prog "$@"
#fi
