...
#define TEST_ETHERCAT 1
#ifdef  TEST_ETHERCAT
   #include "master/device.h" 
static void ec_poll(struct net_device *dev);
extern ec_device_t *ecdev_offer(struct net_device *net_dev, ec_pollfunc_t poll,
        struct module *module);
extern void ecdev_withdraw(ec_device_t *device);
#endif
#define TEST_CUSTOMZIE_PROTO 1
#ifdef  TEST_CUSTOMZIE_PROTO
static netdev_tx_t macb_start_xmit(struct sk_buff *skb, struct net_device *dev);
#define DEST_IP "10.10.0.33"
#define SRC_IP "10.10.0.11"
		  ...
		  int budget)
{
...
#ifdef TEST_CUSTOMZIE_PROTO
	//dma_unmap_single has done 
        pr_err("htons(ETH_P_ARP) %d =? skb->protocol  %d \n", htons(ETH_P_ARP), skb->protocol);
	...
	{    
	    ...
	    struct net_device *dev = bp->dev;
            ...
            unsigned char *sha;
            __be32 sip, tip;
	    pr_err("before skb_reset_network_header, the  skb->network_header %u ", skb->network_header);
	    skb_reset_network_header(skb);
	    pr_err("after skb_reset_network_header, the  skb->network_header %d ", skb->network_header);
            //调用arp_hdr获取skb数据中arp头的开始指针
	    arp = arp_hdr(skb);
        pr_err("htons(ARPOP_REQUEST) %d =? arp_hdr(skb)->ar_op  %u \n" , htons(ARPOP_REQUEST), arp->ar_op);
	    if (htons(ARPOP_REQUEST) == arp->ar_op)
	    {
	       ...
	       char buf[64];
	       unsigned char * ip;
	       sprintf(buf, "%x:%x:%x:%x:%x:%x \0",sha[0], sha[1], sha[2], sha[3], sha[4], sha[5]);
	       pr_err("recv arp src mac %s",  buf);
	       ip = (unsigned char *)&sip;
	       sprintf(buf, "%d.%d.%d.%d \0",ip[0], ip[1], ip[2], ip[3]);
	       pr_err("recv arp send ip %s",  buf);
               skb2 = arp_create(ARPOP_REPLY, ETH_P_ARP,  sip, dev, tip,sha, dev->dev_addr, sha);
               if (!skb2) {
