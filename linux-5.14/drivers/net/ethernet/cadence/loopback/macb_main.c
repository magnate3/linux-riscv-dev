...
#include "macb.h"
#define TEST 1
#define TEST_REG_STATUS 1
#define TEST_CDS_REG 1
#define FIXED_HCLK_RATE 48000000
#define TEST_CLK 0
#define TEST_DMA
#define TEST_SKB_DMA 0
#define TEST_AUTO_UDP_SEND 0
#define TEST_AUTO_UDP_SEND_BY_SYS 1
#define TEST_AUTO_ARP_SEND 0
#define DEST_IP "10.10.10.33"
#define SRC_IP "10.10.10.16"
#if TEST_AUTO_UDP_SEND
    extern struct sk_buff *  build_udp_skb(struct net_device* dev, uint8_t dest_addr[ETH_ALEN], uint16_t proto);
#endif
#if TEST_AUTO_UDP_SEND_BY_SYS
#define TEST_PROTO 0x88A4
#define TEST_DATA_LEN 1400
static int gmac_sys_init(void);
#define ETH0 "eth0"
    struct net_device  *g_gmacdev;
    extern struct sk_buff *  build_udp_skb(struct net_device* dev, uint8_t dest_addr[ETH_ALEN], uint16_t proto);
    extern struct sk_buff *  build_eth_skb(struct net_device* dev, uint8_t dest_addr[ETH_ALEN], uint16_t proto, int data_len);
#endif
#if TEST_AUTO_ARP_SEND 
#include<linux/if_arp.h>
#include<net/arp.h>
#include<linux/inet.h>
#endif
//#define FIXED_HCLK_RATE 81250000
...
static void print_eth_proto(struct sk_buff *skb, bool rx);
...
{
        //pr_err("tracing hw_readl_native base  0x%p, offest  %x", bp->regs, offset);
	return __raw_readl(bp->regs + offset);
}

static void hw_writel_native(struct macb *bp, int offset, u32 value)
{
        //pr_err("tracing hw_writel_native base  0x%p, offest  %x, value %x", bp->regs, offset, value);
	__raw_writel(value, bp->regs + offset);
}

static u32 hw_readl(struct macb *bp, int offset)
{
        //pr_err("tracing hw_readl base  0x%p, offest  %x", bp->regs, offset);
	return readl_relaxed(bp->regs + offset);
}

static void hw_writel(struct macb *bp, int offset, u32 value)
{
        //pr_err("tracing hw_writel base  0x%p, offest  %x, value %x", bp->regs, offset, value);
...
static int macb_mdio_read(struct mii_bus *bus, int mii_id, int regnum)
{
	...
	status = macb_mdio_wait_for_idle(bp);
        //pr_err("tracing macb_mdio_wait_for_idle %d, %d", status, __LINE__);
	if (status < 0)
		goto mdio_read_exit;
	...
	status = MACB_BFEXT(DATA, macb_readl(bp, MAN));
        ...
        //pr_err("tracing MACB_BFEXT %d %d", status,__LINE__);
mdio_read_exit:
			   ...
			   u16 value)
{
	...
	status = macb_mdio_wait_for_idle(bp);
        //pr_err("tracing macb_mdio_wait_for_idle %d line %d", status,__LINE__);
	if (status < 0)
		goto mdio_write_exit;
...
#if TEST_CDS_REG
	pr_err(" tracing reading NCFGR  %x \n", ctrl);
#endif
...
#if TEST_CDS_REG
	u32 ctrl2 = macb_or_gem_readl(bp, NCFGR);
	pr_err(" tracing after set ctrl %x , reading NCFGR   %x \n", ctrl, ctrl2);
#endif
...
#if TEST_REG_STATUS
	u32 regs_ncr = macb_readl(bp, NCR);
	pr_err(" tracing after link up , reading NCR   %x \n", regs_ncr);
#endif
...
static int macb_phylink_connect(struct macb *bp)
{
	...
	if (!dn || (ret && !macb_phy_handle_exists(dn))) {
		netdev_err(dev, "tracing call phy_find_first \n");
	...
	if (ret) {
		netdev_err(dev, "Could not attach PHY (%d) and NULL == dn %d \n", ret, NULL == dn);
		return ret;
	}
...
#if TEST_SKB_DMA
			unsigned char * skb_data1 =  skb->data;
#endif
			/* properly align Ethernet header */
			skb_reserve(skb, NET_IP_ALIGN);
#if TEST_SKB_DMA
			unsigned char * skb_data2 =  skb->data;
			pr_err(" gem_refill skb_data1:  %x , skb_data2 %x",skb_data1, skb_data2);
#endif
		} else {
			desc->ctrl = 0;
			dma_wmb();
		  ...
		  int budget)
{
	...
	while (count < budget) {
		...
		if(htons(TEST_PROTO) == eth->h_proto)
		{
                     char * head =(char*) (eth +1);
                     char buf[1500];
		     snprintf(buf, TEST_DATA_LEN + 1, "%s", head); 
		     pr_err("************* recv : %s \n", buf);
		}
#endif
...
static int macb_poll(struct napi_struct *napi, int budget)
{
	...
	//pr_err( "tracing poll: status = %08lx, budget = %d\n",(unsigned long)status, budget);
...
static void print_eth_proto(struct sk_buff *skb, bool rx)
 {
    ...
    if (rx){
	char buf[64];
	sprintf(buf, "%x:%x:%x:%x:%x:%x", eth->h_dest[0], eth->h_dest[1], eth->h_dest[2], eth->h_dest[3], eth->h_dest[4], eth->h_dest[5]);
	pr_err("in recv proto  %x and dest mac %s", ntohs(eth->h_proto), buf);
    }
    else {
	pr_err("in send %x",  ntohs(eth->h_proto));
    }
    switch (eth->h_proto) {
        case htons(ETH_P_IP):
		pr_err("tracing proto %x ip \n", ntohs(eth->h_proto));
		break;
	case htons(ETH_P_ARP):
		pr_err("tracing proto %x arp \n", ntohs(eth->h_proto));
		break;
	default:
		pr_err("tracing proto %x \n", ntohs(eth->h_proto));
...
static netdev_tx_t macb_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
     ...
     char mac[ETH_ALEN] = {0xff,0xff,0xff,0xff,0xff,0xff};
     __be32 src_addr, dst_addr;
     memcpy(dest_mac, mac,ETH_ALEN);
     in4_pton(DEST_IP, strlen(DEST_IP),(u8 *)&dst_addr, '\0', NULL);
     in4_pton(SRC_IP, strlen(SRC_IP), (u8 *)&src_addr, '\0', NULL);
     skb = arp_create(ARPOP_REQUEST, ETH_P_ARP,  dst_addr, dev, src_addr,dest_mac, dev->dev_addr, NULL);
     if (!skb) {
        pr_err(" tracing arp_create fails \n");
     }
     skb->dev = dev;
     skb->pkt_type = PACKET_OUTGOING;
     skb->no_fcs = 1;
     //skb->protocol = htons(ETH_P_ARP);have done in arp_create
#endif
#if TEST_AUTO_UDP_SEND
     dev_kfree_skb_any(skb);
     //struct sk_buff *skb;
     ...
     //in4_pton(DEST_IP, strlen(DEST_IP),(u8 *)&dst_addr, '\0', NULL);
     //in4_pton(SRC_IP, strlen(SRC_IP), (u8 *)&src_addr, '\0', NULL);
     skb = build_udp_skb(dev,dest_mac, ETH_P_IP);
#endif
	...
	netdev_tx_t ret = NETDEV_TX_OK;
        //pr_err("tracing  macb_start_xmit \n %d",__LINE__); 
...
//#if defined(DEBUG) && defined(VERBOSE_DEBUG)
	//pr_err("start_xmit: queue %hu len %u head %p data %p tail %p end %p\n",
	//	    queue_index, skb->len, skb->head, skb->data,
	//	    skb_tail_pointer(skb), skb_end_pointer(skb));
	//print_eth_proto(skb,false);
	netdev_vdbg(bp->dev,
...
//#endif
	...
	if (!macb_tx_map(bp, queue, skb, hdrlen)) {
		//pr_err("tracing a dma map error");
		dev_kfree_skb_any(skb);
		goto unlock;
	}
	...
	//pr_err("tracing tx_head = %u, tx_tail = %u\n",
	//		   queue->tx_head, queue->tx_tail);
...
static int macb_alloc_consistent(struct macb *bp)
{
	...
	for (q = 0, queue = bp->queues; q < bp->num_queues; ++q, ++queue) {
		...
		netdev_dbg(bp->dev,
			   ...
			   queue->tx_ring);
	       pr_err("Allocated TX ring for queue %u of %d bytes at %08lx (mapped %p)\n",
			   q, size, (unsigned long)queue->tx_ring_dma,
			   queue->tx_ring);
		...
		pr_err("Allocated RX ring of %d bytes at %08lx (mapped %p)\n",
			   size, (unsigned long)queue->rx_ring_dma, queue->rx_ring);
	}
...
#if TEST_CDS_REG
	u32 regs_ncr = macb_readl(bp, NCR);
	u32 regs_ncfgr  = macb_or_gem_readl(bp, NCFGR);
	u32 regs_ncr2 = macb_readl(bp, NSR);
	pr_err("tracing regs %x,%x,%x,%x \n", regs_ncr, regs_ncfgr, regs_ncr2, config);
#endif
	macb_writel(bp, NCFGR, config);
#if TEST_CDS_REG
	regs_ncr = macb_readl(bp, NCR);
	regs_ncfgr  = macb_or_gem_readl(bp, NCFGR);
	regs_ncr2 = macb_readl(bp, NSR);
	pr_err("tracing after set NCFGR regs %x,%x,%x,%x \n", regs_ncr, regs_ncfgr, regs_ncr2, config);
#endif
...
static int macb_open(struct net_device *dev)
{
	...
	netdev_err(bp->dev, "tracing open\n");
...
#ifdef TEST_REG_STATUS
	u32 regs_ncr = macb_readl(bp, NCR);
	u32 regs_ncfgr  = macb_or_gem_readl(bp, NCFGR);
	pr_err("after macb_open, tracing regs status NCR: %x, NCFGR: %x \n", regs_ncr, regs_ncfgr);
#endif
	return 0;
			 ...
			 struct clk **rx_clk, struct clk **tsu_clk)
{
	...
	dev_err(&pdev->dev, "tracing macb_clk_init NULL == pdata ?  %d.\n", NULL == pdata);
	if (pdata) {
		*pclk = pdata->pclk;
		*hclk = pdata->hclk;
	} else {
		//*pclk = devm_clk_get(&pdev->dev, "timer1");
		//*hclk = devm_clk_get(&pdev->dev, "timer2");
#if TEST_CLK
     /* initialize clocks */
        *pclk = clk_register_fixed_rate(&pdev->dev, "pclk", NULL, 0, FIXED_HCLK_RATE);
	if (IS_ERR_OR_NULL(*pclk)){
		dev_err_probe(&pdev->dev,
				     IS_ERR(*pclk) ? PTR_ERR(*pclk) : -ENODEV,
				     "failed to register pclk\n");
               err = PTR_ERR(*pclk);
               goto err_pclk_register;
        }
        *hclk = clk_register_fixed_rate(&pdev->dev, "hclk", NULL, 0,FIXED_HCLK_RATE);
...
static int macb_init(struct platform_device *pdev)
{
	...
	bp->rx_ring_size = DEFAULT_RX_RING_SIZE;
                ...
                int i, len;
                interrupts = of_get_property(pdev->dev.of_node, "interrupts", &len);
                if(NULL == interrupts){
			dev_err(&pdev->dev, "request IRQ empty \n");
                        return -ENOMEM;
                }
		dev_err(&pdev->dev, "IRQ num : ");
                for(i = 0; i < len; ++ i)
                {
		     dev_err(&pdev->dev, "%d\n ,", interrupts[i]);
                }
                */
                int irq_count = of_irq_count(pdev->dev.of_node);
                int index = 0;
		dev_err(&pdev->dev, "IRQ %d num :%d, %d \n ",of_irq_count(pdev->dev.of_node), platform_get_irq(pdev, 0), platform_get_irq(pdev, 1));
		//dev_err(&pdev->dev, "IRQ %d num :%d, %d \n ",of_irq_count(pdev->dev.of_node), of_irq_get(pdev->dev.of_node, 0), of_irq_get(pdev->dev.of_node, 1));
#endif
	...
	for (hw_q = 0, q = 0; hw_q < MACB_MAX_QUEUES; ++hw_q) {
		...
		//queue->irq = platform_get_irq(pdev, 0);
		//queue->irq =  62;
#else
			       ...
			       struct clk **rx_clk, struct clk **tsu_clk)
{
	...
	pr_err("tracing fu540_c000_init call fu540_c000_clk_init %s \n", pdev->name);
...
{
	pr_err("tracing fu540_c000_init call fu540_c000_init %s \n", pdev->name);
	mgmt->reg = devm_platform_ioremap_resource(pdev, 1);
	if (IS_ERR(mgmt->reg)) {
	      pr_err("tracing fu540_c000_init resources not exist ************** \n");
	//	return PTR_ERR(mgmt->reg);
        }

	pr_err("tracing fu540_c000_init call macb_init \n");
...
#if TEST_AUTO_UDP_SEND_BY_SYS
static int gmac_value;
static ssize_t gmac_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
   ...
   while(-- loop > 0)
   {
      skb = build_eth_skb(g_gmacdev,dest_mac, TEST_PROTO,TEST_DATA_LEN);
      macb_start_xmit(skb, g_gmacdev); 
   }
   skb = build_udp_skb(g_gmacdev,dest_mac, ETH_P_IP);
   macb_start_xmit(skb, g_gmacdev); 
   return 0;
}
static ssize_t gmac_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
#if 0
    struct macb *bp = netdev_priv(g_gmacdev);
    u32 read, reg_addr, page, reg_value;
    u32 ctrl;
    sscanf(buf, "%x,%x,%x",&read, &reg_addr, &reg_value);
    if(read)
    {
         //ctrl = macb_or_gem_readl(bp, reg_addr);
         //pr_err(" read ctrl %x status  %x \n" ,reg_addr, ctrl);
    }
    else
    {
	 if (MACB_NCFGR  == reg_addr)
          {
         ctrl = macb_or_gem_readl(bp, NCFGR);
         pr_err(" before  set reg %x ctrl %x \n" ,reg_addr, ctrl);
         ctrl |= GEM_BIT(GBE);
         macb_or_gem_writel(bp, NCFGR, ctrl);
	  } 
	 else
         {
             //ctrl = macb_or_gem_readl(bp, reg_addr);
             //pr_err(" before  set reg %x ctrl %x \n" ,reg_addr, ctrl);
             //macb_or_gem_writel(bp, NCFGR, reg_value);
	 }
    }
#endif
    int rw,value, addr;
    u32 ctrl;
    struct macb *bp = netdev_priv(g_gmacdev);
    sscanf(buf, "%x,%x,%x", &rw,&addr, &value);
    if (0 == rw)
    {
         pr_info("set reg addr %x, value %x \n", addr, value);
	 hw_writel_native(bp, addr, value);
   }
   else if ( 1 == rw)
   {
            pr_info("reg addr: %x , status : %x \n",  addr, hw_readl_native(bp,addr));
   }
    else if (3 == rw)
   {
	 if (MACB_NCFGR  == addr)
...
static struct kobj_attribute gmac_attribute = __ATTR(gmac_value, 0664, gmac_show, gmac_store);
static struct kobject *gmac_kobj = NULL;
static int gmac_sys_init(void)
{
    //struct net_device *ndev = phydev->attached_dev
    int retval;
   pr_info("***********  gmac sys init %s \n", g_gmacdev->name);
    gmac_kobj = kobject_create_and_add("gmac", kernel_kobj);
    if (!gmac_kobj)
        return -ENOMEM;
    retval = sysfs_create_file(gmac_kobj, &gmac_attribute.attr);
    if (retval)
        kobject_put(gmac_kobj);
   return retval;
}
static void gmac_remove(struct phy_device *phydev)
{
	if (0 != strncmp(ETH0, g_gmacdev->name,strlen(ETH0)))
	{
             return;
	}
	g_gmacdev = NULL;
...
static int macb_probe(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev, "tracing devm_platform_get_and_ioremap_resource %d.\n", IS_ERR(mem));
	if (IS_ERR(mem))
		return PTR_ERR(mem);
	...
	dev_err(&pdev->dev, "tracing clk_init %d.\n", err);
	if (err)
		return err;
	...
	////"11:10 Takes the value of the `gem_dma_bus_width_def DEFINE" 
	//u32 reg_cfg5  = gem_readl(bp, DCFG5);
	////dma_addr_bus_width_1
	//u32 reg_d1 = macb_readl(bp, DMA_CONFIG);
	//// "31:28 Takes the value of the `gem_emac_bus_width  DEFINE.
	//u32 reg_cfg10  = gem_readl(bp, DCFG10);
	//pr_err("dma addr width reg_cfg5 %x DMA_CONFIG %x , DCFG10 %x \n",reg_cfg5, reg_d1, reg_cfg10);
	//macb_writel(bp, DMA_CONFIG,reg_d1 & MACB_DMA_ADDR_WD_32B);
	////reg_d1 = macb_readl(bp, DMA_CONFIG);
	////pr_err("after set dma addr width , DMA_CONFIG %x \n", reg_d1);
	...
	dev_err(&pdev->dev, "tracing macb probe call  platform_get_irq %d.\n", dev->irq);
	if (dev->irq < 0) {
		err = dev->irq;
	...
	/* IP specific init */
	err = init(pdev);
	dev_err(&pdev->dev, "tracing macb probe call  init %d.\n", err);
	if (err)
		goto err_out_free_netdev;
	err = macb_mii_init(bp);
	dev_err(&pdev->dev, "tracing macb_mii_init %d.\n", err);
	if (err)
		goto err_out_free_netdev;
...
#if TEST_AUTO_UDP_SEND_BY_SYS
     if (0 == strncmp(ETH0, dev->name,strlen(ETH0)))
     {
         g_gmacdev = dev;
         gmac_sys_init();
     }
...
static int macb_remove(struct platform_device *pdev)
{
	...
	if (dev) {
		bp = netdev_priv(dev);
#if TEST_CLK
		clk_unregister(bp->pclk);
	        clk_unregister(bp->hclk);
#endif
		mdiobus_unregister(bp->mii_bus);
		mdiobus_free(bp->mii_bus);
