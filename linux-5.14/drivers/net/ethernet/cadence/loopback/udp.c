...
{
  unsigned char* data;
  int eth_data_total_len = data_len;
  char * str = "AA5BB";
  const int len = strlen(str);
  int loop = eth_data_total_len/len;
  int index = 0;
/* skb */
  // push move data, put move tail,  data, udp,ip,ether
  struct sk_buff* skb = alloc_skb(ETH_HLEN+eth_data_total_len, GFP_ATOMIC);//allocate a network buffer
  skb->dev = dev;
  skb->pkt_type = PACKET_OUTGOING;
  skb_reserve(skb, ETH_HLEN);//adjust headroom
/* allocate space to data and write it */
  data = skb_put(skb,eth_data_total_len);
