...
#define  TEST_REG_DP83867 0
#define  TEST_AUTO_GENE 0
enum {
...
struct dp83867_private {
  ...
  static int dp83867_sys_init(void);
  struct phy_device *g_phydev = NULL;
  #define ETH0 "43080000.ethernet"
#endif
...
static int dp83867_read_status(struct phy_device *phydev)
{
	...
	int ret;
        //int rgmii_ctl = phy_read_mmd(phydev, DP83867_DEVADDR, DP83867_RGMIICTL);
        //pr_err("tracing phy_read DP83867__PHYSTS %x \n", status);
...
static int dp83867_of_init(struct phy_device *phydev)
{
	...
	pr_err("tracing dp83867_of_init not empty func !of_node = ? %d \n", !of_node);
	if (!of_node)
		return -ENODEV;
...
static int dp83867_of_init(struct phy_device *phydev)
{
	pr_err("tracing dp83867_of_init empty func \n");
	return 0;
}
...
static int dp83867_probe(struct phy_device *phydev)
{
	...
	dp83867 = devm_kzalloc(&phydev->mdio.dev, sizeof(*dp83867),
			       GFP_KERNEL);
	pr_err("tracing dp83867_probe\n");
	if (!dp83867)
		return -ENOMEM;

	phydev->priv = dp83867;
#if TEST_DP83867_SYS
        pr_err("mii_bus id %s",phydev->mdio.bus->id);
	// the first
	if (0 == strncmp(ETH0, phydev->mdio.bus->id,strlen(ETH0)))
        {
	    g_phydev = phydev;
	    dp83867_sys_init();
	}
...
static int dp83867_config_init(struct phy_device *phydev)
{
	...
	if (phy_interface_is_rgmii(phydev)) {
		...
		val = phy_read_mmd(phydev, DP83867_DEVADDR, DP83867_RGMIICTL);
	        ...
	        //pr_err("tracing phy_read_mmd DP83869_RGMIICT %x \n", val);
		val &= ~(DP83867_RGMII_TX_CLK_DELAY_EN | DP83867_RGMII_RX_CLK_DELAY_EN);
	//	if (phydev->interface == PHY_INTERFACE_MODE_RGMII_ID || phydev->interface == PHY_INTERFACE_MODE_RGMII)
		if (phydev->interface == PHY_INTERFACE_MODE_RGMII_ID)
			val |= (DP83867_RGMII_TX_CLK_DELAY_EN | DP83867_RGMII_RX_CLK_DELAY_EN);
...
#else
		phy_write_mmd(phydev, DP83867_DEVADDR, DP83867_RGMIICTL, 0x00d3);
#endif
		delay = 0;
...
#if TEST_DP83867_PHY
		phy_write_mmd(phydev, DP83867_DEVADDR, DP83867_RGMIIDCTL,0x0077);
#else
...
#if TEST_REG_DP83867 
	int val_r2  = phy_read(phydev, DP83867_CFG3);
	pr_err("tracing read DP83867_CFG3 reg %x , DP83867_CFG3_ROBUST_AUTO_MDIX is setted ? %lu \n", val_r2 , val_r2 & DP83867_CFG3_ROBUST_AUTO_MDIX);
	if (val_r2 & DP83867_CFG3_ROBUST_AUTO_MDIX){
	     phy_write(phydev, DP83867_CFG3, val_r2 & ~DP83867_CFG3_ROBUST_AUTO_MDIX);
	     val_r2  = phy_read(phydev, DP83867_CFG3);
	     pr_err("tracing after clear AUTO_MDIX , read DP83867_CFG3 reg %x , DP83867_CFG3_ROBUST_AUTO_MDIX is setted ? %lu \n", val_r2 , val_r2 & DP83867_CFG3_ROBUST_AUTO_MDIX);
	}
	else {
	     pr_err("tracing read DP83867_CFG3 reg  AUTO_MDIX is not initially enable  ");
	}
#endif
	val |= DP83867_CFG3_ROBUST_AUTO_MDIX;
	phy_write(phydev, DP83867_CFG3, val);
#if TEST_REG_DP83867 
	int val_r  = phy_read(phydev, DP83867_CFG3);
	pr_err("tracing read DP83867_CFG3 reg %x , DP83867_CFG3_ROBUST_AUTO_MDIX is setted ? %lu \n", val_r , val_r & DP83867_CFG3_ROBUST_AUTO_MDIX);
...
#if TEST_AUTO_GENE
	u32 auto_g  = phy_read(phydev, DP83867_BISCR);
	pr_err("auto BISCR reg %x \n", auto_g);
	phy_write(phydev, DP83867_BISCR, 0x0020);
	//phy_write(phydev, DP83867_BISCR,  auto_g | 0x7020);
	//auto_g  = phy_read(phydev, DP83867_BISCR);
	//pr_err("after set , BISCR reg %x \n", auto_g);
...
#if TEST_DP83867_SYS
static int dp83867_value;
static ssize_t dp83867_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
   //sprintf(buf, "%d\n", hello_value);
   ...
   if (!g_phydev)
	return 0;
   u32 reg  = phy_read(g_phydev,0x00);
   pr_err("BMCR %x \n",reg);
   reg  = phy_read(g_phydev,0x01);
   pr_err("BMSR %x \n",reg);
   reg = phy_read_mmd(g_phydev, DP83867_DEVADDR, DP83867_RGMIICTL);
   pr_err("DP83867_RGMIICTL %x \n",reg);
   reg = phy_read_mmd(g_phydev, DP83867_DEVADDR, DP83867_RGMIIDCTL);
   pr_err("DP83867_RGMIIDCTL %x \n",reg);
   reg = phy_read(g_phydev, MII_DP83867_PHYSTS);
   pr_err("MII_DP83867_PHYSTS %x \n",reg);
   reg = phy_read(g_phydev, DP83867_BISCR);
   pr_err("DP83867_BISCR %x \n",reg);
   pr_err("phy state %d \n",g_phydev->state);
   return 0;
}
static ssize_t dp83867_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    u32 reg_addr, reg_value;
    sscanf(buf, "%x,%x", &reg_addr, &reg_value);
    pr_err("reg add %x , reg value %x\n",reg_addr, reg_value);
    if(DP83867_RGMIIDCTL == reg_addr || DP83867_RGMIICTL == reg_addr)
    {
	phy_write_mmd(g_phydev, DP83867_DEVADDR, reg_addr, reg_value);
    }
    else if (0x00 ==  reg_addr || DP83867_BISCR == reg_addr)
    {
        phy_write(g_phydev,reg_addr, reg_value);
    }
    return count;

}
static struct kobj_attribute dp83867_attribute = __ATTR(dp83867_value, 0664, dp83867_show, dp83867_store);
static struct kobject *dp83867_kobj = NULL;
static int dp83867_sys_init(void)
{
    //struct net_device *ndev = phydev->attached_dev
    int retval;
    dp83867_kobj = kobject_create_and_add("dp83867", kernel_kobj);
    if (!dp83867_kobj)
        return -ENOMEM;
    retval = sysfs_create_file(dp83867_kobj, &dp83867_attribute.attr);
    if (retval)
