...
#define TEST_MARVEL_SYS 1
#define TEST_MARVELL_PHY 1
#if TEST_MARVEL_SYS
/* 88E151x PHY defines */
/* Page 2 registers */
...
#define MIIM_88E151x_RGMII_RXTX_DELAY	(BIT(5) | BIT(4))
  ...
  static int marvel_sys_init(void);
	    ...
	    if (phy_interface_is_rgmii(phydev)) {
		    ...
		    u16 reg;
		          phy_write(phydev, MII_MARVELL_PHY_PAGE, 2);
			            reg = phy_read(phydev,  MIIM_88E151x_PHY_MSCR);
				    ...
				    if (phydev->interface == PHY_INTERFACE_MODE_RGMII ||
				           ...
				           reg |= MIIM_88E151x_RGMII_TX_DELAY;
				    phy_write(phydev, MIIM_88E151x_PHY_MSCR, reg);
...
#else
	if (phy_interface_is_rgmii(phydev)) {
        pr_info("*********tracing  %s m88e1121_config_aneg_rgmii_delays\n", __func__);
...
error:
 ...
 static int test_m88e1121_config_aneg(struct phy_device *phydev)
{
 ...
 static int test_m88e1318_config_aneg(struct phy_device *phydev)
{
    int err, oldpage, mscr;
    ...
    return test_m88e1121_config_aneg(phydev);
}
//#if TEST_MARVELL_PHY 
#if 0
...
static int test_marvell_config_aneg_fiber(struct phy_device *phydev)
{
	int changed = 0;
	int err;
	int adv, oldadv;
...
static int m88e1510_config_aneg(struct phy_device *phydev)
{
	int err;
        ...
        pr_err( "%s() %s speed %i %s supported 0x%x\n",
            __func__,
	...
	err = test_marvell_config_aneg_fiber(phydev);
	if (err < 0)
		goto error;
...
#if 0
//#if TEST_MARVELL_PHY
	err = test_m88e1318_config_aneg(phydev);
	return err;
#else
	err = marvell_set_page(phydev, MII_MARVELL_COPPER_PAGE);
	if (err < 0)
...
static int m88e1111_config_init(struct phy_device *phydev)
{
	...
	if (phy_interface_is_rgmii(phydev)) {
		err = m88e1111_config_init_rgmii(phydev);
                pr_err(" tracing m88e1111_config_init_rgmii %d ",err );
		if (err < 0)
			return err;
	}
	...
	err = marvell_of_reg_init(phydev);
        pr_err(" tracing marvell_of_reg_init %d ",err );
	if (err < 0)
		return err;
...
#if 0
//#if TEST_MARVELL_PHY
static int test_m88e1121_config_init(struct phy_device *phydev)
{
	u16 reg;
	int ret;
...
static int m88e1510_config_init(struct phy_device *phydev)
{
	...
	return test_m88e1121_config_init(phydev);
}
#else
static int test2_m88e1510_config_init(struct phy_device *phydev)
{
	static const struct {
		u16 reg17, reg16;
	} errata_vals[] = {
...
static int m88e1510_config_init(struct phy_device *phydev)
{
	...
	pr_err("************* m88e1510_config_init begin \n");
        return test2_m88e1510_config_init(phydev);
#if TEST_MARVELL_PHY
//#if 0
             //int old; 
	     //old =  phy_select_page(phydev, 0x00ff);
		...
		if (err < 0)
		{
	            pr_err("************* m88e1510_config_init reg fail \n");
	      ...
	      marvell_set_page(phydev,MII_MARVELL_COPPER_PAGE);
	     pr_err("************* m88e1510_config_init reg succ \n");
#endif
	/* SGMII-to-Copper mode initialization */
	if (phydev->interface == PHY_INTERFACE_MODE_SGMII) {
		/* Select page 18 */
...
//#if TEST_MARVELL_PHY
              ...
              //phy_write(phydev,  MIIM_88E1118_PHY_PAGE, 0x00ff);
              //phy_write(phydev,  MII_MARVELL_PHY_PAGE , 0x00ff);
	      //phy_write(phydev,  17, 0x214B);
	      //phy_write(phydev,  16, 0x2144);
              //phy_write(phydev,  17, 0x0C28);
	      //phy_write(phydev,  16, 0x2146);
	      ...
	      //phy_write(phydev,  17, 0xCC0C);
	      //phy_write(phydev,  16, 0x2159);
	      ////phy_write(phydev,  MIIM_88E1118_PHY_PAGE, 0x0000);
	      //phy_write(phydev,  MII_MARVELL_PHY_PAGE , 0x0000);


	    if (phy_interface_is_rgmii(phydev)) {
	      pr_err("************* m88e1510_config_init rgmii \n");
		    u16 reg;
		          phy_write(phydev, MII_MARVELL_PHY_PAGE, 2);
			            reg = phy_read(phydev,  MIIM_88E151x_PHY_MSCR);
				    //reg &= ~MIIM_88E151x_RGMII_RXTX_DELAY;
...
static int marvell_hwmon_probe(struct phy_device *phydev)
{
	...
	int err;
        ...
        printk(KERN_ALERT "********** tracing marvell_hwmon_probe \n");
	if (!ops)
		return 0;
...
static int marvell_hwmon_probe(struct phy_device *phydev)
{
        printk(KERN_ALERT "tracing marvell_hwmon_probe return 0\n");
	return 0;
}
#endif

#if TEST_MARVEL_SYS
static int marvel_value;
static ssize_t marvel_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
#if 0
   if (!g_phydev)
	return 0;
...
static ssize_t marvel_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    ...
    else if (3 == read)
    {
       ...
       genphy_soft_reset(g_phydev);
    }
    //phy_write(g_phydev,reg_addr, reg_value);
    //phy_write_paged(g_phydev,
 #if 0
	return phy_modify_paged(phydev, MII_MARVELL_MISC_TEST_PAGE,
				MII_88E6390_TEMP_SENSOR,
				MII_88E6393_TEMP_SENSOR_THRESHOLD_MASK,
...
static struct kobj_attribute marvel_attribute = __ATTR(marvel_value, 0664, marvel_show, marvel_store);
static struct kobject *marvel_kobj = NULL;
static int marvel_sys_init(void)
{
    //struct net_device *ndev = phydev->attached_dev
    int retval;
    marvel_kobj = kobject_create_and_add("marvel", kernel_kobj);
    if (!marvel_kobj)
        return -ENOMEM;
    retval = sysfs_create_file(marvel_kobj, &marvel_attribute.attr);
    if (retval)
        kobject_put(marvel_kobj);
   return retval;
}
static void marvel_remove(struct phy_device *phydev)
{
