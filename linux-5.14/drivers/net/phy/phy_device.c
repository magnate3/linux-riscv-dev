...
#define TEST_GET_PHYID 0
#define TEST_PHY_FEATURE 0
...
static int phy_request_driver_module(struct phy_device *dev, u32 phy_id)
{
	 ...
	 */
	phydev_err(dev, "ret %d loading PHY driver module for ID 0x%08lx\n",
				     ...
				     struct phy_c45_device_ids *c45_ids)
{
	...
	if (is_c45 && c45_ids) {
        ...
        pr_err("tracing phy_request_driver_module ret %d", ret);
...
static int get_phy_c22_id(struct mii_bus *bus, int addr, u32 *phy_id)
{
	...
	phy_reg = mdiobus_read(bus, addr, MII_PHYSID1);
        pr_err("tracing get_phy_c22_id %s mdiobus_read MII_PHYSID1 %d,  addr %d \n", bus->name, phy_reg,  addr);
	if (phy_reg < 0) {
	...
	*phy_id = phy_reg << 16;
	...
	phy_reg = mdiobus_read(bus, addr, MII_PHYSID2);
        pr_err("tracing get_phy_c22_id %s mdiobus_read MII_PHYSID2 %d,  addr %d \n", bus->name, phy_reg,  addr);
	if (phy_reg < 0) {
		/* returning -ENODEV doesn't stop bus scanning */
		return (phy_reg == -EIO || phy_reg == -ENODEV) ? -ENODEV : -EIO;
	}
...
struct phy_device *get_phy_device(struct mii_bus *bus, int addr, bool is_c45)
{
	...
	if (is_c45)
        ...
        pr_err("in get_phy_device tracing get_phy_cXX %d  is_c45 %d and phy_id  %x\n",r, is_c45, phy_id);
	if (r)
		return ERR_PTR(r);
...
int phy_init_hw(struct phy_device *phydev)
{
	...
	if (phydev->drv->config_init) {
		ret = phydev->drv->config_init(phydev);
	        pr_err("tracing drv->config_init %d \n", ret);
		if (ret < 0)
			return ret;
	}
	...
	if (phydev->drv->config_intr) {
		ret = phydev->drv->config_intr(phydev);
	        pr_err("tracing drv->config_intr %d \n", ret);
		if (ret < 0)
			return ret;
	}
		      ...
		      u32 flags, phy_interface_t interface)
{
	...
	phydev_err(phydev, "tracing phy_init_hw %d \n", err);
	if (err)
		goto error;

	err = phy_disable_interrupts(phydev);
	phydev_err(phydev, "tracing phy_disable_interrupts %d \n", err);
	if (err)
		return err;
...
{
	u16 ctl = 0;
        pr_err("genphy_setup_forced speed %d \n", phydev->speed);
	phydev->pause = 0;
	phydev->asym_pause = 0;
...
int genphy_update_link(struct phy_device *phydev)
{
...
#if TEST_GET_PHYID
	 u32 phy_id = 0;  
	 get_phy_c22_id(phydev->mdio.bus, phydev->mdio.addr, &phy_id);
	 pr_err(" tracing phydev addr    %d and phy_id  %x \n", phydev->mdio.addr, phy_id);
#endif
	bmcr = phy_read(phydev, MII_BMCR);
        //pr_err("tracing phy_read  MII_BMCR  %x , bmcr < 0 ?  %d,  bmcr & BMCR_ANRESTART = %d  \n",  bmcr, bmcr < 0 , bmcr & BMCR_ANRESTART);
	if (bmcr < 0)
		return bmcr;
	...
	if (!phy_polling_mode(phydev) || !phydev->link) {
		status = phy_read(phydev, MII_BMSR);
	        //pr_err("tracing phy_read  MII_BMSR  %x %d \n",  status, __LINE__);
		if (status < 0)
			return status;
...
int genphy_read_status_fixed(struct phy_device *phydev)
{
	...
	if (bmcr & BMCR_SPEED1000)
        ...
        //pr_err("tracing genphy_read_status_fixed speed %d \n",phydev->speed);
	return 0;
}
...
int genphy_soft_reset(struct phy_device *phydev)
{
	...
	ret = phy_modify(phydev, MII_BMCR, BMCR_ISOLATE, res);
        pr_err("tracing call genphy_soft_reset  phy_modify %d", ret);
	if (ret < 0)
		return ret;
	...
	ret = phy_poll_reset(phydev);
        pr_err("tracing call  phy_poll_reset %d", ret);
	if (ret)
		return ret;
	...
	if (phydev->autoneg == AUTONEG_DISABLE)
        ...
        pr_err("tracing call  genphy_setup_forced %d", ret);
	return ret;
}
...
int genphy_read_abilities(struct phy_device *phydev)
{
	...
	if (val < 0)
		return val;
#if TEST_PHY_FEATURE
	//phy_write(phydev, MII_BMCR, 0X2140);
	//int val2 = phy_read(phydev, MII_BMCR);
	//pr_err("tracing genphy_read_abilities MII_BMSR  %x and MII_BMCR %x \n",val, val2);
	phydev->speed = SPEED_100;
	phydev->duplex = DUPLEX_FULL;
	val &= ~BMSR_ANEGCAPABLE; 
        //phy_write(phydev, MII_BMSR, val);
	//val &= ~BMSR_10FULL;
	//val &= ~BMSR_10HALF;
	//val |= BMSR_100FULL;
#endif
...
static int phy_probe(struct device *dev)
{
	...
	pr_err("tracing phydev->features %d", err);
