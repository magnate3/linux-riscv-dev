...
#if TEST_CLK_SYS
//static ssize_t sp804_clocksource_store(struct device *dev,
//					 struct device_attribute *attr,
//					 const char *buf, size_t count)
//static ssize_t sp804_clocksource_show(struct device *dev,
//					  struct device_attribute *attr,
//					  char *buf)
static ssize_t sp804_clocksource_show(struct kobject *kobj ,
					  ...
					  char *buf)
{
	...
	struct clocksource  *cs;
	list_for_each_entry(cs, &clocksource_list, list) {
		if (strcmp(cs->name, "arm,sp804") == 0)
		{
			pr_err("sp804_read: %x ", cs->read(cs));
		}
	}
	return 0;
}
static int sp804_value;
//static ssize_t sp804_clocksource_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf);
static struct kobj_attribute sp804_attribute = __ATTR(sp804_value, 0664, sp804_clocksource_show, NULL);
static struct kobject *sp804_kobj = NULL;
static int sp804_sys_init(void)
{
    int retval;
    sp804_kobj = kobject_create_and_add("sp804", kernel_kobj);
    if (!sp804_kobj)
         return -ENOMEM;
