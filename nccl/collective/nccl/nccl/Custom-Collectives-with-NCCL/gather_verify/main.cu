...
int main(int argc, char** argv){
    ...
    double *B_testr = (double*)malloc(N * size * sizeof(double));
    double *B_verr = (double*)malloc(N * size * sizeof(double));
    ...
    cudaErrorCheck(cudaMalloc(&d_Br, N * sizeof(double) * size));
    cudaErrorCheck(cudaMemcpy(d_Ar, Ar, N * sizeof(double), cudaMemcpyHostToDevice));


    NCCLCHECK(gather_custom3(d_Ar, d_Br, N, ncclDouble, 0, comm, s));

    MPI_Gather(Ar, N, MPI_DOUBLE, B_verr, N, MPI_DOUBLE, 0, MPI_COMM_WORLD);


    // cudaErrorCheck(cudaMemcpy(A, d_A, N * sizeof(double), cudaMemcpyDeviceToHost));
    cudaErrorCheck(cudaMemcpy(B_testr, d_Br, N * size * sizeof(double), cudaMemcpyDeviceToHost));
    if (rank == 0) {
        bool func_corr = true;
