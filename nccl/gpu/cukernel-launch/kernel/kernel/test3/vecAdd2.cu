...
//void invoke_VecAdd(double* d_A, double* d_B, double* d_C, int N) {
extern "C" __global__ void invoke_VecAdd(double* d_A, double* d_B, double* d_C, int N) {
    int threadsPerBlock = 256;
	int blocksPerGrid = N / threadsPerBlock;
    VecAdd2<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_B, d_C, N);
};
#endif
