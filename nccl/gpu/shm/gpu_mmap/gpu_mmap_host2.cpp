...
int verify_memory(int dev_idx, CUdeviceptr d_ptr, int size){
    ...
    uint8_t* host_orig = (uint8_t*)g_addr;
  ...
  for (int j = 0; j < size; j++) {
    if (host_orig[j] != host_mem[j]) {
          printf("Process %d: Verification mismatch at %d: %d != %d\n", getpid(), j, host_orig[j], (uint8_t)host_mem[j]);
      break;
    }
  }
    delete[] host_mem;
...
static int child_main(struct sockaddr_un *addr)
{
	...
	verify_memory(cuda_dindex,cuda_buffer,buffer_size);
	return 0;
}
int unmmap_alloc(void *addr, size_t size)
{
...
#if 1
	cudaError_t err;
        g_addr = map_alloc(buffer_size);
	init_device(0);
	cudaHostRegister(g_addr, buffer_size, cudaHostRegisterMapped);
        err=cudaHostGetDevicePointer((void **)&cuda_buffer, (void *)g_addr, 0);
        if (err != cudaSuccess)
        {
        }
        fprintf(stderr, "cuda_buffer %p =? %p\n", (void *)cuda_buffer, g_addr);
        {
             cudaPointerAttributes attr;
             cudaError_t rc = cudaPointerGetAttributes(&attr, (void *)g_addr);
             if (rc != cudaSuccess)
             {
                 fprintf(stderr, "fail\n");
             }
             fprintf(stderr, "prop[%p]: dev %u, device ptr %p, host ptr %p\n", g_addr, attr.device, attr.devicePointer, attr.hostPointer);
        }
#endif
	rc = cuInit(0);
	if (rc != CUDA_SUCCESS)
...
int main(int argc, char *argv[])
{
	...
	unlink(addr.sun_path);
        unmmap_alloc(g_addr,buffer_size);
	return 0;
}
