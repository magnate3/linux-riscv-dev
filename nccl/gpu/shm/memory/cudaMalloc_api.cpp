...
int main() {
    ...
    // ------------------------- 测试 cudamalloc-api ---------------------------------------------------------------------
    size_t mem_size =  80624ULL * 1024 * 1024;  // 使用ULL确保计算不会发生溢出

    void* ptr;
    while (true) {
        mem_size = mem_size + 1024 * 1024;  // 增加分配粒度
        cudaError_t err = cudaMalloc(&ptr, mem_size);
        if (err == cudaSuccess) {
            cudaError_t err_free = cudaFree(ptr);
            if (err_free != cudaSuccess) {
                std::cerr << "释放显存出错 CUDA error: " << cudaGetErrorString(err_free) << std::endl;
                return -1;
            }
        }
        if (err != cudaSuccess) {
            ...
            std::cout << "最大分配内存: " << mem_size / 1024 / 1024 << " MB." << "  /  " <<  mem_size / 1024 / 1024 / 1024 << " GB." << std::endl;
            return -1;
        }
    }
    // ------------------------- 测试 cudamalloc-api ---------------------------------------------------------------------
