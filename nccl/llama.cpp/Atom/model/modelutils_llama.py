...
def reorder_model_llama(model, device, args, reorder_index):
    ...
    for i in tqdm(range(len(layers))):
        ...
        if isinstance(layers[i], (LlamaDecoderLayer, MistralDecoderLayer)):
            ...
            # 如果未来有其他变体，直接作为通用 Layer 处理
            m = QLlamaDecoderLayer(originalLayer=layers[i], args=args)
    ...
    for i in tqdm(range(len(layers))):
        ...
        for names in sequential:
            ...
            for j in range(args.nsamples):
                ...
                layer(inps[j].unsqueeze(0), attention_mask=current_attention_mask, position_ids=position_ids, position_embeddings=position_embeddings)
            for h in handles:
                h.remove()
        ...
        for j in range(args.nsamples):
            ...
            if current_attention_mask is None:
                ...
                current_attention_mask = (1.0 - current_attention_mask) * torch.finfo(dtype).min
            position_embeddings = model.model.rotary_emb(inps[j].unsqueeze(0), position_ids)
            outs[j] = layer(inps[j].unsqueeze(0), attention_mask=current_attention_mask, position_ids=position_ids, position_embeddings=position_embeddings)
