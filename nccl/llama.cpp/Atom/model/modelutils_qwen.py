...
from qQwenLayer import QQwen3DecoderLayer  # 假设你有对应的Qwen量化层
...
def reorder_model_qwen(model, device, args, reorder_index):
    ...
    for i in tqdm(range(len(layers))):
        ...
        # Qwen3 MLP结构：gate_proj, up_proj, down_proj
        ...
        # Qwen3特有的Q/K RMSNorm - 注意需要处理这些额外的归一化层
        ...
        # Qwen3特有的Q/K归一化层
        # if hasattr(m.self_attn, 'q_norm'):
        #     m.self_attn.q_norm.register_buffer('reorder_index', 
        #         reorder_index[nameTemplate.format(i, 'self_attn', 'q_proj', 'input')]
        #     )
        # if hasattr(m.self_attn, 'k_norm'):
        #     m.self_attn.k_norm.register_buffer('reorder_index', 
        #         reorder_index[nameTemplate.format(i, 'self_attn', 'k_proj', 'input')]
        #     )
...
def add_act_quant_wrapper_qwen(model, device, args, scales):
    model.config.use_cache = False
    layers = model.model.layers
    for i in tqdm(range(len(layers))):
        m = None
        ...
        # Qwen3特有的Q/K归一化层量化配置
        # if hasattr(m.self_attn, 'q_norm_act_quant'):
        #     m.self_attn.q_norm_act_quant.configure(
        #         partial(quantize_activation_wrapper, args=args),
        #         scales[nameTemplate.format(i, 'self_attn', 'q_proj')]
        #     )
        # if hasattr(m.self_attn, 'k_norm_act_quant'):
        #     m.self_attn.k_norm_act_quant.configure(
        #         partial(quantize_activation_wrapper, args=args),
...
def quantize_model_qwen(model, device, args):
    model.config.use_cache = False
    layers = model.model.layers
    for i in tqdm(range(len(layers))):
        m = None
...
def quantize_model_gptq_qwen(model, device, args, dataloader):
    print('Starting GPTQ quantization for Qwen3 ...')
    ...
    # 在quantize_model_gptq_qwen函数中也使用这个Catcher
    ...
    # 检查是否成功捕获了样本
    if cache['i'] == 0:
        print("WARNING: No samples captured! Check dataloader and model forward pass.")
        # 使用虚拟数据继续
        if inps.shape[0] > 0:
            inps = inps[:1]  # 只保留一个样本
        else:
            # 创建虚拟输入
            inps = torch.randn(1, model.seqlen, model.config.hidden_size, dtype=dtype, device=device)
    ...
    for i in tqdm(range(len(layers))):
        ...
        for names in sequential:
            ...
            for j in range(args.nsamples):
                if j >= cache['i']:  # 如果捕获的样本不足，跳过
                    continue
                    
                current_attention_mask = attention_mask
                # if current_attention_mask is None:
                seq_len = inps[j].unsqueeze(0).shape[1]
                current_attention_mask = torch.tril(torch.ones(1, 1, seq_len, seq_len, device=device))
        ...
        # 前向传播更新输出
        for j in range(args.nsamples):
            if j >= cache['i']:  # 如果捕获的样本不足，使用最后一个样本
                j = min(j, cache['i'] - 1)
