...
@torch.no_grad()
def get_act_stats_qwen(model, dataloader, device_, metric='hessian'):
    nsamples = len(dataloader)
    device = device_
    act_scales = {}
    ...
    for i in tqdm(range(len(layers))):
        ...
        for j in range(nsamples):
            ...
            # 对于Qwen3，我们不需要手动处理滑动窗口掩码
            # 因为模型内部会根据attention_type自动选择正确的掩码
            # 我们只需要传递正确的attention_mask即可
            seq_len = inps[j].unsqueeze(0).shape[1]
            current_attention_mask = torch.tril(torch.ones(1, 1, seq_len, seq_len, device=device, dtype=inps[j].dtype))
...
def get_act_scales(model, dataloader, device_, args):
    ...
    for i in tqdm(range(len(layers))):
        ...
        for j in range(nsamples):
            ...
            current_attention_mask = torch.tril(torch.ones(1, 1, seq_len, seq_len, device=device, dtype=inps[j].dtype))
            current_attention_mask = (1.0 - current_attention_mask) * torch.finfo(dtype).min
            outs[j] = layer(inps[j].unsqueeze(0), attention_mask=current_attention_mask, position_ids=position_ids)
        layers[i] = layer.cpu()
        del layer
        inps, outs = outs, inps
