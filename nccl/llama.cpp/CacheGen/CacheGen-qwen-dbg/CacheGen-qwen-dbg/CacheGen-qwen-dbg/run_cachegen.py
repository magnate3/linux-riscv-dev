...
import sys
# 1. 尝试从新路径导入 GenerationMixin
try:
    from transformers.generation.utils import GenerationMixin
except ImportError:
    from transformers.generation import GenerationMixin

# 2. 动态将其注入并挂载到 transformers.generation 命名空间中
...
# 1. 动态或手动定义 Qwen2.5-0.5B 的底层结构参数

num_layers = 24
num_kv_heads = 2
head_size = 64
chunk_size = 512 # 默认的 LMCache 块大小，可以根据你的 cachegen/lmcache 配置文件核对
#chunk_size = 256  # 默认的 LMCache 块大小，可以根据你的 cachegen/lmcache 配置文件核对
...
if __name__ == "__main__":
    ...
    for doc_id in range(args.start, args.end):
        ...
        meta_data = LMCacheEngineMetadata(model_name=args.model_id, fmt="huggingface", world_size=1, worker_id=0,kv_shape=kv_shape)
    ...
    for doc_id in range(args.start, args.end):
        ...
        if isinstance(decoded_kv, tuple):
            new_past_key_values = []
            for layer in decoded_kv:
                k, v = layer[0], layer[1]
                new_past_key_values.append((k.to(torch.bfloat16), v.to(torch.bfloat16)))
            decoded_kv = tuple(new_past_key_values)
