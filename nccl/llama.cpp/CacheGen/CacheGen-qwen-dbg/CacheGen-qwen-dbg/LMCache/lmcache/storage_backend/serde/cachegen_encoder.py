...
def encode_function(
    ...
    # ⏳ 结束计时并强行同步 GPU 核心
    cdf_calc_end.record()
    torch.cuda.synchronize()  # 💡 极其重要：强制等待 CUDA 内部的原子加法和累积求和彻底流式结束
    
    # 计算当前块生成的纯算子耗时（毫秒）
    cdf_time_ms = cdf_calc_start.elapsed_time(cdf_calc_end)
    
    print(f"   ├─ [纯算子监控] 动态构建 Key & Value CDF 耗时: {cdf_time_ms:.3f} 毫秒 (ms)")
...
class CacheGenSerializer(Serializer):
    ...
    def to_bytes(
        ...
        # 1. 🛑 在函数最开始：初始化 GPU 高精度计时事件
        start_event = torch.cuda.Event(enable_timing=True)
        end_event = torch.cuda.Event(enable_timing=True)
        
        # 2. ⏳ 启动计时：包裹住最核心的 GPU 量化与算术编码算子
        start_event.record()
        ...
        # 3. ⏳ 结束计时并强行同步 GPU 核心
        end_event.record()
        torch.cuda.synchronize()  # 💡 极其重要：强制等待 GPU 编码位移操作全部完成
        
        # 4. 计算耗时（单位：毫秒）
        elapsed_time_ms = start_event.elapsed_time(end_event)
        
        # 动态获取当前处理的 Token 长度用于打印
        #ntokens = key.shape[1] if hasattr(key, 'shape') else "Unknown"
        print("-" * 55)
        print(f"⚡ [CacheGen 算术编码成功]")
        print(f"   ├─ 处理 Token 长度 (Sequence Length): {ntokens}")
        print(f"   └─ GPU 算术加密 + 差分量化总耗时: {elapsed_time_ms:.3f} 毫秒 (ms)")
        print("-" * 55)
