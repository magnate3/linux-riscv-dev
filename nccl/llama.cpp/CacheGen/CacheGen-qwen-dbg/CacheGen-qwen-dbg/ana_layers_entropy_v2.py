...
for idx in range(num_layers):
    ...
    # 论文精髓：采样多个核心通道（或遍历全部 128 个通道）来代表这一层的宏观属性
    # 为了 HP 工作站的运行效率，我们每隔 4 个通道采样一个，拉通评估整体
    for ch_idx in range(0, num_channels, 4):
        # 提取当前层、当前通道
        slice_B_raw = kv_matrix[idx, :, :, ch_idx]
        slice_C_delta = delta_kv_matrix[idx, :, :, ch_idx]
        
        layer_raw_list.append(calculate_shannon_entropy(slice_B_raw))
        layer_delta_list.append(calculate_shannon_entropy(slice_C_delta))
        
    # 计算整层的平均熵与平均差分收益
    mean_raw_ent = np.mean(layer_raw_list)
    mean_delta_ent = np.mean(layer_delta_list)
    mean_gain = mean_raw_ent - mean_delta_ent
    
    layers_raw_entropy.append(mean_raw_ent)
    layers_delta_entropy.append(mean_delta_ent)
    layers_entropy_gain.append(mean_gain)
    
    # 实时打印多通道平均后的稳健进度
    if idx % 4 == 0 or idx == num_layers - 1:
        print(f"  [Layer {idx:02d}] 宏观平均原始熵: {mean_raw_ent:.3f} bits | 差分熵: {mean_delta_ent:.3f} bits | 稳健收益: {mean_gain:.3f} bits")
