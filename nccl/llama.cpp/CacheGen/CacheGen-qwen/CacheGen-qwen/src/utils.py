...
def define_model_and_tokenizer(model_id, num_gpus=1, max_gpu_memory=48):
    ...
    if model_id == "Yukang/LongAlpaca-70B-16k":
        ...
        # ⬇️ 【修改此处】：放弃不兼容新模型的 FastChat，改为原生的 bfloat16 / float16 加载
        print(f"!!!! use native Transformers load: {model_id}")
        ...
        model = AutoModelForCausalLM.from_pretrained(
            model_id,
            torch_dtype=torch.bfloat16,  # Qwen3 推荐使用 bfloat16。如果显卡较旧，可改为 float16
            device_map="auto",           # 让 Transformers 自动管理多卡切分
            low_cpu_mem_usage=True,
            trust_remote_code=True
        )
