...
int main() {
    ...
    ggml_backend_load_all_from_path("/workspace/llama.cpp/backend/");
    //ggml_backend_load_all_from_path("/workspace/llama.cpp/build/bin");
    ggml_backend_load_all();
    auto * cpu_dev                             = ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_CPU);
    if (cpu_dev == nullptr) {
	fprintf(stderr, "No cpu device found\n");
        return -1;
    }
