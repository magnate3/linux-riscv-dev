...
int main(int argc, char ** argv) {
	...
	n_keep = 32;
        ...
        llama_batch batch = llama_batch_get_one(prompt_tokens.data(), n_past);
#else
	//printf("prompt is %s \n",prompt.c_str());
        // prepare a batch for the prompt
	    ...
	    // Get max position for sequence 0 and add 1 to get token count
           ...
           int pos_max = llama_memory_seq_pos_max(llama_get_memory(ctx), 0);
           int token_count = (pos_max < 0) ? 0 : static_cast<int>(pos_max + 1);

           printf("KV cache token count: %d,  batch n_tokens %d \n", token_count, batch.n_tokens);
#endif
