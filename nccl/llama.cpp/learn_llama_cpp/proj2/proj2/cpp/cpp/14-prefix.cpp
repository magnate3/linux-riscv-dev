...
int main() {
    ...
    for (int i = 0; i < (int)tokens_user_a.size(); ++i) {
        ...
        // 位置从 n_prefix 开始
        llama_batch_add(batch, tokens_user_a[i], n_prefix + i, {0, user_a_slot}, i == (int)tokens_user_a.size()-1);
    }
    ...
    for (int i = 0; i < (int)tokens_user_b.size(); ++i) {
        ...
        llama_batch_add(batch, tokens_user_b[i], n_prefix + i, {0, user_b_slot}, i == (int)tokens_user_b.size()-1);
    }
