...
void llama_kv_cache::debug_set_input_k_idxs(ggml_tensor * dst, const llama_ubatch * ubatch, const slot_info & sinfo) const {
    const uint32_t n_tokens = ubatch->n_tokens;
    GGML_ASSERT(n_tokens == (int64_t) sinfo.size()*sinfo.n_stream());
	...
	std::cout<< "stream index " << s << " k data ";
        for (uint32_t i = 0; i < sinfo.size(); ++i) {
            //data[s*sinfo.size() + i] = offs + sinfo.idxs[s][i];
	    std::cout<< offs + sinfo.idxs[s][i] << "(" << sinfo.idxs[s][i] << ")" << " ";
        }
	std::cout<< std::endl;
    }
}

void llama_kv_cache::debug_set_input_v_idxs(ggml_tensor * dst, const llama_ubatch * ubatch, const slot_info & sinfo) const {
    const uint32_t n_tokens = ubatch->n_tokens;
    GGML_ASSERT(n_tokens == (int64_t) sinfo.size()*sinfo.n_stream());
    ...
    if (!v_trans) {
	    ...
	    std::cout<< "stream index " << s << " v data ";
            for (uint32_t i = 0; i < sinfo.size(); ++i) {
                //data[s*sinfo.size() + i] = offs + sinfo.idxs[s][i];
	        std::cout<< offs + sinfo.idxs[s][i]<< "(" << sinfo.idxs[s][i] << ")"  << " ";
            }
        ...
        for (uint32_t s = 0; s < sinfo.n_stream(); ++s) {
	        ...
	        std::cout<< "stream index " << s << " v data ";
                for (uint32_t j = 0; j < n_embd_v_gqa; ++j) {
                    //data[s*sinfo.size()*n_embd_v_gqa + i*n_embd_v_gqa + j] = offs + j*kv_size + sinfo.idxs[s][i];
	            std::cout<< offs + j*kv_size + sinfo.idxs[s][i] << "(" << sinfo.idxs[s][i] << ")"  << " ";
                }
	        std::cout<< std::endl;
            }
        }
