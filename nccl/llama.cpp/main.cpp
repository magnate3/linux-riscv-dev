...
struct mpt_layer {
    ...
    struct ggml_tensor * norm_1_bias;
    ...
    struct ggml_tensor * norm_2_bias;

    // ff
    struct ggml_tensor * ffn_up_proj_weight;
    struct ggml_tensor * ffn_down_proj_weight;

    struct ggml_tensor * ffn_up_proj_bias;
    struct ggml_tensor * ffn_down_proj_bias;
};

struct mpt_model {
    mpt_hparams hparams;

    struct ggml_tensor * wte_weight;    // token embedding
    struct ggml_tensor * wpe_weight;    // position embedding

    // language model head
    struct ggml_tensor * norm_f_weight; 
    struct ggml_tensor * norm_f_bias; 
...
bool mpt_model_load(const std::string & fname, mpt_model & model, gpt_vocab & vocab) {
    ...
    // load hparams
    {
        ...
        fin.read((char *) &hparams.qk_ln,          sizeof(hparams.qk_ln));
        ...
        printf("%s: qk_ln          = %s\n", __func__, hparams.qk_ln ? "true" : "false");
    ...
    // TODO: Properly quantify context size
    size_t ctx_size = 0;
...

    {
        ...
        ctx_size += n_layer * (ggml_row_size(wtype, 3 * n_embd * n_embd)); // attn_Wqkv_bias
        ctx_size += n_layer * (ggml_row_size(wtype,     n_embd * n_embd)); // attn_out_proj_weight
        ctx_size += n_layer * (ggml_row_size(wtype,     n_embd * n_embd)); // attn_out_proj_bias
...
bool mpt_eval(const mpt_model & model, const int n_threads, const int n_past,
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // n = self.mlp(m)
        {
            ...
            cur = ggml_mul_mat(ctx0, model.layers[il].ffn_up_proj_weight, cur);
            ...
            cur = ggml_mul_mat(ctx0, model.layers[il].ffn_down_proj_weight, cur);
        }
