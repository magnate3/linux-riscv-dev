...
static struct ggml_cgraph * llm_build_llama(struct ggml_context * ctx0, struct ggml_tensor *K, struct ggml_tensor *Q,struct ggml_tensor *V,float  KQ_scale,  const int n_past) {
    // ...
...
int main(void) {
	...
	struct ggml_tensor * embd = ggml_new_tensor_1d(ctx_cgraph, GGML_TYPE_I32, cols_A);
	struct ggml_tensor * rows = ggml_get_rows(ctx_cgraph, tensor_a, embd);
