...
namespace {
  ...
  __device__ __forceinline__ void runRing(int tid, int nthreads, struct ncclDevWorkColl* work) {
    ...
    for (ssize_t elemOffset = 0; elemOffset < channelCount; elemOffset += loopCount) {
      ...
      // printf("runRing %d TID %d, Line 46", ringIx, tid);
      if (ringIx == 0) {
        // introduce artificial delay of 0.56ms
      ...
      for (int j = 2; j < nranks; ++j) {
        ...
        offset = gridOffset + elemOffset + chunkOffset;
        nelem = (int)min(chunkCount, remCount - chunkOffset);
        // printf("runRing: TID %d: Receiving and reducing data at offset %zd, nelem %d\n", tid, offset, nelem);
        // printf("runRing %d TID %d, Line 61", ringIx, tid); // Happens only for nranks > 2
        if (ringIx == 0) {
            // introduce artificial delay of 0.56ms
            for (int i = 0; i < 10000; i++) { 
            printf("");
      ...
      offset = gridOffset + elemOffset + chunkOffset;
      nelem = (int)min(chunkCount, remCount - chunkOffset);
      // printf("runRing: TID %d: Directly receiving, reducing, copying, and sending data at offset %zd, nelem %d\n", tid, offset, nelem);
      // printf("runRing %d TID %d, Line 76", ringIx, tid);
      if (ringIx == 0) {
        // introduce artificial delay of 0.56ms
        for (int i = 0; i < 10000; i++) { 
          printf("");
      ...
      for (int j = 1; j < nranks - 1; ++j) {
        ...
        offset = gridOffset + elemOffset + chunkOffset;
        nelem = (int)min(chunkCount, remCount - chunkOffset);
        // printf("runRing: TID %d: Directly receiving and copying data at offset %zd, nelem %d\n", tid, offset, nelem);
        // printf("runRing %d TID %d, Line 90", ringIx, tid); // Happens only for nranks > 2
        if (ringIx == 0) {
            // introduce artificial delay of 0.56ms
            for (int i = 0; i < 10000; i++) { 
            printf("");
      ...
      offset = gridOffset + elemOffset + chunkOffset;
      nelem = (int)min(chunkCount, remCount - chunkOffset);
      // printf("runRing: TID %d: Directly receiving final data at offset %zd, nelem %d\n", tid, offset, nelem);
      // printf("runRing %d TID %d, Line 104", ringIx, tid);
      if (ringIx == 0) {
        // introduce artificial delay of 0.56ms
        for (int i = 0; i < 10000; i++) { 
          printf("");
  ...
  __device__ __forceinline__ void runTreeUpDown(int tid, int nthreads, struct ncclDevWorkColl* work) {
    ...
    printf("runTreeUpDown: TID %d: Up %d, Down %d\n", tid, tree->up, tree->down[0]);
    ...
    { // Reduce : max number of recv is 3, max number of send is 1 (binary tree + local)
      ...
      if (tree->up == -1) {
        for (size_t elemOffset = 0; elemOffset < channelCount; elemOffset += chunkCount) {
          ...
          printf("runTreeUpDown: TID %d: Receiving, reducing, and copying data at offset %zd, nelem %d\n", tid, offset, nelem);
        ...
        for (size_t elemOffset = 0; elemOffset < channelCount; elemOffset += chunkCount) {
          offset = gridOffset + elemOffset;
          nelem = min(chunkCount, channelCount - elemOffset);
          printf("runTreeUpDown: TID %d: Sending data at offset %zd, nelem %d\n", tid, offset, nelem);
          prims.send(offset, nelem);
        }
      }
      else {
        for (size_t elemOffset = 0; elemOffset < channelCount; elemOffset += chunkCount) {
          offset = gridOffset + elemOffset;
          nelem = min(chunkCount, channelCount - elemOffset);
          printf("runTreeUpDown: TID %d: Receiving, reducing, and sending data at offset %zd, nelem %d\n", tid, offset, nelem);
          prims.recvReduceSend(offset, nelem);
        }
      }
    }
    ...
    { // Broadcast : max number of recv is 1, max number of send is 3 (binary tree + local)
      ...
      if (tree->up == -1) {
        for (size_t elemOffset = 0; elemOffset < channelCount; elemOffset += chunkCount) {
          offset = gridOffset + elemOffset;
          nelem = min(chunkCount, channelCount - elemOffset);
          printf("runTreeUpDown: TID %d: Directly sending data from output at offset %zd, nelem %d\n", tid, offset, nelem);
        ...
        for (size_t elemOffset = 0; elemOffset < channelCount; elemOffset += chunkCount) {
          offset = gridOffset + elemOffset;
          nelem = min(chunkCount, channelCount - elemOffset);
          printf("runTreeUpDown: TID %d: Directly receiving data at offset %zd, nelem %d\n", tid, offset, nelem);
          prims.directRecv(offset, nelem);
        }
      }
      else {
        for (size_t elemOffset = 0; elemOffset < channelCount; elemOffset += chunkCount) {
          offset = gridOffset + elemOffset;
          nelem = min(chunkCount, channelCount - elemOffset);
          printf("runTreeUpDown: TID %d: Directly receiving and copying data at offset %zd, nelem %d\n", tid, offset, nelem);
          prims.directRecvCopySend(offset, nelem);
        }
      }
    }
  ...
  __device__ __forceinline__ void runTreeSplit(int tid, int nthreads, struct ncclDevWorkColl* work) {
    ...
    if (tree->up == -1) {
      ...
      for (size_t elemOffset = 0; elemOffset < channelCount; elemOffset += chunkCount) {
        offset = gridOffset + elemOffset;
        nelem = min(chunkCount, channelCount - elemOffset);
        printf("runTreeSplit: TID %d: Directly receiving, reducing, copying, and sending data at offset %zd, nelem %d\n", tid, offset, nelem);
      ...
      if (tree->down[0] == -1) {
        for (size_t elemOffset = 0; elemOffset < channelCount; elemOffset += chunkCount) {
          offset = gridOffset + elemOffset;
          nelem = min(chunkCount, channelCount - elemOffset);
          printf("runTreeSplit: TID %d: Sending data at offset %zd, nelem %d\n", tid, offset, nelem);
          prims.send(offset, nelem);
        }
      }
      else {
        for (size_t elemOffset = 0; elemOffset < channelCount; elemOffset += chunkCount) {
          offset = gridOffset + elemOffset;
          nelem = min(chunkCount, channelCount - elemOffset);
          printf("runTreeSplit: TID %d: Receiving, reducing, and sending data at offset %zd, nelem %d\n", tid, offset, nelem);
          prims.recvReduceSend(offset, nelem);
        }
      }
    }
      ...
      if (tree->down[0] == -1) {
        for (size_t elemOffset = 0; elemOffset < channelCount; elemOffset += chunkCount) {
          offset = gridOffset + elemOffset;
          nelem = min(chunkCount, channelCount - elemOffset);
          printf("runTreeSplit: TID %d: Directly receiving data at offset %zd, nelem %d\n", tid, offset, nelem);
          prims.directRecv(offset, nelem);
        }
      }
      else {
        for (size_t elemOffset = 0; elemOffset < channelCount; elemOffset += chunkCount) {
          offset = gridOffset + elemOffset;
          nelem = min(chunkCount, channelCount - elemOffset);
          printf("runTreeSplit: TID %d: Directly receiving and copying data at offset %zd, nelem %d\n", tid, offset, nelem);
          prims.directRecvCopySend(offset, nelem);
        }
      }
    }
