...
#else
  double pushThroughput, pushDuration;
  int  numTriggers, warmupTriggers;
  double syncThroughput, syncDuration;
  int syncNumTriggers, syncWarmupTriggers;
  std::tie(pushThroughput, pushDuration, numTriggers, warmupTriggers) =
      runSingleKernelVariant(kernelFifoPush, hostFifo, stream, numParallel);

  std::tie(syncThroughput, syncDuration, syncNumTriggers, syncWarmupTriggers) =
      runSingleKernelVariant(kernelFifoPushSync, hostFifo, stream, numParallel);
#endif
