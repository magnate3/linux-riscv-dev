...
int main(int argc, char** argv) {
    // Load image
    std::string imagepath("/pytorch/ncnn/build/onnx2ncnn/images/horse.png");
    ...
    // Load NCNN model
    ncnn::Net net;
    int ret = net.load_param("/pytorch/ncnn/build/onnx2ncnn/model.ncnn.param");
    if (ret) std::cerr << "Failed to load model parameters" << std::endl;
    ret = net.load_model("/pytorch/ncnn/build/onnx2ncnn/model.ncnn.bin");
