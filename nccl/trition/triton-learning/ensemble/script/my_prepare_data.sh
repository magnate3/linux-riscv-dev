...
if [ ! -f detection.onnx ] ; then
    sudo docker run --rm  -v ${WORKSPACE}:/workspace nvcr.io/nvidia/tensorflow:24.07-tf2-py3 /bin/bash -c \
...
if [ ! -f str.onnx ] ; then
    sudo docker run --rm  -v ${WORKSPACE}:/workspace nvcr.io/nvidia/pytorch:24.07-py3 /bin/bash -c \
    "cd /workspace/data && python /workspace/conceptual-guide/utils/cpu_pth2onnx.py --input None-ResNet-None-CTC.pth --output str.onnx"
fi

if [ ! -f batch_str.onnx ] ; then
    sudo docker run --rm  -v ${WORKSPACE}:/workspace nvcr.io/nvidia/pytorch:24.07-py3 /bin/bash -c \
    "cd /workspace/data && python /workspace/conceptual-guide/utils/cpu_pth2onnx.py --input None-ResNet-None-CTC.pth --output batch_str.onnx --batch"
fi
