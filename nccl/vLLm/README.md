...
{"id":"chat-f005c59baf4e409883726a7a3e58fa43","object":"chat.completion","created":1778813835,"model":"/models/Qwen2___5-0___5B-Instruct","choices":[{"index":0,"message":{"role":"assistant","content":"San Francisco, officially known as the \"San Francisco Bay Area\" or \"San Francisco Bay Area City\" in the United States, is a major metropolitan area located in the San Francisco Bay Area. It is the largest city in California by population, the fifth largest by area, and the first largest city in the contiguous United States.\n\nThe area includes San Francisco, Oakland, San Mateo, Alameda, Contra Costa, Marin, and Sonoma counties and surrounding areas. It is home to a diverse population, including 1.1 million people in the city and thousands of families in surrounding neighborhoods.\n\nSan Francisco is known for its striking skyline, historic architecture, and lively culture. It is a leading city in the United States in terms of business, finance, entertainment, and tourism. The city is also a major center for science, technology, and innovation, with world-renowned institutions and research institutions such as the University of California, Berkeley, San Francisco State University, and the University of California at Santa Cruz.","tool_calls":[]},"logprobs":null,"finish_reason":"stop","stop_reason":null}],"usage":{"prompt_tokens":33,"total_tokens":238,"completion_tokens":205},"prompt_logprobs":null}
```

+ 环境配置
```
root@ubuntu:/vllm-workspace# python3
...
```
ubuntu@ubuntu:~$ sudo docker exec -it  vllm-sch bash
root@ubuntu:/vllm-workspace# python3 --version
Python 3.12.6
root@ubuntu:/vllm-workspace# pip list | grep vllm
vllm                              0.6.2
root@ubuntu:/vllm-workspace# nvidia-smi 
Thu May 14 19:52:20 2026       
+-----------------------------------------------------------------------------------------+
...
| 31%   37C    P8             24W /  350W |   20692MiB /  24576MiB |      0%      Default |
|                                         |                        |                  N/A |
+-----------------------------------------+------------------------+----------------------+
...
+-----------------------------------------------------------------------------------------+
```
nvidia-smi 和 nvcc -V 不同驱动API (Driver API, nvidia-smi)：
显卡驱动所能支持的最高版本。    
运行API (Runtime API, nvcc -V)：用户安装的 CUDA Toolkit 版本。    
结论：nvcc 版本不能高于 nvidia-smi 显示的版本。    
...
> ## 模型并行
在 Transformer 的世界观里，一个 Layer 就是一个生产车间。   
+ 车间的前半轴是 Attention 流水线，里面并排开着 多个 Head（如 32/64 个） 的微型加工位；    
+ 车间的后半轴是 MLP 流水线，里面由 3 组庞大的矩阵升降维阵列组成    
...
[图解-vLLM源码之模型并行](https://zhuanlan.zhihu.com/p/716124020)    
Transformer的模型并行主要作用于attention、MLP、embedding和最后的softmax这些层，又以attention和MLP最为重要。无论是attention还是MLP的模型并行，都涉及到分布式矩阵乘法，故本节介绍一下分布式矩阵乘法。分布式矩阵乘法 Y = XA，分为行切和列切，这里的行切和列切指的是右侧A矩阵使用的是行切还是列切。   

![images](pipe1.png)
如上图展示了行切的例子，如果A使用了行切，那么X矩阵必须是列切的矩阵，由上图可以看出，绿色的矩阵在卡0，蓝色的矩阵在卡1，执行矩阵乘法之后，2张卡的数据均是全量（最终输出矩阵shape对的）不完整的数据，需要进行***AllReduce***才能得到完整正确的数据。    
...
+ 如果左边X为按照列切分的话，右边W必须按照行切分，结果需要经过AllReduce；  
+ 如果左边X如果没有切分的话，右边的W可以按照列切分，结果需要经过AllGather；  
...
当你调用 vLLM 时，ModelRunner 最终向 GPU 送入的平铺控制包通常包含以下组合（以 PyTorch Tensor 形式）：   
```
# vLLM 下发给底层的平铺 Batch 控制对象 (抽象自源码中的 InputTokens)
batched_inputs = {
    "input_ids": tensor([101, 102, 103, 104, 201, 301, 302, 303]), # 平铺的Token
    "cu_seq_lens": tensor([0, 4, 5, 8]),                         # 边界控制表
    "slot_mapping": tensor([32, 33, 34, 35, 94, 144, 145, 146]),   # 显存位置路由表
    "block_tables": tensor([[2, -1], [5, -1], [9, -1]])           # 历史块映射表(供Decode追溯)
}
...
# batch mask
将 Prefill 请求（处理新输入的 Prompt）和 Decode 请求（为老请求生成新 Token）混合在同一个批次（Batch）中运行，是 vLLM 能够实现高吞吐的关键，而 Mask（掩码）则是实现这种混合计算的核心数学桥梁。在 vLLM 底层，这种混合计算通常通过 块对角因果掩码（Block-Diagonal Causal Mask） 来实现。
