...
#define TEST_SET_UP_VM_FINAL_LOG 0
#define TEST_SET_UP_VM_FINAL 1
...
//#if 1
	...
	//memblock_reserve(0xc0000000, 0x180000000);
#else
...
#endif
	pr_info("debug  setup_vm_final step1 \n");
...
#if TEST_SET_UP_VM_FINAL_LOG
	                pr_info("debug  setup_vm_final pa %llx and map size %lx \n", pa, map_size);
#endif
			dbg_create_pgd_mapping(swapper_pg_dir, va, pa, map_size,
					   pgprot_from_va(va));
		}
	}
	...
	pr_info("debug  setup_vm_final step2 \n");
	...
	pr_info("debug  setup_vm_final step3 \n");
...
{
	setup_bootmem();
	pr_info("debug begin to setup_vm_final \n");
	setup_vm_final();
	pr_info("debug end setup_vm_final \n");
}
